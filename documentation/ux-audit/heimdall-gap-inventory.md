...
| D-6 | B | Dashboard `PageHeader` | `eyebrow="Main"` and `idChip="/"` were incorrectly set — prototype has no eyebrow on dashboard. Fixed: `eyebrow=""` (required prop, passed empty string). |
