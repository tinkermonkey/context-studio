...
class SearchRequest(BaseModel):
    ...
    analysis_depth: int | None = 1


class PathRequest(BaseModel):
    source_id: str
    target_id: str
    source_type: str | None = "term"
    target_type: str | None = "term"


class CentralityRequest(BaseModel):
    method: str | None = "pagerank"


class NeighborsRequest(BaseModel):
    entity_id: str
    entity_type: str | None = "term"
    depth: int | None = 1
...
async def execute_sparql_query(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"SPARQL query failed: {e!s}"
        )
...
async def export_rdf(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"RDF export failed: {e!s}"
        )
...
async def calculate_centrality(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Centrality calculation failed: {e!s}"
        )
...
async def detect_communities(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Community detection failed: {e!s}"
        )
...
async def export_graph_data(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Graph export failed: {e!s}"
        )
    ...
    return {
        ...
        "find_terms_in_domain": """
            ...
            SELECT ?term ?title WHERE {
                ?term a cs:Term ;
                      rdfs:label ?title ;
                      cs:is_a <http://context-studio.local/entity/domain/DOMAIN_ID> .  # noqa: E501
            }
        """,
        ...
        "find_term_relationships": """
            ...
            SELECT ?source ?predicate ?target ?sourceLabel ?targetLabel WHERE {
                ...
                FILTER(?predicate != rdf:type && ?predicate != rdfs:label && ?predicate != rdfs:comment)  # noqa: E501
            }
        """,
