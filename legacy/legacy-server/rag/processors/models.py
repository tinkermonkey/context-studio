...
class ProcessorInput(BaseModel):
    ...
    enable_trace: bool = Field(
        default=False, description="Enable trace capture for observability"
    )
...
class ExtractedPhrase(BaseModel):
    ...
    sentence_index: int = Field(
        ..., description="Index of sentence containing this phrase"
    )
...
class GapConcept(BaseModel):
    ...
    priority: GapPriority = Field(
        ..., description="Gap priority based on grammatical role"
    )
...
class ResolvedConcept(BaseModel):
    ...
    confidence: float = Field(
        ..., ge=0.0, le=1.0, description="Resolution confidence score"
    )
...
class ConceptResolutionOutput(ProcessorOutput):
    ...
    web_searches_performed: int = Field(
        default=0, description="Number of web searches performed"
    )
