...
class CreateAnnotationRequest(BaseModel):
    ...
    start_char: int = Field(
        ..., ge=0, description="Starting character position (inclusive)"
    )
    end_char: int = Field(
        ..., ge=0, description="Ending character position (exclusive)"
    )
    structure_node_id: str = Field(
        ..., min_length=1, description="ID of the structure node in local.db"
    )
