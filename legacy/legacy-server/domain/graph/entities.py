...
class GraphMetrics:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            isinstance(self.edge_count, bool)
            or not isinstance(self.edge_count, int)
        ...
        if (
            isinstance(self.connected_components, bool)
            or not isinstance(self.connected_components, int)
            or self.connected_components < 1
        ):
...
class PathResult:
    ...
    def __post_init__(self) -> None:
        ...
        if len(self.path) > 0:
            for node_id in self.path:
                if not isinstance(node_id, str):
                    raise TypeError(
                        f"All node IDs in path must be strings, got {type(node_id).__name__}"
                    )
        if (
        ...
        if len(self.path) > 0 and self.length != len(self.path) - 1:
            raise ValueError(
                "length must equal number of edges (nodes - 1) in the path"
            )
...
class KnowledgeGraph:
    ...
    def __post_init__(self) -> None:
        ...
        for node_id in self.nodes:
            if not isinstance(node_id, str):
                raise TypeError(
                    f"All node IDs must be strings, got {type(node_id).__name__}"
                )
        ...
        for edge_id in self.edges:
            if not isinstance(edge_id, str):
                raise TypeError(
                    f"All edge IDs must be strings, got {type(edge_id).__name__}"
                )
