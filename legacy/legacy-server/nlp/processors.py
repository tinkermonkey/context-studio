...
def extract_token_data(doc, filter: bool = False) -> List[TokenData]:
    ...
    def make_token_ref(tok):
        return TokenReference(
            ...
            end_idx=(
                ...
                if hasattr(tok, "idx") and hasattr(tok, "text")
                else None
            ),
        )
    ...
    relations_of_interest = [
        r.lower() for r in settings.concepcy_config.get("relations_of_interest", [])
    ]
    ...
    for token in doc_tokens:
        ...
        try:
                    ...
                    if hasattr(token._, rel):
                        ...
                        if rel_val:
                            ...
                            rel_items = (
                                rel_val
                                if (
                                    hasattr(rel_val, "__iter__")
                                    and not isinstance(rel_val, (str, bytes, dict))
                                )
                                else [rel_val]
                            )
                            for item in rel_items:
                                ...
                                try:
                                    ...
                                    if isinstance(item, dict):
                                        ...
                                        logger.warning(
                                            f"Unexpected item type for Concepcy relation '{rel}' on token '{token.text}': {type(item)}"
                                        )
                                except Exception as rel_e:
                                    logger.debug(
                                        f"Failed to build ConcepcyRelation for '{token.text}' relation '{rel}': {rel_e}"
                                    )
                if len(terms_list):
                        ...
                        if hasattr(synset_iter, "__iter__"):
                            for s in synset_iter:
                                try:
                                    synset_data = {
                                        "name": (
                                            str(s.name()) if hasattr(s, "name") else ""
                                        ),
                                        "definition": (
                                            str(s.definition())
                                            if hasattr(s, "definition")
                                            else ""
                                        ),
                                        "lemmas": (
                                            [str(lemma.name()) for lemma in s.lemmas()]
                                            if hasattr(s, "lemmas")
                                            else []
                                        ),
                                        "pos": (
                                            str(s.pos()) if hasattr(s, "pos") else ""
                                        ),
                                        "offset": (
                                            int(s.offset())
                                            if hasattr(s, "offset")
                                    ...
                                    logger.debug(
                                        f"Failed to process synset for '{token.text}': {synset_e}"
                                    )
                    except Exception as synsets_e:
                        logger.debug(
                            f"Failed to get synsets for '{token.text}': {synsets_e}"
                        )
                    try:
                            ...
                            for lemma in lemma_iter:
                                try:
                                    lemma_data = {
                                        ...
                                        "synset": (
                                            str(lemma.synset().name())
                                            if hasattr(lemma, "synset")
                                            and hasattr(lemma.synset(), "name")
                                            else ""
                                        ),
                                        "count": (
                                            int(lemma.count())
                                            if hasattr(lemma, "count")
                                            and lemma.count() is not None
                                            else 0
                                        ),
                                    }
                                    ...
                                    logger.debug(
                                        f"Failed to process lemma for '{token.text}': {lemma_e}"
                                    )
                    except Exception as lemmas_e:
                        logger.debug(
                            f"Failed to get lemmas for '{token.text}': {lemmas_e}"
                        )
                    wordnet = WordNetData(
                        synsets=synsets,
                        lemmas=lemmas,
            ...
            try:
                ...
                pos_val = (
                    str(token.pos_) if hasattr(token, "pos_") and token.pos_ else None
                )
                dep_val = (
                    str(token.dep_) if hasattr(token, "dep_") and token.dep_ else None
                )
                tag_val = (
                    str(token.tag_) if hasattr(token, "tag_") and token.tag_ else None
                )
                start_idx_val = token.idx if hasattr(token, "idx") else None
                end_idx_val = (
                    (token.idx + len(token.text))
                    if hasattr(token, "idx") and hasattr(token, "text")
                    else None
                )
                ...
                if hasattr(token, "children"):
                    try:
                        children_refs = [
                            make_token_ref(child) for child in token.children
                        ]
                    except Exception as e:
                        logger.debug(
                            f"Failed to extract children for '{token.text}': {e}"
                        )
                ...
                if hasattr(token, "ancestors"):
                    try:
                        ancestors_refs = [
                            make_token_ref(ancestor) for ancestor in token.ancestors
                        ]
                    except Exception as e:
                        logger.debug(
                            f"Failed to extract ancestors for '{token.text}': {e}"
                        )
                ...
                if hasattr(token, "subtree"):
                    try:
                        subtree_refs = [
                            make_token_ref(subtok) for subtok in token.subtree
                        ]
                    except Exception as e:
                        logger.debug(
                            f"Failed to extract subtree for '{token.text}': {e}"
                        )

                is_alpha_val = (
                    bool(token.is_alpha) if hasattr(token, "is_alpha") else None
                )
                ...
                like_url_val = (
                    bool(token.like_url) if hasattr(token, "like_url") else None
                )
                is_digit_val = (
                    bool(token.is_digit) if hasattr(token, "is_digit") else None
                )
                ent_iob_val = (
                    str(token.ent_iob_)
                    if hasattr(token, "ent_iob_") and token.ent_iob_
                    else None
                )
                ent_type_val = (
                    str(token.ent_type_)
                ...
                ent_kb_id_val = (
                    str(token.ent_kb_id_)
                    if hasattr(token, "ent_kb_id_") and token.ent_kb_id_
                    else None
                )
                ent_id_val = (
                    int(token.ent_id)
                    if hasattr(token, "ent_id") and token.ent_id is not None
                    else None
                )
                sentiment_val = (
                    float(token.sentiment)
                    if hasattr(token, "sentiment") and token.sentiment is not None
                    else None
                )
                ...
                logger.warning(
                    f"TokenData construction failed for '{token.text}': {te}"
                )
                try:
                    tokens.append(TokenData(text=str(token.text) if hasattr(token, "text") else ""))  # type: ignore
                except Exception as backup_e:
                    logger.error(
                        f"Even backup TokenData construction failed: {backup_e}"
                    )
...
def extract_entity_data(doc) -> List[EntityData]:
    ...
    for ent in doc.ents:
        try:
            ...
            raw_result = (
                getattr(ent._, "dbpedia_raw_result", None)
                if hasattr(ent._, "dbpedia_raw_result")
                else None
            )
            similarity = (
                getattr(ent._, "dbpedia_similarity", None)
                if hasattr(ent._, "dbpedia_similarity")
                else None
            )
            uri = (
                getattr(ent._, "dbpedia_uri", None)
                if hasattr(ent._, "dbpedia_uri")
                else None
            )
            label = (
                getattr(ent._, "dbpedia_label", None)
                if hasattr(ent._, "dbpedia_label")
                else None
            )
            if uri or label or similarity or raw_result:
                dbpedia = DBpediaData(
                    uri=uri, label=label, similarity=similarity, raw_result=raw_result
                )
            entities.append(
                EntityData(
                    text=ent.text,
...
def process_nlp_result(text: str, doc: Any) -> NLPAnalysisResponse:
    ...
    return NLPAnalysisResponse(text=text, tokens=tokens, entities=entities)
