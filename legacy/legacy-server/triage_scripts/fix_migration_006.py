...
def fix_database(db_path: str, dry_run: bool = True):
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            result = conn.execute(
                text(
                    "SELECT version, description, checksum FROM schema_history WHERE version = 6"
                )
            )
            ...
            print(
                f"change_events table: {'EXISTS' if change_events_exists else 'MISSING'}"
            )
            print(
                f"structure_nodes table: {'EXISTS' if structure_nodes_exists else 'MISSING'}"
            )
            ...
            try:
                ...
                spec = importlib.util.spec_from_file_location(
                    "migration_006", migration_006_path
                )
                if not spec or not spec.loader:
                    print(f"❌ Failed to load migration 006 from {migration_006_path}")
                    return False
                ...
                try:
                    ...
                    if os.path.exists(migration_007_path):
                        ...
                        spec = importlib.util.spec_from_file_location(
                            "migration_007", migration_007_path
                        )
                        if not spec or not spec.loader:
                            print(
                                f"❌ Failed to load migration 007 from {migration_007_path}"
                            )
                            raise ValueError(
                                f"Unable to load migration module from {migration_007_path}"
                            )
...
def main():
    ...
    parser.add_argument("--database", type=str, help="Fix specific database file")
