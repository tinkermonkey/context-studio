...
class CRDTMergeEngine:
    ...
    def merge_changeset(
        ...
        try:
            ...
            logger.info(
                f"Found {len(changeset_versions)} versions to merge in changeset {changeset_id}"
            )
            ...
            for (entity_type, entity_id), versions in entities_to_merge.items():
                logger.debug(
                    f"Processing entity {entity_type}:{entity_id} with {len(versions)} versions"
                )
                ...
                try:
                    ...
                    merged_version = self.version_manager.create_version(
                        ...
                        metadata={
                            "merge_source": "crdt_auto_merge",
                            "source_versions": [v.id for v in versions],
                        },
                    )

                    # Update canonical reference
                    self._update_canonical_version(
                        entity_type, entity_id, merged_version.id
                    )
                    ...
                    logger.debug(
                        f"Successfully merged entity {entity_type}:{entity_id}"
                    )

                except CRDTConflictError as e:
                    logger.warning(
                        f"CRDT conflict for entity {entity_type}:{entity_id}: {e}"
                    )
                    conflicts.append(
                        {
                            "entity_type": entity_type,
            ...
            if not conflicts:
                self.changeset_manager.update_changeset_state(
                    changeset_id, ChangesetState.MERGED
                )
                logger.info(
                    f"Successfully merged changeset {changeset_id} with no conflicts"
                )
            else:
                logger.warning(
                    f"Changeset {changeset_id} merge completed with {len(conflicts)} conflicts"
                )
            ...
            result = ChangesetMergeResult(
                changeset_id=changeset_id,
                merged_entities=len(
                    [r for r in merge_results if r.status == "success"]
                ),
            ...
            logger.info(
                f"Changeset {changeset_id} merge completed: {result.merged_entities} merged, {result.conflicts} conflicts"
            )
            return result
    ...
    def resolve_conflict(
        ...
        """
        logger.info(
            f"Manually resolving conflict for {entity_type}:{entity_id} by {resolver_id}"
        )
        ...
        try:
            ...
            logger.error(
                f"Failed to resolve conflict for {entity_type}:{entity_id}: {e}"
            )
            self.db.rollback()
    ...
    def _get_canonical_version(
        ...
        try:
            ...
            if not result:
                logger.debug(
                    f"No canonical version found for {entity_type}:{entity_id}"
                )
                return None
            ...
            if not version_result:
                logger.warning(
                    f"Canonical version {canonical_version_id} not found for {entity_type}:{entity_id}"
                )
                return None
            ...
            logger.error(
                f"Failed to get canonical version for {entity_type}:{entity_id}: {e}"
            )
            return None
    ...
    def _apply_crdt_merge(
        ...
        for version in changeset_versions:
            try:
                merged_content = self._merge_content_crdt(
                    merged_content, version.content
                )
    ...
    def _merge_content_crdt(
        ...
        if conflicting_fields:
            raise CRDTConflictError(
                f"Unresolvable conflicts in fields: {', '.join(conflicting_fields)}"
            )

        return result

    def _can_resolve_scalar_conflict(
    ...
    def _resolve_scalar_conflict(
        ...
        if (
            ...
            if isinstance(base_value, str) and isinstance(change_value, str):
                ...
                )  # String comparison for ISO timestamps
    ...
    def _update_canonical_version(
        ...
        """
        logger.debug(
            f"Updating canonical version for {entity_type}:{entity_id} to {version_id}"
        )
        ...
        try:
            ...
            logger.error(
                f"Failed to update canonical version for {entity_type}:{entity_id}: {e}"
            )
            raise
