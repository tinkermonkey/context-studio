...
class HierarchicalDiffEngine:
    ...
    def compute_hierarchical_diff(
        ...
        if self._has_circular_references(before) or self._has_circular_references(
            after
        ):
        ...
        semantic_analysis = self._compute_semantic_analysis_for_operations(
        ...
        self._record_performance_metrics(
            "compute_hierarchical_diff", processing_time_ms, operations, before, after
        )

        logger.debug(
            f"Hierarchical diff completed in {processing_time_ms:.2f}ms with {len(operations)} operations"
        )
    ...
    def _break_circular_references(
        ...
        if isinstance(obj, dict):
            ...
            return [
                self._break_circular_references(item, visited.copy()) for item in obj
            ]
        ...
        for op in operations:
            if op["operation"] == "modify":
                ...
                if old_val is not None and new_val is not None:
                    ...
                    if similarity < 0.7:
                        semantic_changes = semantic_analysis["semantic_changes"]
                        if isinstance(semantic_changes, dict):
                            semantic_changes[op["path"]] = {
                                "similarity_score": similarity,
                                "change_type": "significant_modification",
                            }

        if similarity_count > 0:
            semantic_analysis["content_similarity"] = (
                total_similarity / similarity_count
            )
        else:
            # If no modifications, calculate similarity based on structure
            semantic_analysis["content_similarity"] = (
                self._calculate_semantic_similarity(before, after)
            )
        ...
        if self.nlp_service:
            try:
                ...
                text_changes = [
                    op
                    for op in operations
                    if isinstance(op.get("old_value"), str)
                    and isinstance(op.get("new_value"), str)
                ]
    ...
    def _analyze_semantic_conflict(
        ...
        base_local_similarity = self._calculate_semantic_similarity(
            base_value, local_value
        )
        base_remote_similarity = self._calculate_semantic_similarity(
            base_value, remote_value
        )
        local_remote_similarity = self._calculate_semantic_similarity(
            local_value, remote_value
        )

        # Determine conflict type based on similarity scores
        avg_similarity = (
            base_local_similarity + base_remote_similarity + local_remote_similarity
        ) / 3
    ...
    def compute_three_way_diff(
        ...
        for operation in local_diff["operations"]:
            if operation["path"] not in conflict_paths:
                mergeable_changes.append({"source": "local", "operation": operation})

        # Add non-conflicting remote changes
        for operation in remote_diff["operations"]:
            if operation["path"] not in conflict_paths:
                mergeable_changes.append({"source": "remote", "operation": operation})
        ...
        logger.info(
            f"Three-way diff completed: {len(conflicts)} conflicts found in {computation_time:.2f}ms"
        )
            ...
            for key in common_keys:
                ...
                if before_val != after_val:
                    ...
                    if type(before_val) is not type(after_val):
                        ...
                        nested_diff = self._compute_structural_diff(
                            before_val, after_val
                        )
                        modified_values = result["modified_values"]
                        if isinstance(modified_values, dict):
                            modified_values[key] = nested_diff
                        ...
                        if isinstance(modified_values, dict):
                            modified_values[key] = {
                                "before": before_val,
                                "after": after_val,
                                "change_type": self._classify_value_change(
                                    before_val, after_val
                                ),
    ...
    def _compute_list_diff(
        ...
        for op, i1, i2, j1, j2 in opcodes:
            if op == "delete":
                ...
                removed_items_list = list_changes["removed_items"]
                if isinstance(removed_items_list, list):
                    removed_items_list.extend(removed_items)
                operations_list = list_changes["operations"]
                ...
                added_items_list = list_changes["added_items"]
                if isinstance(added_items_list, list):
                    added_items_list.extend(added_items)
                operations_list = list_changes["operations"]
                if isinstance(operations_list, list):
                    operations_list.append(
                        {
        ...
        for key, change in structural_diff.get("modified_values", {}).items():
            if isinstance(change, dict) and "before" in change and "after" in change:
                ...
                semantic_changes[key] = {
                    ...
                    "change_classification": self._classify_semantic_change(
                        similarity_score
                    ),
                    "impact_level": self._assess_change_impact(
                        key, change, similarity_score
                    ),
                }
    ...
    def _identify_three_way_conflicts(
        ...
        for path in common_paths:
            ...
            if is_conflict:
                conflict_id = hashlib.sha256(
                    f"conflict_{path}_{time.time()}".encode()
                ).hexdigest()[:16]
                ...
                confidence_score = (
                    0.95 if local_op["operation"] != remote_op["operation"] else 0.8
                )
                ...
                conflict = ConflictDescriptor(
                    ...
                    severity=self._calculate_conflict_severity(
                        conflict_type, confidence_score, local_value, remote_value
                    ),
                )
        ...
        add_remove_conflicts = (local_added & remote_removed) | (
            remote_added & local_removed
        )

        for key in add_remove_conflicts:
            conflict_id = hashlib.sha256(
                f"add_remove_conflict_{key}_{time.time()}".encode()
            ).hexdigest()[:16]
    ...
    def _generate_merge_result(
        ...
        for key in local_structural.get("added_keys", set()):
            if key not in conflict_keys:
                merge_result[key] = (
                    local_diff["local_changes"][key]
                    if "local_changes" in local_diff
                    else None
                )
        ...
        for key in remote_structural.get("added_keys", set()):
            if key not in conflict_keys:
                merge_result[key] = (
                    remote_diff["remote_changes"][key]
                    if "remote_changes" in remote_diff
                    else None
                )
    ...
    def _analyze_change_patterns(
        ...
        if added_count > 10 or removed_count > 10:
            patterns["bulk_operations"].append(
                {
                    "type": (
                        "bulk_add" if added_count > removed_count else "bulk_remove"
                    ),
            ...
            patterns["refactoring_indicators"].append(
                {
                    ...
                    "confidence": sum(elem["confidence"] for elem in moved_elements)
                    / len(moved_elements),
                }
            )
            ...
            suggestions.append(
                {
                    ...
                    "confidence": 0.9,
                    "actions": [
                        "Apply semantic merge where possible",
                        "Use last-writer-wins for simple conflicts",
                    ],
                }
            )
            ...
            suggestions.append(
                {
                    ...
                    "actions": [
                        ...
                        "Break down into smaller merges",
    ...
    def _calculate_conflict_severity(
        ...
        if any(
            pattern in str(local_value).lower() + str(remote_value).lower()
    ...
    def _classify_value_change(self, before_val: Any, after_val: Any) -> str:
        ...
        elif isinstance(before_val, (int, float)) and isinstance(
            after_val, (int, float)
        ):
            return "numeric_modification"
        elif isinstance(before_val, (list, dict)) and isinstance(
            after_val, (list, dict)
        ):
    ...
    def _values_semantically_equal(
        self, val1: Any, val2: Any, threshold: float = 0.9
    ) -> bool:
    ...
    def _classify_conflict_type(
        ...
        elif isinstance(local_val, (int, float)) and isinstance(
            remote_val, (int, float)
        ):
            return "numeric_conflict"
        elif isinstance(local_val, (dict, list)) and isinstance(
            remote_val, (dict, list)
        ):
    ...
    def _generate_conflict_resolution_suggestions(
        ...
        suggestions.sort(key=get_confidence, reverse=True)
    ...
    def _calculate_diff_metadata(self, diff_result: dict[str, Any]) -> dict[str, Any]:
        ...
        change_depth = self._calculate_change_depth(
            structural.get("modified_values", {})
        )
        ...
        for change in modified_values.values():
            if isinstance(change, dict) and "modified_values" in change:
                nested_depth = self._calculate_change_depth(
                    change["modified_values"], current_depth + 1
                )
    ...
    def _calculate_change_complexity(self, diff_result: dict[str, Any]) -> float:
        ...
        pattern_complexity = (
            len(diff_result.get("patterns", {}).get("bulk_operations", [])) * 0.4
        )

        total_complexity = (
            base_complexity + move_complexity + type_complexity + pattern_complexity
        )
    ...
    def get_performance_statistics(self) -> dict[str, Any]:
        ...
        total_operations = sum(
            metric["operations_count"] for metric in self.performance_metrics
        )
        total_execution_time = sum(
            metric["execution_time_ms"] for metric in self.performance_metrics
        )
        total_data_bytes = sum(
            metric["data_size_bytes"] for metric in self.performance_metrics
        )
        ...
        if total_execution_time > 0:
            ...
            ) / total_execution_time  # Convert ms to seconds
        else:
            operations_per_second = 0.0
