...
class ProposalManager:
    ...
    def create_proposal(
        ...
        if changeset.state not in [ChangesetState.DRAFT, ChangesetState.PROPOSED]:
            raise ValueError(
                f"Cannot create proposal for changeset in {changeset.state.value} state"
            )
        ...
        try:
            ...
            self.changeset_manager.update_changeset_state(
                changeset_id, ChangesetState.PROPOSED
            )
            ...
            logger.info(
                f"Successfully created proposal {proposal_id} for changeset {changeset_id}"
            )
            return proposal
    ...
    def vote_on_proposal(
        ...
        if proposal.status != ProposalStatus.OPEN:
            raise ValueError(
                f"Cannot vote on proposal with status {proposal.status.value}"
            )
        ...
        try:
            ...
            if existing_vote:
                logger.info(
                    f"Updated vote for user {user_id} on proposal {proposal_id}"
                )
            else:
                logger.info(
                    f"Recorded new vote for user {user_id} on proposal {proposal_id}"
                )
    ...
    def list_proposals(
        ...
        """
        logger.debug(
            f"Listing proposals (status={status}, created_by={created_by}, limit={limit})"
        )
    ...
    def get_user_vote(self, proposal_id: str, user_id: str) -> ProposalVote | None:
        ...
        try:
            result = self.db.execute(
                text(
                    "SELECT * FROM proposal_votes WHERE proposal_id = :proposal_id AND user_id = :user_id"
                ),
    ...
    def _evaluate_proposal_status(self, proposal_id: str):
        ...
        try:
            ...
            if approve_votes >= proposal.required_approvals:
                logger.info(
                    f"Auto-approving proposal {proposal_id} ({approve_votes}/{proposal.required_approvals})"
                )
            ...
            # (This is a simple policy, could be made configurable)
                ...
                reject_votes > approve_votes
                and reject_votes >= proposal.required_approvals
            ):
                logger.info(
                    f"Auto-rejecting proposal {proposal_id} ({reject_votes} rejections)"
                )
    ...
    def _update_proposal_status(
        ...
        try:
            ...
            logger.info(
                f"Successfully updated proposal {proposal_id} status to {new_status.value}"
            )
            return True
    ...
    def _store_vote_locally(self, vote: ProposalVote, is_update: bool = False) -> None:
        ...
        """
        logger.debug(
            f"Storing vote from {vote.user_id} for proposal {vote.proposal_id}"
        )
    ...
    def _push_proposal_to_s3(self, proposal: Proposal):
        ...
        if not self.s3_sync.s3_config:
            logger.debug(
                f"Skipping S3 push for proposal {proposal.id} - S3 not configured"
            )
            return
        ...
        try:
            ...
            if not success:
                logger.warning(
                    f"Failed to push proposal {proposal.id} to S3: write operation returned False"
                )
            else:
            ...
            logger.warning(
                f"Failed to push proposal {proposal.id} to S3: {type(e).__name__}: {e}"
            )
    ...
    def _push_vote_to_s3(self, vote: ProposalVote):
        ...
        try:
            ...
            if not success:
                logger.warning(
                    f"Failed to push vote to S3 for proposal {vote.proposal_id}: write operation returned False"
                )
            else:
                logger.debug(
                    f"Successfully pushed vote to S3 for proposal {vote.proposal_id}"
                )

        except Exception as e:
            logger.warning(
                f"Failed to push vote to S3 for proposal {vote.proposal_id}: {type(e).__name__}: {e}"
            )
            # Don't fail the operation if S3 push fails
