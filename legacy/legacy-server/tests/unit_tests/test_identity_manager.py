...
class TestIdentityManager:
    ...
    def identity_manager(self, mock_db_session, mock_s3_sync_manager):
        ...
        return IdentityManager(db=mock_db_session, s3_sync_manager=mock_s3_sync_manager)
    ...
    def test_initialization(
        self, identity_manager, mock_db_session, mock_s3_sync_manager
    ):
    ...
    def test_register_user_invalid_email(self, identity_manager):
        ...
        with pytest.raises(ValueError, match="Invalid email format"):
            identity_manager.register_user(
                email="invalid-email", display_name="Test User"
            )

        with pytest.raises(ValueError, match="Invalid email format"):
            identity_manager.register_user(email="test@", display_name="Test User")
    ...
    def test_register_user_empty_fields(self, identity_manager):
        ...
        with pytest.raises(ValueError, match="Email and display name are required"):
            identity_manager.register_user(email="", display_name="Test User")

        with pytest.raises(ValueError, match="Email and display name are required"):
            identity_manager.register_user(email="test@example.com", display_name="")
    ...
    def test_verify_email_no_code(self, identity_manager):
        ...
        with patch.object(
            identity_manager, "_get_verification_code", return_value=None
        ):
            success = identity_manager.verify_email("user123", "123456")
    ...
    def test_trust_user_success(self, identity_manager, mock_db_session):
        ...
        with patch.object(identity_manager, "get_user") as mock_get_user:
            ...
            with patch.object(
                identity_manager, "_count_trust_relationships", return_value=1
            ):
    ...
    def test_trust_user_promotion_to_team_verified(
        self, identity_manager, mock_db_session
    ):
        ...
        with patch.object(identity_manager, "get_user") as mock_get_user:
            mock_get_user.side_effect = [mock_trustee, mock_trusted]
            with patch.object(
                identity_manager, "_count_trust_relationships", return_value=2
            ):
        ...
        assert (
            call_found
        ), f"Expected trust_level=2 update not found in calls: {mock_db_session.execute.call_args_list}"
    ...
    def test_list_users_with_filters(self, identity_manager, mock_db_session):
        ...
        users = identity_manager.list_users(
            verified_only=True, min_trust_level=2, limit=50
        )
    ...
    def test_get_trust_network_success(self, identity_manager, mock_db_session):
        ...
        mock_trusted_rows = [("trusted1", "2023-01-03T00:00:00+00:00")]
    ...
    def test_verification_code_cleanup_on_verification(
        self, identity_manager, mock_db_session
    ):
