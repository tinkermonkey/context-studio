...
class TestNodeConversionEmbeddingDeserialization:
    ...
    def test_deserialize_definition_embedding_from_numpy_bytes(
        self, sample_embedding_bytes
    ):
...
class TestEmbeddingErrorRecovery:
    ...
    def test_mismatched_embedding_size_handled(self):
        ...
        assert result.title == "Test Node"
        assert (
            result.title_embedding is None
        )  # Misaligned embedding should be excluded when error occurs
