...
class DBpediaSource(BaseReferenceSource):
    ...
    async def get_resource_data(
        self, resource_url: str, format: str = "json"
    ) -> DBpediaResourceResponse:
        try:
    ...
    async def search(
        self, query: str, limit: int = 10, offset: int = 0, format: str = "json"
    ) -> DBpediaSearchResponse:
        try:
            ...
            if isinstance(response_data, dict) and "docs" in response_data:
                for item in response_data.get("docs", []):
                    ...
                    label = (
                        label.replace("<B>", "").replace("</B>", "") if label else ""
                    )

                    # Comments contain descriptions
                    description = (
                        item.get("comment", [""])[0] if item.get("comment") else ""
                    )
                    description = (
        ...
        try:
            ...
            headers = {
                "Accept": (
                    ...
                    if format == "json"
                    else "application/sparql-results+xml"
                )
            }
