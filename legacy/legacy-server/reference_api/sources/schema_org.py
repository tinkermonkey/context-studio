...
class SchemaOrgSource(BaseReferenceSource):
    ...
    async def get_entity(
        ...
        try:
            ...
            if data is None:
                return SchemaOrgEntityResponse(
                    **self._create_base_response(success=False, error="not_found")
                )
            return SchemaOrgEntityResponse(
                **self._create_base_response(), identifier=identifier, entity=data
            )
    ...
    async def get_property(
        self, identifier: str, include_usage: bool = True
    ) -> SchemaOrgPropertyResponse:
        try:
            ...
            data = await loop.run_in_executor(None, _lookup)
            if data is None:
                return SchemaOrgPropertyResponse(
                    **self._create_base_response(success=False, error="not_found")
                )
            return SchemaOrgPropertyResponse(
                **self._create_base_response(), identifier=identifier, property=data
            )
        except Exception as e:
            return SchemaOrgPropertyResponse(
                **self._create_base_response(success=False, error=str(e))
            ...
            def _search():
                ...
                with ReferenceManager(self.ref_config) as ref_manager:
                    try:
                        ...
                        logger.warning(
                            "Vector search not available: embeddings module not found"
                        )
                        return []

            loop = __import__("asyncio").get_event_loop()
            results = await loop.run_in_executor(None, _search)
            return SchemaOrgSearchResponse(
