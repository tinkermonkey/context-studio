...
class DBpediaResourceRequest(BaseModel):
    ...
    def validate_resource_url(cls, v):
        ...
        if "dbpedia.org/resource/" not in v:
            raise ValueError(
                "Must be a valid DBpedia resource URL containing dbpedia.org/resource/"
            )
        return v
...
class WikidataEntityRequest(BaseModel):
    ...
    def validate_entity_url(cls, v):
        ...
        if not ("wikidata.org" in v and ("/wiki/Q" in v or "/entity/Q" in v)):
            raise ValueError(
                "Must be a valid Wikidata entity URL containing wikidata.org and entity/Q pattern"
            )
        return v
...
class SchemaOrgPropertyRequest(BaseModel):
    ...
    include_usage: bool = Field(
        True, description="Include entities that use this property"
    )
...
class SchemaOrgSearchRequest(BaseModel):
    ...
    query: str = Field(..., min_length=1, description="Search query")
    search_type: Literal["entities", "properties", "both"] = Field(
        "both", description="Type of items to search"
    )
    limit: int = Field(20, ge=1, le=100, description="Result limit")
    offset: int = Field(0, ge=0, description="Result offset")
    similarity_threshold: float = Field(
        0.7, ge=0.0, le=1.0, description="Similarity threshold for semantic search"
    )
...
class MultiSourceSearchRequest(BaseModel):
    ...
    limit: int = Field(
        default=20, ge=1, le=100, description="Maximum number of results per source"
    )
