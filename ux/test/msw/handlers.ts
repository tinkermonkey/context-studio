...
export const handlers = [
  ...
  rest.put(new RegExp('.*/api/domains/.+'), async (req, res, ctx) => {
    ...
    if (!body || typeof body.title !== 'string' || body.title.trim() === '') {
      return res(ctx.status(400), ctx.json({ message: 'Validation failed: title is required' }));
    }
  ...
  rest.get('/api/domains/:id', (req, res, ctx) => {
    ...
    return res(ctx.status(200), ctx.json({ data: { id, title: `Domain ${id}` } }));
  }),
  ...
  rest.post('/api/domains', async (req, res, ctx) => {
    const body = await req.json();
    return res(ctx.status(201), ctx.json({ data: { ...body, id: 'new-domain' } }));
  }),
  ...
  rest.get('/api/layers', (req, res, ctx) => {
    return res(ctx.status(200), ctx.json({ data: [{ id: 'layer-1', title: 'Layer 1' }] }));
  }),
  ...
  rest.get('/api/layers/:id', (req, res, ctx) => {
    const { id } = req.params as { id: string };
    return res(ctx.status(200), ctx.json({ data: { id, title: `Layer ${id}` } }));
  }),
  ...
  rest.get('/api/terms', (req, res, ctx) => {
    return res(ctx.status(200), ctx.json({ data: [{ id: 'term-1', title: 'Term 1' }] }));
  }),
  ...
  rest.post('/api/terms', async (req, res, ctx) => {
    const body = await req.json();
    return res(ctx.status(201), ctx.json({ data: { ...body, id: 'new-term' } }));
  }),
