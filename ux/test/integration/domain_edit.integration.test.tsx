...
vi.mock('@tanstack/react-router', () => {
  ...
  return { Link: (props: any) => React.createElement('a', props, props.children) };
});
vi.mock('@/api/hooks/layers/useLayers', () => ({
  useLayer: (id: string) => ({ data: { id, title: 'Layer 1' }, isLoading: false }),
  useLayers: () => ({ data: [{ id: 'layer-1', title: 'Layer 1' }], isLoading: false }),
}));
vi.mock('@/api/hooks/terms/useTerms', () => ({
  useTerms: (_params: any) => ({ data: [], isLoading: false }),
}));

import { renderWithProviders as render, makeTestQueryClient } from '@/test/utils/renderWithProviders';
...
describe('DomainDetails edit flow', () => {
  ...
  const updateSpy = vi.spyOn(domainService, 'update').mockResolvedValue(updatedDomain as any);
  ...
  const save = screen.getByRole('button', { name: /save|save changes|create domain/i });
  ...
  expect(invalidateSpy).toHaveBeenCalledWith({ queryKey: [QUERY_KEYS.DOMAINS] });
  // check for per-domain invalidation
  expect(invalidateSpy).toHaveBeenCalledWith({ queryKey: [QUERY_KEYS.DOMAINS, domain.id] });
