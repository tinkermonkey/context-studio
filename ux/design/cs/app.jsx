...
function CSApp() {
  ...
  const [ncDesc, setNcDesc] = useState('Motor neurons carry signals from the central nervous system to muscle fibers.');
  ...
  else if (route === 'schema/classes')         Page = <CSSchemaClasses route={route} onNav={navigate} onOpenNewClass={() => setNewClassOpen(true)} />;
  ...
  const paletteCommands = useMemo(() => [
    ...
    { id: 'go-classes',       group: 'Navigate', label: 'Go to Classes',          description: '/schema/classes',                   icon: 'schema',    onSelect: () => navigate('schema/classes') },
    ...
    { id: 'go-pipelines',     group: 'Navigate', label: 'Go to Pipelines',        description: '/pipelines/all',                    icon: 'pipeline',  onSelect: () => navigate('pipelines/all') },
    { id: 'go-graph',         group: 'Navigate', label: 'Go to Graph view',       description: 'Inferred KG layout',                icon: 'graph',     onSelect: () => navigate('graph') },
    { id: 'new-class',        group: 'Actions',  label: 'New class…',             description: 'Open the new-class modal',          icon: 'plus',      onSelect: () => setNewClassOpen(true) },
    { id: 'new-pipeline-run', group: 'Actions',  label: 'Run pipeline…',          description: 'Trigger a pipeline run',            icon: 'pipeline',  onSelect: () => setToast({ variant:'info', title:'Pipeline scheduled', subtitle:'pubmed_genes · next slot' }) },
  ], [route]);
  ...
  return (
    <>
      ...
      <Modal
        ...
        footer={
          <>
            <Button variant="ghost" onClick={() => setNewClassOpen(false)}>Cancel</Button>
            <Button variant="primary" onClick={() => {
              setNewClassOpen(false);
              setToast({ variant: 'success', title: 'Class created', subtitle: `${ncIdent} · ${ncScheme}` });
            }}>Create class</Button>
          </>
        }
      >
        ...
        <Field label="Concept scheme" required>
          <Select value={ncScheme} onChange={setNcScheme}>
            {window.CS_DATA.concept_schemes.map((s) => (
              <Select.Item key={s.id} value={s.id}>{s.title} · {s.id}</Select.Item>
            ))}
          </Select>
        </Field>
        <Field label="Parent class" hint="optional">
          <Select value={ncParent} onChange={setNcParent} placeholder="— none —">
            <Select.Item value="">— none —</Select.Item>
            {window.CS_DATA.classes.map((c) => (
              <Select.Item key={c.id} value={c.id}>{c.title} · {c.id}</Select.Item>
            ))}
          </Select>
        </Field>
