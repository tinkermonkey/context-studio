...
const EntitySurface = forwardRef(function EntitySurface(
  { type, rows, columns, bulkOps = [{ id: "delete", label: "Delete", icon: "trash", danger: true, kind: "delete" }],
    createContext, inspectorEmpty, inspectorWidth = 420, childAction }, ref) {
  ...
  const cancelCreate = () => { setMode(entity ? "view" : "view"); setInitialDraft(null); };

  const duplicate = (e) => {
    const base = { ...e };
    if (base.identifier) { base.identifier = base.identifier + "_copy"; base.id = ""; }
    else { base.id = (base.id || "") + "_copy"; }
    base.title = (base.title || base.name || "") + " (copy)";
    base.version = undefined; base.updated = undefined;
    setInitialDraft(base); setSelectedId(null); setMode("create"); setDraftKey((k) => k + 1);
  };

  const askDelete = (e) => setDel({ open: true, target: e, ids: null });
  const confirmDelete = () => {
    if (del.ids) { CSStore.bulkRemove(type, del.ids); setSelection(new Set()); }
    else if (del.target) { CSStore.remove(type, del.target.id); if (selectedId === del.target.id) setSelectedId(null); }
  ...
  const rowActions = () => [
    ...
    ...(childAction ? [{ id: "__child", label: childAction.label, icon: childAction.icon || "plus" }] : []),
    { type: "separator" },
  ...
  const runBulk = (opId) => {
    ...
    if (!op) return;
    if (op.kind === "delete") { setDel({ open: true, target: null, ids: [...selection] }); return; }
    setBulkOp({ op, value: op.default ?? (op.options && op.options[0] ? op.options[0].value : "") });
  };
  const applyBulk = () => {
    ...
    setBulkOp(null); setSelection(new Set());
  };
    ...
    <>
      {(isCreate || !isEmpty) ? (
        <div style={{ display: "grid", gridTemplateColumns: (isCreate || showRail) ? `1fr ${inspectorWidth}px` : "1fr", gap: 14, alignItems: "start" }}>
          ...
          />
          {isCreate ? (
            <CreateDrawer inline key={`create-${draftKey}`} type={type} initialDraft={initialDraft}
              onCreated={onCreated} onClose={cancelCreate} />
          ) : showRail && entity ? (
            <EntityInspector key={`${entity.id}-${mode}`} type={type} entity={entity} mode={mode}
              onEnterEdit={enterEdit} onDone={done}
              onDelete={() => askDelete(entity)} onDuplicate={() => duplicate(entity)} />
          ) : null}
        </div>
      ) : (
        <CreateEmptyState icon={(inspectorEmpty && inspectorEmpty.icon) || "layout"} noun={CSStore.NICE[type]}
          onCreate={() => startCreate()} />
      )}

      <BulkBar
        count={selection.size} label={CSStore.NICE[type]}
        actions={bulkOps.map((o) => ({ id: o.id, label: o.label, icon: o.icon, danger: o.danger }))}
        onAction={runBulk} onClear={clearSel}
      />

      <CascadeDeleteDialog open={del.open} type={type} target={del.target} ids={del.ids}
        onClose={() => setDel({ open: false, target: null, ids: null })} onConfirm={confirmDelete} />

      {bulkOp && (
        <Modal isOpen onClose={() => setBulkOp(null)} title={bulkOp.op.title || bulkOp.op.label}
          subtitle={`Applies to ${selection.size} selected ${CSStore.NICE[type]}${selection.size === 1 ? "" : "s"}.`}
          footer={<><Button variant="ghost" onClick={() => setBulkOp(null)}>Cancel</Button><Button variant="primary" onClick={applyBulk}>Apply</Button></>}>
          {bulkOp.op.kind === "segmented" ? (
            <SegmentedControl value={bulkOp.value} onChange={(v) => setBulkOp((b) => ({ ...b, value: v }))} options={bulkOp.op.options} />
          ) : (
            <Field label={bulkOp.op.fieldLabel || "Value"}>
              <Select value={bulkOp.value} onChange={(v) => setBulkOp((b) => ({ ...b, value: v }))}>
                {bulkOp.op.options.map((o) => <Select.Item key={o.value} value={o.value} description={o.description}>{o.label}</Select.Item>)}
              </Select>
            </Field>
          )}
        </Modal>
