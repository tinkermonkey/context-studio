...
const CD_FORMS = {
  taxonomy: {
    ...
    idPrefix: "tax_",
    titleKey: "title",
    blurb: "A top-level domain that groups related concept schemes.",
  },
  scheme: {
    icon: "schema",
    noun: "concept scheme",
    ...
    idPrefix: "sch_",
    titleKey: "title",
    blurb: "A focused vocabulary inside a taxonomy. Classes live in exactly one scheme.",
  },
  class: {
    icon: "graph",
    noun: "class",
    ...
    idPrefix: "cls_",
    titleKey: "title",
    blurb: "A structural node of the graph — the type that individuals instantiate.",
  },
  property: {
    icon: "component",
    noun: "property",
    ...
    idPrefix: "",
    titleKey: "title",
    blurb: "A named predicate that relationships use to connect nodes.",
  },
  individual: {
    icon: "data",
    noun: "individual",
    ...
    idPrefix: "ind_",
    titleKey: "title",
    blurb: "A concrete instance that belongs to one or more classes.",
  },
  relationship: {
    icon: "link",
    noun: "relationship",
    ...
    idPrefix: "",
    titleKey: null,
    blurb: "A typed triple connecting a source node to a target node.",
  },
};
...
function cdFinalize(type, d) {
  ...
  if (type === "property")
    return { ...d, id: "prop_" + d.identifier, origin: d.origin || "manual" };
  if (type === "individual") return { ...d, confidence: parseFloat(d.confidence) || 1 };
  if (type === "relationship")
    return {
      ...d,
      id: "rel_" + Math.random().toString(36).slice(2, 6),
      created: "now",
      confidence: parseFloat(d.confidence) || 1,
    };
  return d;
}
...
function CreateDrawer({ type, initialDraft, onCreated, onClose, inline }) {
  ...
  const idErr = form.idKey
    ? CSStore.validateIdent(type === "property" ? "property" : type, draft[form.idKey] || "")
    : null;
  const reqs = useMemo(() => {
    ...
    if (form.idKey)
      list.push({
        key: form.idKey,
        label: type === "property" ? "Identifier" : "Identifier",
        ok: !idErr,
      });
    if (type === "scheme")
      list.push({ key: "taxonomy_id", label: "Taxonomy", ok: !!draft.taxonomy_id });
    if (type === "class")
      list.push({ key: "concept_scheme_id", label: "Scheme", ok: !!draft.concept_scheme_id });
    if (type === "individual")
      list.push({ key: "class_ids", label: "Class", ok: !!(draft.class_ids || []).length });
    if (type === "relationship") {
      list.push({ key: "source_id", label: "Source", ok: !!draft.source_id });
      list.push({
        key: "property_definition_id",
        label: "Predicate",
        ok: !!draft.property_definition_id,
      });
  ...
  const submit = useCallback(() => {
    ...
    if (createAnother) {
      setDraft({
        ...base,
        ...(type === "class" || type === "scheme" || type === "relationship"
          ? carryContext(type, base)
          : {}),
      });
      setTouched(new Set());
      setAttempted(false);
  ...
  const panel = (
    <div
      className={"cd-panel" + (inline ? " cd-panel--inline" : "")}
      role="dialog"
      aria-modal={inline ? undefined : "true"}
      aria-label={`Create ${form.noun}`}
    >
      {/* header */}
      <div className="cd-head">
        <div className={`cd-head__icon cd-head__icon--${type}`}>
        ...
        <button type="button" className="cd-head__close" onClick={onClose} aria-label="Close">
          <Icon name="x" size={15} />
        </button>
      </div>
      ...
      <div className="cd-progress" aria-hidden="true">
        <div className="cd-progress__track">
          <div
            className={"cd-progress__bar" + (canCreate ? " is-complete" : "")}
            style={{ width: pct + "%" }}
          />
        </div>
        <div className="cd-progress__chips">
          {reqs.map((r) => (
            <span key={r.key} className={"cd-req" + (r.ok ? " is-ok" : "")}>
              <span className="cd-req__dot">{r.ok ? <Icon name="check" size={9} /> : null}</span>
              {r.label}
            </span>
          ))}
          <span className="cd-progress__count">
            {canCreate ? "ready" : `${doneCount} / ${reqs.length}`}
          </span>
        </div>
      </div>
      ...
      <div className="cd-body">
        {justCreated && (
          <div className="cd-created" role="status">
            <Icon name="check" size={13} /> Created <strong>{justCreated}</strong> — add another
            below.
          </div>
        )}
        <CreateBody {...ctx} />
      ...
      <div className="cd-foot">
        ...
        <div className="cd-foot__actions">
          ...
          <button
            type="button"
            className={"cd-create" + (canCreate ? "" : " is-disabled")}
            onClick={submit}
...
function carryContext(type, base) {
  if (type === "class")
    return {
      concept_scheme_id: base.concept_scheme_id,
      taxonomy_id: base.taxonomy_id,
      domain: base.domain,
      parent_class_id: base.parent_class_id,
    };
...
function TitleField({ ctx, label = "Title", placeholder = "Give it a clear, human name", hint }) {
  ...
  return (
    <Field label={label} required hint={hint} error={err}>
      <TextInput
        ref={titleRef}
        value={draft[form.titleKey] || ""}
        placeholder={placeholder}
        error={!!err}
        onChange={(e) => onTitle(e.target.value)}
        onBlur={() => markTouched(form.titleKey)}
      />
    </Field>
  );
}
    ...
    >
      <div className="cd-id">
        <TextInput
          ...
          error={!!err}
          onChange={(e) => onIdEdit(e.target.value)}
          onBlur={() => markTouched(form.idKey)}
        />
        {idDirty && (
  ...
  return (
    <Field label="Description" hint={<span className="cd-id-hint">optional</span>}>
      <SuggestField
        ...
        type={ctx.type}
        title={draft[ctx.form.titleKey]}
        rows={rows}
        onSave={(v) => setField("description", v)}
        onChangeLive={(v) => setField("description", v)}
      />
    </Field>
  );
}
...
function SchemeBody(ctx) {
  ...
  const onTax = (v) => {
    ...
    setField("taxonomy_id", v);
    if (t) setField("domain", t.domain);
    markTouched("taxonomy_id");
  };
  ...
  return (
    <div className="cd-form">
      ...
      <Field
        ...
        required
        error={taxErr}
        hint={<span className="cd-id-hint">sets the domain</span>}
      >
      ...
      {tax && (
        <div className="cd-derived">
          <DomainPill domain={tax.domain} label={`domain: ${tax.domain}`} />
          <span className="cd-derived__note">inherited from {tax.title}</span>
        </div>
      )}
      <DescField ctx={ctx} />
    </div>
...
function ClassBody(ctx) {
  ...
  const parentOpts = [{ value: "", label: "— root (no parent) —" }].concat(
    data.classes
      .filter((x) => x.concept_scheme_id === draft.concept_scheme_id)
      .map((x) => ({ value: x.id, label: x.title, description: x.id })),
  );
  const onScheme = (v) => {
    const s = data.concept_schemes.find((x) => x.id === v);
    setDraftScheme(setField, v, s);
    markTouched("concept_scheme_id");
  };
  const schErr =
    showErr("concept_scheme_id") && !draft.concept_scheme_id ? "Pick a concept scheme." : null;
  const parent = draft.parent_class_id
    ? data.classes.find((x) => x.id === draft.parent_class_id)
    : null;
  return (
    <div className="cd-form">
      ...
      <Field label="Parent class" hint={<span className="cd-id-hint">optional</span>}>
        <Select
          value={draft.parent_class_id || ""}
          placeholder="— root —"
          onChange={(v) => setField("parent_class_id", v)}
          disabled={!draft.concept_scheme_id}
        >
          {parentOpts.map((o) => (
            <Select.Item key={o.value || "root"} value={o.value} description={o.description}>
              {o.label}
            </Select.Item>
          ))}
        </Select>
      ...
      {(sch || tax || parent) && (
        <div className="cd-derived">
          ...
          <span className="cd-derived__note">
            {[tax && `taxonomy ${tax.title}`, parent && `under ${parent.title}`]
              .filter(Boolean)
              .join(" · ") || "root class"}
          </span>
        </div>
...
function setDraftScheme(setField, v, s) {
  setField("concept_scheme_id", v);
  setField("taxonomy_id", s?.taxonomy_id);
  setField("domain", s?.domain);
  setField("parent_class_id", "");
}
...
function PropertyBody(ctx) {
  const { draft, setField } = ctx;
  const relVal =
    draft.is_relevant === true ? "true" : draft.is_relevant === false ? "false" : "null";
  ...
  return (
    <div className="cd-form">
      <TitleField
        ...
        hint={<span className="cd-id-hint">human label</span>}
      />
...
function IndividualBody(ctx) {
  const { data, draft, setField, markTouched, showErr } = ctx;
  const clsErr =
    showErr("class_ids") && !(draft.class_ids || []).length ? "Assign at least one class." : null;
  return (
    <div className="cd-form">
      ...
      <Field
        ...
        required
        error={clsErr}
        hint={<span className="cd-id-hint">one or more</span>}
      >
        <Select
          multiple
          values={draft.class_ids || []}
          placeholder="Assign classes…"
          onValuesChange={(vals) => {
            setField("class_ids", vals);
            markTouched("class_ids");
          }}
        >
          {data.classes.map((cl) => (
      ...
      <div className="cd-row2">
        ...
        <Field label="Confidence">
          <ParamSlider
            active
            label=""
            value={parseFloat(draft.confidence) || 0}
            min={0}
            max={1}
            step={0.01}
            fmt={(v) => v.toFixed(2)}
...
function RelationshipBody(ctx) {
  ...
  const onBuilder = (v) => {
    ...
    if (v.predicate) {
      const p = predByIdent(v.predicate);
      if (p) setField("property_definition_id", p.id);
    }
  };
  return (
    <div className="cd-form">
  ...
  return (
    <div className="cd-empty">
      ...
      <div className="cd-empty__sub">
        Create the first {noun} to start building out this part of the graph.
      </div>
      <button type="button" className="cd-empty__cta" onClick={onCreate}>
        <Icon name="plus" size={14} /> Create {noun}
      </button>
    </div>
  );
}
