...
function DatasetInspector({
  ...
  const saveField = (k) =>
    isCreate
      ? (v) => setField(k, v)
      : (v) => CSStore.update("dataset", entity.id, { [k]: v }, { field: k });
  ...
  const validId =
    isCreate &&
    CSStore.validateIdent("dataset", idVal) === null &&
    draft.title.trim() &&
    draft.filename.trim();
  const create = () => {
    const row = CSStore.create("dataset", {
      ...draft,
      id: idVal,
      created_at: "today",
      last_accessed: "now",
  ...
  return (
    <InlineInspector
      mode={mode}
      typeLabel="dataset"
      eyebrow={`DATASET · ${dsExtLabel(cur.filename)} · v${cur.schema_version || "—"}`}
      title={cur.title}
      id={idVal || "—"}
      ...
      footerHint={`PUT /api/v1/admin/datasets/${idVal || "…"}`}
    >
          ...
          rows={[
            kvRowD(
              ...
              isCreate ? (
                ...
                >
                  {idVal || "ds_…"}
                  <span style={{ marginLeft: 8, fontSize: 10.5 }}>auto from title</span>
                </span>
              ) : (
            ...
            kvRowD(
              ...
              <EditableField
                ...
                mono
                kind="text"
                onSave={saveField("filename")}
                onChangeLive={liveField("filename")}
                placeholder="source.jsonl"
              />,
            ),
            kvRowD(
              "SCHEMA",
              isCreate ? (
                <EditableField
                  active
                  kind="select"
                  value={cur.schema_version}
                  options={[
                    { value: "2.0", label: "2.0 (current)" },
                    { value: "1.2", label: "1.2" },
                    { value: "1.0", label: "1.0" },
                  ]}
                  onSave={saveField("schema_version")}
                />
              ) : (
                <Chip variant={(entity.schema_version || "").startsWith("2") ? "cyan" : "neutral"}>
                  v{entity.schema_version}
                </Chip>
              ),
            ),
            kvRowD(
              ...
              <SuggestField
                ...
                onChangeLive={liveField("description")}
                rows={3}
              />,
            ),
          ]}
      ...
      {!isCreate && (
        <>
          <InspectorPanel.Section
            title="Status"
            actions={isActive ? <Chip variant="emerald">active</Chip> : null}
          >
            {isActive ? (
              <div className="ds-callout">
                <Icon name="check" size={15} />
                <span>
                  This is the <strong>active</strong> dataset — every extraction, graph and query
                  operation in this workspace targets it.
                </span>
              </div>
            ) : (
              <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
                <span style={{ fontSize: 12.5, color: "rgb(var(--canvas-fg-2))" }}>
                  Inactive — loaded but not the current working set.
                </span>
                <Button
                  variant="primary"
                  size="sm"
          ...
          <InspectorPanel.Section
            title="Contents"
            actions={<span style={monoSm}>once loaded</span>}
          >
            ...
            <KVGrid
              ...
              rows={[
                kvRowD(
                  ...
                  >
                    <Icon name={fileIconFor(entity.filename)} size={13} />
                    {entity.filename}
                  </span>,
                ),
                ...
                kvRowD(
                  "VERSION",
                  <span style={{ fontFamily: "var(--font-mono)" }}>{entity.version}</span>,
                ),
              ]}
            />
          </InspectorPanel.Section>
...
// =========================================================================
const DatasetSurface = forwardRef(function DatasetSurface(
  { rows, columns, inspectorWidth = 460 },
  ref,
) {
  const data = useCS();
  ...
  const confirmDelete = () => {
    if (del.ids) {
      CSStore.bulkRemove("dataset", del.ids);
      setSelection(new Set());
    } else if (del.target) {
      CSStore.remove("dataset", del.target.id);
  ...
  const rowActions = (row) => [
    ...(row.is_active
      ? [{ id: "active", label: "Active (current)", icon: "check", disabled: true }]
  ...
  return (
    <>
      {isCreate || !isEmpty ? (
        ...
        >
          ...
          {(isCreate || (showRail && entity)) && (
            <DatasetInspector
              key={isCreate ? `create-${draftKey}` : `${entity.id}-${mode}`}
              entity={entity}
              mode={mode}
      ...
      <BulkBar
        ...
        onAction={(id) => {
          if (id === "delete") setDel({ open: true, target: null, ids: [...selection] });
        }}
      ...
      <Modal
        ...
        title={
          del.ids
            ? `Delete ${del.ids.length} dataset${del.ids.length === 1 ? "" : "s"}?`
            : `Delete ${del.target ? del.target.title : "dataset"}?`
        }
        ...
        footer={
          <>
            <Button
              variant="ghost"
              onClick={() => setDel({ open: false, target: null, ids: null })}
            >
              Cancel
            </Button>
      ...
      >
        {del.target && del.target.is_active && (
          <div className="ds-callout ds-callout--warn">
            ...
            <span>
              This is the active dataset. Deleting it leaves the workspace with no active working
              set until you activate another.
            </span>
          </div>
        )}
      </Modal>
...
function CSDatasets({ onNav }) {
  ...
  const rows = data.datasets.filter((d) => {
    ...
    if (!q) return true;
    return (d.title + d.id + d.filename + (d.description || ""))
      .toLowerCase()
  ...
  const statusCell = (_, d) => (
    <span
      className={"ds-dot" + (d.is_active ? " ds-dot--on" : "")}
    ...
    <div data-screen-label="08 Data · Datasets">
      ...
      {activeDs && (
        <div className="ds-active-banner">
          ...
          <span className="ds-active-banner__file">
            <Icon name={fileIconFor(activeDs.filename)} size={12} /> {activeDs.filename}
          </span>
          <span className="ds-active-banner__meta">
            {activeDs.metrics.individuals_count} individuals · {activeDs.metrics.terms_count} terms
            · {activeDs.metrics.relationships_count} relationships
          </span>
        </div>
      )}
      ...
      <FilterBar
        q={q}
        setQ={setQ}
        placeholder="Search datasets, filenames, descriptions…"
        ...
        columns={[
          ...
          },
          {
            ...
            render: (v) => (
              ...
              >
                <Icon name={fileIconFor(v)} size={13} />
                {v}
              </span>
            ),
          },
          ...
          },
          {
            ...
            width: "84px",
            render: (v) => (
              <Chip variant={(v || "").startsWith("2") ? "cyan" : "neutral"}>v{v}</Chip>
            ),
          },
          {
            key: "is_active",
            ...
            width: "104px",
            render: (v) =>
              v ? <Chip variant="emerald">active</Chip> : <Chip variant="neutral">inactive</Chip>,
          },
          {
            key: "version",
            label: "Ver",
