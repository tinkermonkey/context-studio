...
function newDraft(type, ctx = {}) {
  ...
  if (type === "class")
    return {
      id: "",
      title: "",
      concept_scheme_id: ctx.concept_scheme_id || "",
      taxonomy_id: ctx.taxonomy_id || "",
      parent_class_id: ctx.parent_class_id || "",
      domain: ctx.domain || "life",
      description: "",
      individuals: 0,
    };
  if (type === "property")
...
function TaxonomyInspector({
  ...
  const validId =
    c.isCreate && CSStore.validateIdent(identType, c.draft.id) === null && c.draft.title.trim();

  const schemes = entity ? data.concept_schemes.filter((s) => s.taxonomy_id === entity.id) : [];
  const create = () => {
    const r = CSStore.create("taxonomy", { ...c.draft, classes: 0, individuals: 0 });
    onCreated(r.id);
  };
  ...
  return (
    <InlineInspector
      ...
      footerHint={`PUT /taxonomies/${idVal || "…"}`}
    >
          ...
          rows={[
            kvRow(
              ...
              c.isCreate ? (
                <EditableField
                  ...
                  value={c.draft.id}
                  kind="ident"
                  placeholder="tax_my_domain"
                  onChangeLive={(v) => c.setField("id", v)}
                  onSave={(v) => c.setField("id", v)}
                  validate={(v) => CSStore.validateIdent(identType, v)}
                  autoFocus
                />
              ) : (
            ...
            kvRow(
              ...
              <EditableField
                active={c.active}
                value={c.cur.title}
                onSave={c.saveField("title")}
                onChangeLive={c.liveField("title")}
            ...
            kvRow(
              ...
              c.active ? (
                <EditableField
                  ...
                  kind="select"
                  value={c.cur.domain}
                  options={domSelectOptions}
                  onSave={c.saveField("domain")}
                />
              ) : (
            ...
            kvRow(
              ...
              <SuggestField
                active={c.active}
                value={c.cur.description}
                type="taxonomy"
                entityId={entity?.id}
                title={c.cur.title}
                onSave={c.saveField("description")}
                onChangeLive={c.liveField("description")}
              />,
            ),
          ]}
        />
      ...
      {!c.isCreate && (
        <>
          <InspectorPanel.Section title="Concept schemes" count={schemes.length}>
            {schemes.length === 0 ? (
              <em style={{ fontSize: 12, color: "rgb(var(--canvas-fg-3))" }}>— none —</em>
            ) : (
          ...
          <InspectorPanel.Section title="Stats">
            <KVGrid
              ...
              rows={[
                kvRow(
                  "CLASSES",
                  <span style={{ fontFamily: "var(--font-mono)" }}>{entity.classes}</span>,
                ),
                kvRow(
                  "INDIVIDUALS",
                  <span style={{ fontFamily: "var(--font-mono)" }}>{entity.individuals}</span>,
                ),
                kvRow(
                  "UPDATED",
                  <span
...
function SchemeInspector({
  ...
  const validId =
    c.isCreate &&
    CSStore.validateIdent("scheme", c.draft.id) === null &&
    c.draft.title.trim() &&
    c.draft.taxonomy_id;
  ...
  const onTax = (v) => {
    ...
    if (c.isCreate) {
      c.setField("taxonomy_id", v);
      if (t) c.setField("domain", t.domain);
    } else {
      CSStore.update(
        "scheme",
        entity.id,
        { taxonomy_id: v, domain: t ? t.domain : entity.domain },
        { field: "taxonomy" },
      );
    }
  };
  ...
  const create = () => {
    const r = CSStore.create("scheme", { ...c.draft, classes: 0 });
    onCreated(r.id);
  };
  ...
  return (
    <InlineInspector
      mode={mode}
      typeLabel="scheme"
      eyebrow={`CONCEPT SCHEME · ${c.cur.domain || ""}`}
      title={c.cur.title}
      id={idVal || "—"}
      version={entity?.version}
      onEdit={onEnterEdit}
      ...
      onDelete={onDelete}
      onDuplicate={onDuplicate}
      footerHint={`PUT /concept_schemes/${idVal || "…"}`}
    >
      <InspectorPanel.Section title="Identity">
        <KVGrid
          keyWidth={120}
          rows={[
            kvRow(
              ...
              c.isCreate ? (
                <EditableField
                  ...
                  value={c.draft.id}
                  kind="ident"
                  placeholder="sch_my_scheme"
                  onChangeLive={(v) => c.setField("id", v)}
                  onSave={(v) => c.setField("id", v)}
                  validate={(v) => CSStore.validateIdent("scheme", v)}
                  autoFocus
                />
              ) : (
                <span style={{ fontFamily: "var(--font-mono)", fontSize: 12 }}>{entity.id}</span>
            ...
            kvRow(
              "DOMAIN",
              <DomainPill domain={c.cur.domain} label={(c.cur.domain || "—") + " · inherited"} />,
            ),
            kvRow(
              "DESCRIPTION",
              <SuggestField
...
function ClassInspector({
  ...
  const parentOpts = [{ value: "", label: "— root (no parent) —" }].concat(
    data.classes
      .filter(
        (x) =>
          x.concept_scheme_id === c.cur.concept_scheme_id &&
          x.id !== entity?.id &&
  ...
  const parent = c.cur.parent_class_id
    ? data.classes.find((x) => x.id === c.cur.parent_class_id)
    : null;
  ...
  const indSample = entity
    ? data.individuals.filter((i) => i.class_ids.includes(entity.id)).slice(0, 5)
    : [];
  const rels = entity
    ? data.relationships.filter((r) => r.source_id === entity.id || r.target_id === entity.id)
    : [];
  const validId =
    c.isCreate &&
    CSStore.validateIdent("class", c.draft.id) === null &&
    c.draft.title.trim() &&
    c.draft.concept_scheme_id;
  ...
  const create = () => {
    const r = CSStore.create("class", { ...c.draft, individuals: 0 });
    onCreated(r.id);
  };
    ...
    >
            ...
            kvRow(
              ...
              c.isCreate ? (
                <EditableField
                  ...
                  value={c.draft.id}
                  kind="ident"
                  placeholder="cls_my_class"
                  onChangeLive={(v) => c.setField("id", v)}
                  onSave={(v) => c.setField("id", v)}
                  validate={(v) => CSStore.validateIdent("class", v)}
                  autoFocus
                />
              ) : (
                <span style={{ fontFamily: "var(--font-mono)", fontSize: 12 }}>{entity.id}</span>
            ...
            kvRow(
              ...
              c.active ? (
                <EditableField
                  active
                  kind="select"
                  value={c.cur.parent_class_id || ""}
                  options={parentOpts}
                  onSave={c.saveField("parent_class_id")}
                  placeholder="— root —"
                />
              ) : parent ? (
                <DomainPill domain={parent.domain} label={parent.title} />
              ) : (
        ...
        {c.active && parentMismatch && (
          <div style={{ marginTop: 10 }}>
            <FormCallout variant="warn" icon="alert">
              Parent `{parent.id}` is in a different scheme. A parent class must share this class's
              concept scheme.
            </FormCallout>
          </div>
        )}
      ...
      {!c.isCreate && (
        <>
          <InspectorPanel.Section title="Children" count={children.length}>
            {children.length === 0 ? (
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {children.map((cl) => (
                  <DomainPill key={cl.id} domain={cl.domain} label={cl.title} />
                ))}
              </div>
            )}
          </InspectorPanel.Section>
          ...
          >
            {indSample.length === 0 ? (
              <em style={{ fontSize: 12, color: "rgb(var(--canvas-fg-3))" }}>— none yet —</em>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                {indSample.map((i) => (
                  <div
...
function PropertyInspector({
  ...
  const usedIn = entity
    ? data.relationships.filter((r) => r.property_definition_id === entity.id)
    : [];
  const validId =
    c.isCreate &&
    CSStore.validateIdent("property", c.draft.identifier) === null &&
    c.draft.title.trim();

  const relevanceVal =
    c.cur.is_relevant === true ? "true" : c.cur.is_relevant === false ? "false" : "null";
  const setRelevance = (v) => {
    const val = v === "true" ? true : v === "false" ? false : null;
    if (c.isCreate) c.setField("is_relevant", val);
    else CSStore.update("property", entity.id, { is_relevant: val }, { field: "relevance" });
  };
  const relChip = (r) =>
    r === true ? (
  ...
  const create = () => {
    const r = CSStore.create("property", { ...c.draft, id: "prop_" + c.draft.identifier });
    onCreated(r.id);
  };
  ...
  return (
    <InlineInspector
      ...
      onDelete={onDelete}
      onDuplicate={onDuplicate}
      footerHint={`PUT /property_definitions/${entity?.id || "…"}`}
    >
      <InspectorPanel.Section title="Identity">
        <KVGrid
          keyWidth={120}
          rows={[
            kvRow(
              ...
              c.isCreate ? (
                <EditableField
                  active
                  value={c.draft.identifier}
                  kind="ident"
                  placeholder="has_part"
                  onChangeLive={(v) => c.setField("identifier", v)}
                  onSave={(v) => c.setField("identifier", v)}
                  validate={(v) => CSStore.validateIdent("property", v)}
                  autoFocus
                />
              ) : (
      ...
      {!c.isCreate && (
        <InspectorPanel.Section title="Used by relationships" count={usedIn.length}>
          {usedIn.length === 0 ? (
            <em style={{ fontSize: 12, color: "rgb(var(--canvas-fg-3))" }}>— not yet used —</em>
          ) : (
            <div
              style={{
                display: "flex",
            ...
            >
              {usedIn.slice(0, 8).map((r) => (
                <div key={r.id} style={{ color: "rgb(var(--canvas-fg-2))" }}>
                  {r.id}
                </div>
              ))}
            </div>
...
function IndividualInspector({
  ...
  const idVal = c.isCreate ? c.draft.id : entity.id;
  const classChips = (c.cur.class_ids || [])
    .map((cid) => data.classes.find((x) => x.id === cid))
    .filter(Boolean);
  const rels = entity
    ? data.relationships.filter((r) => r.source_id === entity.id || r.target_id === entity.id)
    : [];
  const validId =
    c.isCreate &&
    CSStore.validateIdent("individual", c.draft.id) === null &&
    c.draft.title.trim() &&
    (c.draft.class_ids || []).length;

  const create = () => {
    const r = CSStore.create("individual", {
      ...c.draft,
      confidence: parseFloat(c.draft.confidence) || 1,
    });
    onCreated(r.id);
  };
  ...
  return (
    <InlineInspector
      ...
      onDelete={onDelete}
      onDuplicate={onDuplicate}
      footerHint={`PUT /individuals/${idVal || "…"}`}
    >
      <InspectorPanel.Section title="Identity">
        <KVGrid
          keyWidth={110}
          rows={[
            kvRow(
              ...
              c.isCreate ? (
                <EditableField
                  ...
                  value={c.draft.id}
                  kind="ident"
                  placeholder="ind_brca1"
                  onChangeLive={(v) => c.setField("id", v)}
                  onSave={(v) => c.setField("id", v)}
                  validate={(v) => CSStore.validateIdent("individual", v)}
                  autoFocus
                />
              ) : (
                <span style={{ fontFamily: "var(--font-mono)", fontSize: 12 }}>{entity.id}</span>
            ...
            kvRow(
              ...
              c.active ? (
                <Select
                  multiple
                  values={c.cur.class_ids || []}
                  placeholder="Assign classes…"
                  onValuesChange={(vals) => {
                    if (c.isCreate) c.setField("class_ids", vals);
                    else
                      CSStore.update(
                        "individual",
                        entity.id,
                ...
                >
                  {data.classes.map((cl) => (
                    <Select.CheckboxItem key={cl.id} value={cl.id} description={cl.id}>
                      {cl.title}
                    </Select.CheckboxItem>
                  ))}
                </Select>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
                  {classChips.map((cl) => (
                    <Chip key={cl.id} variant="neutral">
                      {cl.title}
                    </Chip>
                  ))}
                </div>
            ...
            kvRow(
              ...
              <EditableField
                active={c.active}
                value={c.cur.source}
                onSave={c.saveField("source")}
                onChangeLive={c.liveField("source")}
                mono
                placeholder="NCBI"
              />,
            ),
            kvRow(
              ...
              c.active ? (
                <ParamSlider
                  active
                  label=""
                  value={parseFloat(c.cur.confidence) || 0}
                  min={0}
                  max={1}
                  step={0.01}
                  fmt={(v) => v.toFixed(2)}
                  onChange={(v) => {
                    if (c.isCreate) c.setField("confidence", v);
                    else
                      CSStore.update(
                        "individual",
                        entity.id,
                ...
                <span style={{ fontFamily: "var(--font-mono)" }}>
                  {(c.cur.confidence ?? 0).toFixed ? c.cur.confidence.toFixed(2) : c.cur.confidence}
                </span>
              ),
            ),
          ]}
          ...
          {rels.length === 0 ? (
            ...
            >
              {rels.map((r) => {
                ...
                const isSrc = r.source_id === entity.id;
                const o =
                  data.classes.find((x) => x.id === (isSrc ? r.target_id : r.source_id)) ||
                  data.individuals.find((x) => x.id === (isSrc ? r.target_id : r.source_id));
                return (
                  <div key={r.id}>
                    {isSrc ? entity.title : o?.title || "?"}
                    <span style={{ color: "rgb(var(--canvas-fg-3))" }}>
...
function RelationshipInspector({
  ...
  const nodeById = (id) =>
    data.classes.find((x) => x.id === id) || data.individuals.find((x) => x.id === id);
  const toResult = (n) =>
    n
      ? { id: n.id, label: n.title, domain: n.domain, domainColor: domainColorOf(n.domain) }
      : undefined;
  ...
  const seed =
    ...
    (entity
      ? {
          ...
          property_definition_id: entity.property_definition_id,
          confidence: entity.confidence,
          source: entity.source,
        }
      : newDraft("relationship"));
  ...
  const search = (q) => {
    ...
    return [...data.classes, ...data.individuals]
  ...
  const onBuilder = (v) => {
    const patch = {
      ...
      property_definition_id: v.predicate
        ? predByIdent(v.predicate)?.id || cur.property_definition_id
        : cur.property_definition_id,
    };
    if (isCreate) setDraft((d) => ({ ...d, ...patch }));
    else CSStore.update("relationship", entity.id, patch, { field: "triple" });
  };

  const src = nodeById(cur.source_id);
  const tgt = nodeById(cur.target_id);
  const prop = predById(cur.property_definition_id);
  ...
  const create = () => {
    ...
    const r = CSStore.create("relationship", {
      ...draft,
      id,
      created: "now",
      confidence: parseFloat(draft.confidence) || 1,
    });
    onCreated(r.id);
  };
  ...
  return (
    <InlineInspector
      ...
      eyebrow="RELATIONSHIP · triple"
      title={src && tgt ? `${src.title} → ${tgt.title}` : "New relationship"}
      id={isCreate ? "—" : entity.id}
      onEdit={onEnterEdit}
      onDone={onDone}
      onCreate={create}
      onCancel={onCancel}
      ...
      onDelete={onDelete}
      onDuplicate={onDuplicate}
      footerHint={`PUT /relationships/${entity?.id || "…"}`}
    >
      <InspectorPanel.Section title="Triple">
        {active ? (
          <RelationshipBuilder
            ...
            onChange={onBuilder}
            sourceQuery={srcQ}
            onSourceQueryChange={setSrcQ}
            sourceResults={search(srcQ)}
            targetQuery={tgtQ}
            onTargetQueryChange={setTgtQ}
            targetResults={search(tgtQ)}
            ...
            kvRow(
              ...
              active ? (
                <ParamSlider
                  active
                  label=""
                  value={parseFloat(cur.confidence) || 0}
                  min={0}
                  max={1}
                  step={0.01}
                  fmt={(v) => v.toFixed(2)}
                  onChange={(v) => {
                    if (isCreate) setDraft((d) => ({ ...d, confidence: v }));
                    else
                      CSStore.update(
                        "relationship",
                        entity.id,
                ...
                <span style={{ fontFamily: "var(--font-mono)" }}>
                  {(cur.confidence ?? 0).toFixed ? cur.confidence.toFixed(2) : cur.confidence}
                </span>
              ),
            ),
            kvRow(
              ...
              <EditableField
                ...
                placeholder="manual"
                onSave={(v) => {
                  if (isCreate) setDraft((d) => ({ ...d, source: v }));
                  else
                    CSStore.update("relationship", entity.id, { source: v }, { field: "source" });
                }}
