...
function ConfBar({ value }) {
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 5 }}>
      ...
      <span
        style={{ fontFamily: "var(--font-mono)", fontSize: 10.5, color: "rgb(var(--canvas-fg-3))" }}
      >
        {value.toFixed(2)}
      </span>
    </span>
...
function RelationshipSuggester({ node }) {
  ...
  const run = () => {
    setBusy(true);
    setProps(null);
    CSStore.suggestRelationships(node).then((r) => {
      setBusy(false);
      setProps(r);
    });
  };
  ...
  return (
    <div style={{ marginTop: 10 }}>
      ...
      {(busy || (props && props.length > 0)) && (
        <div className="ce-suggest-panel" style={{ marginTop: 8 }}>
          <div className="ce-suggest-panel__label">
            <Icon name="zap" size={11} /> Suggested relationships
          </div>
          {busy ? (
            <>
              <div className="ce-shimmer" style={{ width: "100%" }} />
              <div className="ce-shimmer" style={{ width: "80%" }} />
            </>
          ) : (
              ...
              {props.map((p, i) => (
                <div key={i} className="ce-sg-card">
                  <div className="ce-sg-triple">
                    ...
                    <span className="ce-sg-node">
                      <span
                        className="cs-domain-dot"
                        style={{ background: `var(--dom-${p.target_domain || "default"})` }}
                      />{" "}
                      {p.target_label}
                    </span>
                  </div>
                  ...
                  <div className="ce-sg-foot">
                    ...
                    <div style={{ display: "flex", gap: 6, marginLeft: "auto" }}>
                      <Button variant="primary" size="sm" onClick={() => accept(p)}>
                        Accept
                      </Button>
                      <Button variant="ghost" size="sm" onClick={() => dismiss(p)}>
                        Dismiss
                      </Button>
                    </div>
                  </div>
      ...
      {props && props.length === 0 && !busy && (
        ...
        >
          No new relationships to suggest — existing ones are already captured.
        </div>
      )}
    </div>
  );
...
function GroundingSection({ type, node, active }) {
  ...
  const run = () => {
    setBusy(true);
    setProps(null);
    CSStore.suggestGrounding(node).then((r) => {
      setBusy(false);
      setProps(r);
    });
  };
  const accept = (p) => {
    CSStore.addGrounding(type, node, p);
    setProps((ps) => ps.filter((x) => x !== p));
  };
  const dismiss = (p) => setProps((ps) => ps.filter((x) => x !== p));
          ...
          {refs.map((g) => (
            <div key={g.id} className="ce-ground-row">
              <Icon
                name="link"
                size={12}
                style={{ color: "rgb(var(--canvas-fg-3))", flex: "none" }}
              />
              ...
              {active && (
                <button
                  type="button"
                  className="ce-ground-x"
                  onClick={() => CSStore.removeGrounding(type, node, g.id)}
                  aria-label="Remove"
                >
                  <Icon name="x" size={11} />
                </button>
      ...
      {active && (
        <div style={{ marginTop: 10 }}>
          ...
          {(busy || (props && props.length > 0)) && (
            <div className="ce-suggest-panel" style={{ marginTop: 8 }}>
              <div className="ce-suggest-panel__label">
                <Icon name="zap" size={11} /> Suggested reference sources
              </div>
              {busy ? (
                <>
                  <div className="ce-shimmer" style={{ width: "100%" }} />
                  <div className="ce-shimmer" style={{ width: "72%" }} />
                </>
              ) : (
                <div className="ce-sg-list">
                  {props.map((p, i) => (
          ...
          {props && props.length === 0 && !busy && (
            ...
            >
              No new sources to suggest for this domain.
            </div>
          )}
        </div>
      )}
