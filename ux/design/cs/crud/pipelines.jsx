...
function ConfigEditor({
  ...
  const setParam = (k, v) => {
    if (isCreate) setDraft((d) => ({ ...d, parameters: { ...d.parameters, [k]: v } }));
      ...
      CSStore.update(
        "configuration",
        config.id,
        { parameters: { ...config.parameters, [k]: v } },
        { field: "params" },
      );
  };
  const save = (k) => (v) => {
    if (isCreate) setField(k, v);
    else CSStore.update("configuration", config.id, { [k]: v }, { field: k });
  };
  ...
  const onProvider = (v) => {
    ...
    if (isCreate) setDraft((d) => ({ ...d, provider: v, model: m }));
      ...
      CSStore.update("configuration", config.id, { provider: v, model: m }, { field: "provider" });
  };
  ...
  const headerActions = isCreate ? null : isEdit ? (
    ...
    <div className="ce-inspector-actions">
      <Button variant="ghost" size="sm" icon onClick={onEnterEdit} title="Edit">
      ...
      <RowMenu
        ...
        onAction={(a) => {
          if (a === "edit") onEnterEdit();
          if (a === "duplicate") onDuplicate();
      ...
      <div className="cfg-editor">
        {isEdit && (
          <FormCallout variant="info" icon="info">
            Editing — changes save automatically on blur. Saving bumps the configuration version.
          </FormCallout>
        )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div className="cfg-editor__grid2">
              <Field label="Provider">
                {active ? (
                  <Select value={cur.provider} onChange={onProvider}>
                    {PROVIDERS.map((p) => (
              ...
              <Field label="Model">
                {active ? (
                  <Select value={cur.model} onChange={save("model")}>
                    {modelOpts.map((m) => (
            ...
            <Field label="User prompt template" hint="Jinja2 · {{variables}}">
              {active ? (
                <TextArea
                  className="cfg-prompt"
                  mono
                  value={cur.user_prompt_template || ""}
                  rows={4}
                  onChange={(e) => save("user_prompt_template")(e.target.value)}
                  onBlur={(e) => save("user_prompt_template")(e.target.value)}
                />
              ) : (
                <pre
                  style={{
                ...
                >
                  {cur.user_prompt_template || "—"}
                </pre>
              )}
              <VariableChips
                template={cur.user_prompt_template}
                onInsert={active ? insertVar : undefined}
              />
            </Field>
          </div>
        </div>
        ...
        <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 18 }}>
            ...
            <ParamSlider
              active={active}
              label="Max tokens"
              value={params.max_tokens ?? 800}
              min={64}
              max={4096}
              step={32}
              fmt={(v) => Math.round(v)}
              onChange={(v) => setParam("max_tokens", Math.round(v))}
            />
            <ParamSlider
              active={active}
              label="Top-p"
        ...
        {!isCreate && (
          <div>
            ...
            <div className="cfg-stat-row">
              <div>
                <div className="cfg-stat__k">executions</div>
                <div className="cfg-stat__v">{(cur.execution_count || 0).toLocaleString()}</div>
              </div>
              <div>
                <div className="cfg-stat__k">avg duration</div>
                <div className="cfg-stat__v">
                  {cur.average_duration_ms
                    ? (cur.average_duration_ms / 1000).toFixed(2) + "s"
                    : "—"}
                </div>
              </div>
              <div>
                <div className="cfg-stat__k">avg cost</div>
                <div className="cfg-stat__v">
                  {cur.average_cost ? "$" + cur.average_cost.toFixed(4) : "—"}
                </div>
              </div>
              <div>
                <div className="cfg-stat__k">updated</div>
                <div className="cfg-stat__v" style={{ fontSize: 12 }}>
                  {cur.updated}
                </div>
              </div>
            </div>
        ...
        {isCreate && (
          <div style={{ display: "flex", gap: 8 }}>
            <Button variant="primary" onClick={create} disabled={!validNew}>
              <Icon name="check" size={13} /> Create configuration
            </Button>
        ...
        {!isCreate && (
          ...
          >
            PUT /api/pipeline_configurations/{config.id}
          </div>
        )}
      </div>
    </Panel>
...
function CSPipelineTypes({ route, onNav, surfaceRef }) {
  ...
  const confirmDelete = () => {
    CSStore.remove("configuration", del.id);
    if (cfgId === del.id) setCfgId(null);
    setDel(null);
  };
  ...
  return (
    <div data-screen-label="08 Pipelines · Pipeline types">
      <PageHeader
        ...
        actions={
          <>
            <Button variant="ghost" onClick={() => onNav("pipelines/all")}>
              <Icon name="pipeline" size={13} /> All pipelines
            </Button>
            <Button variant="primary" onClick={newCfg}>
        ...
        <div className="pt-list">
          {types.map((t) => {
            ...
            return (
              <button
                key={t.id}
                className={["pt-card", t.id === ptype.id ? "pt-card--active" : ""].join(" ")}
                onClick={() => selectType(t.id)}
              >
                <span className="pt-card__icon">
                  <Icon name={PT_ICON(t.icon)} size={17} />
                </span>
                <span className="pt-card__body">
                  <span className="pt-card__name">
                    {t.name}
                    <Icon name="lock" size={11} style={{ color: "rgb(var(--canvas-fg-4))" }} />
                  </span>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <Panel>
            <div style={{ display: "flex", alignItems: "flex-start", gap: 14 }}>
              <span className="pt-card__icon" style={{ width: 40, height: 40 }}>
                <Icon name={PT_ICON(ptype.icon)} size={20} />
              </span>
            ...
            >
              <div>
                <div className="cfg-stat__k">module</div>
                <div
                  style={{
                    fontFamily: "var(--font-mono)",
                    fontSize: 12,
              ...
              <div>
                <div className="cfg-stat__k">input</div>
                <div style={{ fontSize: 12.5, color: "rgb(var(--canvas-fg-2))", marginTop: 3 }}>
                  {ptype.inputs}
                </div>
              </div>
              <div>
                <div className="cfg-stat__k">output</div>
                <div style={{ fontSize: 12.5, color: "rgb(var(--canvas-fg-2))", marginTop: 3 }}>
                  {ptype.outputs}
                </div>
              </div>
            ...
            >
              <span className="cfg-stat__k" style={{ marginRight: 4 }}>
                stages
              </span>
              {ptype.stages.map((s, i) => (
                <React.Fragment key={s}>
                  <Chip variant="neutral">{s}</Chip>
                  {i < ptype.stages.length - 1 && (
                    <Icon
                      name="arrowRight"
                      size={11}
                      style={{ color: "rgb(var(--canvas-fg-4))" }}
                    />
                  )}
                </React.Fragment>
              ))}
          ...
          >
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <div className="cs-between" style={{ marginBottom: 2 }}>
                ...
                <Button variant="ghost" size="sm" icon onClick={newCfg} title="New configuration">
                  <Icon name="plus" size={13} />
                </Button>
              </div>
              {configs.length === 0 && (
                <div style={{ fontSize: 12, color: "rgb(var(--canvas-fg-3))", padding: "8px 2px" }}>
              ...
              {configs.map((cfg) => (
                <button
                  ...
                  className={[
                    "cfg-row",
                    cfg.id === cfgId && mode !== "create" ? "cfg-row--active" : "",
                  ].join(" ")}
                  onClick={() => selectCfg(cfg.id)}
                  style={{ textAlign: "left", cursor: "pointer" }}
                >
                  <span
                    className="cfg-row__dot"
                    style={{
      ...
      {del && (
        <ConfirmDialog
          ...
          message={
            <span style={{ fontSize: 12.5, color: "rgb(var(--canvas-fg-2))" }}>
              Configuration <code>{del.id}</code> under <strong>{ptype.name}</strong> will be
              deleted.
            </span>
          }
...
function ConfigDuplicator({ dup, onClose, onCreated }) {
  useEffect(() => {
    ...
    const r = CSStore.create("configuration", {
      ...dup,
      id: id || "cfg_" + Math.random().toString(36).slice(2, 6),
      name: dup.name + " (copy)",
      version: 1,
      execution_count: 0,
      average_duration_ms: 0,
