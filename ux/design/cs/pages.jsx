...
function CSDashboard({ onNav }) {
  ...
  return (
    <div data-screen-label="01 Dashboard">
      ...
      <StatGrid columns={4}>
        <StatTile label="TAXONOMIES" value={data.taxonomies.length} color="cyan" icon="schema"
          meta="3 active · 0 archived" sparkData={data.sparks.taxonomies} />
        <StatTile label="CLASSES" value={totalClasses} color="violet" icon="schema"
          delta={{ value: 4, label: 'this week', direction: 'up' }} sparkData={data.sparks.classes} />
        <StatTile label="INDIVIDUALS" value={totalIndividuals.toLocaleString()} color="emerald" icon="data"
          delta={{ value: 38, label: 'last run · 412 ingested', direction: 'up' }} sparkData={data.sparks.individuals} />
        <StatTile label="PIPELINES" value={`${running}/${data.pipelines.length}`} color="amber" icon="pipeline"
          meta={`${running} running · ${failed} failed`} sparkData={data.sparks.ingestRate} />
      </StatGrid>
          ...
          headerAction={
            <div style={{display:'flex', alignItems:'center', gap:12}}>
              <span style={{fontFamily:'var(--font-mono)', fontSize:11, color:'rgb(var(--canvas-fg-3))', whiteSpace:'nowrap'}}>
                {data.taxonomies.length} tax · {data.concept_schemes.length} sch · {totalClasses} cls
              </span>
      ...
      <QuickAccessGrid tiles={[
        ...
        tax: 'schema/taxonomies', cls: 'schema/classes', prop: 'schema/properties',
        ind: 'data/individuals', pipe: 'pipelines/all', ref: 'reference/sources',
      }[id])} />
    </div>
  );
}
...
function CSSchemaClasses({ route, onNav, onOpenNewClass }) {
  ...
  const filtered = useMemo(() => data.classes.filter((c) => {
    if (domain !== 'all' && c.domain !== domain) return false;
    if (!q) return true;
    const Q = q.toLowerCase();
    return c.title.toLowerCase().includes(Q) || c.id.toLowerCase().includes(Q) || c.description.toLowerCase().includes(Q);
  }), [q, domain]);
    ...
    <div data-screen-label="03 Schema · Classes">
      <PageHeader
        ...
        actions={
          <>
            ...
            <Button variant="primary" onClick={onOpenNewClass}><Icon name="plus" size={13}/> New class</Button>
          </>
        }
      />
      ...
      <div style={{display:'flex', alignItems:'center', gap: 10, marginBottom: 14}}>
        <div style={{flex: '0 1 340px', position: 'relative'}}>
          <div style={{position:'absolute', left: 10, top: '50%', transform:'translateY(-50%)', color:'rgb(var(--canvas-fg-3))', pointerEvents:'none'}}>
        ...
        <SegmentedControl
          ...
          onChange={setDomain}
        />
        <span style={{marginLeft:'auto', fontFamily:'var(--font-mono)', fontSize:11, color:'rgb(var(--canvas-fg-3))'}}>
      ...
      <div style={{display:'grid', gridTemplateColumns: '1fr 380px', gap: 14, alignItems: 'start'}}>
        <Panel noPadding>
          <div style={{maxHeight: 600, overflow: 'auto'}}>
            <Table
              columns={[
                { key: 'swatch', label: '', width: '36px', render: (_, c) => (
                  <span className="cs-domain-dot" style={{background:`var(--dom-${c.domain})`}}/>
                )},
                { key: 'id', label: 'Identifier', width: '160px', render: (v) => (
                ...
                { key: 'concept_scheme_id', label: 'Scheme', width: '140px', render: (v) => {
                  const sch = data.concept_schemes.find((s) => s.id === v);
                  return <Chip variant="neutral">{sch?.title}</Chip>;
                }},
                { key: 'individuals', label: 'Ind', width: '70px', render: (v) => (
                  <span style={{fontFamily:'var(--font-mono)', textAlign:'right', display:'block'}}>{v}</span>
                )},
                { key: 'version', label: 'Ver', width: '60px', render: (v) => <VersionPill>{v}</VersionPill> },
              ]}
              data={filtered}
              rowKey="id"
              selectedRows={[selectedId]}
              onRowClick={(row) => setSelectedId(row.id)}
              emptyState={<div style={{padding: 40, textAlign:'center', color:'rgb(var(--canvas-fg-3))'}}>No classes match these filters.</div>}
            />
          </div>
        </Panel>
...
function CSClassInspector({ cls }) {
  ...
  const parent = cls.parent_class_id ? data.classes.find((c) => c.id === cls.parent_class_id) : null;
  ...
  return (
    <InspectorPanel
      ...
      actions={
        <>
          <Button variant="ghost" size="sm" icon><Icon name="edit" size={13}/></Button>
          <Button variant="ghost" size="sm" icon><Icon name="moreVertical" size={13}/></Button>
        </>
      }
    >
      <InspectorPanel.Section title="Identity">
        <KVGrid
          keyWidth={130}
          rows={[
            { key: 'DESCRIPTION', value: <span style={{color:'rgb(var(--canvas-fg-2))'}}>{cls.description}</span> },
            { key: 'DOMAIN', value: domainPill(cls.domain, cls.domain) },
            { key: 'SCHEME', value: <span>{domainPill(sch?.domain, sch?.title)} <span style={{fontFamily:'var(--font-mono)', fontSize:11, color:'rgb(var(--canvas-fg-3))', marginLeft:6}}>{sch?.id}</span></span> },
            { key: 'TAXONOMY', value: <span>{domainPill(tax?.domain, tax?.title)} <span style={{fontFamily:'var(--font-mono)', fontSize:11, color:'rgb(var(--canvas-fg-3))', marginLeft:6}}>{tax?.id}</span></span> },
            { key: 'PARENT CLASS', value: parent ? domainPill(parent.domain, parent.title) : <em style={{color:'rgb(var(--canvas-fg-3))'}}>— root —</em> },
            { key: 'CHILDREN', value: children.length === 0
                ? <em style={{color:'rgb(var(--canvas-fg-3))'}}>— none —</em>
                : <div style={{display:'flex', flexWrap:'wrap', gap: 6}}>{children.map((c) => (
                    <span key={c.id} style={{display:'inline-flex', alignItems:'center', gap:6, fontFamily:'var(--font-mono)', fontSize:11.5}}>
                      <span className="cs-domain-dot" style={{background:`var(--dom-${c.domain})`}}/>
                      <span>{c.title}</span>
                    </span>
                  ))}</div> },
      ...
      <InspectorPanel.Section
        ...
        actions={<Button variant="link" size="sm">Show all →</Button>}
      >
        {indSample.length === 0 ? (
          ...
          <div style={{display:'flex', flexDirection:'column', gap: 4}}>
            {indSample.map((i) => (
              <div key={i.id} style={{display:'flex', alignItems:'center', gap: 12, padding:'4px 0'}}>
      ...
      <InspectorPanel.Section title="Relationships" count={rels.length}>
        {rels.length === 0 ? (
          <em style={{fontSize:12, color:'rgb(var(--canvas-fg-3))'}}>— not used in any relationships —</em>
        ) : (
          <div style={{display:'flex', flexDirection:'column', gap: 8, fontFamily:'var(--font-mono)', fontSize: 11.5}}>
            {rels.map((r) => {
              ...
              const other = isSource
                ? (data.classes.find((c) => c.id === r.target_id) || data.individuals.find((i) => i.id === r.target_id))
                : (data.classes.find((c) => c.id === r.source_id) || data.individuals.find((i) => i.id === r.source_id));
              return (
                <div key={r.id} style={{display:'flex', gap: 12}}>
                  <span style={{flex:1}}>
                    {isSource ? cls.title : (other?.title || '?')}
                    <span style={{color:'rgb(var(--canvas-fg-3))'}}> — {prop?.identifier} → </span>
  ...
  return (
    <div data-screen-label="02 Schema · Taxonomies">
      <PageHeader
        ...
        actions={<><Button variant="ghost"><Icon name="download" size={13}/> Export</Button><Button variant="primary"><Icon name="plus" size={13}/> New taxonomy</Button></>}
      />
      <div style={{height:14}}/>
      <SchemaTabs route={route} onNav={onNav}/>
      <Panel noPadding>
        <Table
          rowKey="id"
          columns={[
            { key: 'swatch', label: '', width: '36px', render: (_, t) => <span className="cs-domain-dot" style={{background:`var(--dom-${t.domain})`}}/> },
            { key: 'id', label: 'Identifier', width: '120px', render: (v) => <span style={{fontFamily:'var(--font-mono)', fontSize:12.5}}>{v}</span> },
            { key: 'title', label: 'Title', render: (v) => <span style={{fontWeight: 500}}>{v}</span> },
            { key: 'description', label: 'Description', render: (v) => <span style={{color:'rgb(var(--canvas-fg-3))', fontSize:12.5}}>{v}</span> },
            { key: 'classes', label: 'Classes', width: '90px', render: (v) => <span style={{fontFamily:'var(--font-mono)', textAlign:'right', display:'block'}}>{v}</span> },
            { key: 'individuals', label: 'Individuals', width: '100px', render: (v) => <span style={{fontFamily:'var(--font-mono)', textAlign:'right', display:'block'}}>{v}</span> },
            { key: 'updated', label: 'Updated', width: '120px', render: (v) => <span style={{fontFamily:'var(--font-mono)', fontSize:11.5, color:'rgb(var(--canvas-fg-3))'}}>{v}</span> },
            { key: 'version', label: 'Ver', width: '60px', render: (v) => <VersionPill>{v}</VersionPill> },
          ]}
  ...
  return (
    <div data-screen-label="03 Schema · Concept schemes">
      <PageHeader
        ...
        actions={<><Button variant="ghost"><Icon name="download" size={13}/> Export</Button><Button variant="primary"><Icon name="plus" size={13}/> New scheme</Button></>}
      />
      <div style={{height:14}}/>
      <SchemaTabs route={route} onNav={onNav}/>
      <Panel noPadding>
        <Table
          rowKey="id"
          columns={[
            { key: 'swatch', label: '', width: '36px', render: (_, s) => <span className="cs-domain-dot" style={{background:`var(--dom-${s.domain})`}}/> },
            { key: 'id', label: 'Identifier', width: '160px', render: (v) => <span style={{fontFamily:'var(--font-mono)', fontSize:12.5}}>{v}</span> },
            { key: 'title', label: 'Title', render: (v) => <span style={{fontWeight: 500}}>{v}</span> },
            { key: 'description', label: 'Description', render: (v) => <span style={{color:'rgb(var(--canvas-fg-3))', fontSize:12.5}}>{v}</span> },
            { key: 'taxonomy_id', label: 'Taxonomy', width: '160px', render: (v) => {
              const t = data.taxonomies.find((x) => x.id === v);
              return <Chip variant="neutral">{t?.title}</Chip>;
            }},
            { key: 'classes', label: 'Classes', width: '80px', render: (v) => <span style={{fontFamily:'var(--font-mono)'}}>{v}</span> },
            { key: 'version', label: 'Ver', width: '60px', render: (v) => <VersionPill>{v}</VersionPill> },
          ]}
...
function CSSchemaProperties({ route, onNav }) {
  const data = D();
  const relChip = (r) => r === true ? <Chip variant="emerald">relevant</Chip>
    : r === false ? <Chip variant="rose">irrelevant</Chip>
  ...
  return (
    <div data-screen-label="04 Schema · Properties">
      <PageHeader
        ...
        actions={<><Button variant="ghost"><Icon name="filter" size={13}/> Filter</Button><Button variant="primary"><Icon name="plus" size={13}/> New property</Button></>}
      />
      <div style={{height:14}}/>
      <SchemaTabs route={route} onNav={onNav}/>
      <Panel noPadding>
        <Table
          rowKey="id"
          columns={[
            { key: 'identifier', label: 'Identifier', width: '180px', render: (v) => <span style={{fontFamily:'var(--font-mono)', fontSize:12.5, fontWeight: 500}}>{v}</span> },
            { key: 'title', label: 'Title' },
            { key: 'description', label: 'Description', render: (v) => <span style={{color:'rgb(var(--canvas-fg-3))', fontSize:12.5}}>{v}</span> },
            { key: 'is_relevant', label: 'Relevance', width: '140px', render: relChip },
            { key: 'origin', label: 'Origin', width: '160px', render: (v) => <span style={{fontFamily:'var(--font-mono)', fontSize:11.5, color:'rgb(var(--canvas-fg-3))'}}>{v}</span> },
            { key: 'used_by', label: 'Used by', width: '80px', render: (v) => <span style={{fontFamily:'var(--font-mono)'}}>{v}</span> },
            { key: 'version', label: 'Ver', width: '60px', render: (v) => <VersionPill>{v}</VersionPill> },
          ]}
...
function CSSchemaRelationships({ route, onNav }) {
  const data = D();
  const lookup = (id) => data.classes.find((c) => c.id === id) || data.individuals.find((i) => i.id === id);
  return (
    <div data-screen-label="05 Schema · Relationships">
      <PageHeader
        ...
        actions={<><Button variant="ghost"><Icon name="download" size={13}/> Export</Button><Button variant="primary"><Icon name="plus" size={13}/> New relationship</Button></>}
      />
      <div style={{height:14}}/>
      <SchemaTabs route={route} onNav={onNav}/>
      <Panel noPadding>
        <Table
          ...
          columns={[
            { key: 'id', label: 'Identifier', width: '120px', render: (v) => <span style={{fontFamily:'var(--font-mono)', fontSize:12.5}}>{v}</span> },
            { key: 'source_id', label: 'Source', render: (v) => {
              const s = lookup(v);
              return <span style={{display:'inline-flex', alignItems:'center', gap:6}}><span className="cs-domain-dot" style={{background:`var(--dom-${s?.domain || 'default'})`}}/>{s?.title}</span>;
            }},
            { key: 'property_definition_id', label: 'Predicate', width: '180px', render: (v) => {
              const p = data.property_definitions.find((pd) => pd.id === v);
              return <span style={{fontFamily:'var(--font-mono)', fontSize:11.5}}>— {p?.identifier} →</span>;
            }},
            { key: 'target_id', label: 'Target', render: (v) => {
              const t = lookup(v);
              return <span style={{display:'inline-flex', alignItems:'center', gap:6}}><span className="cs-domain-dot" style={{background:`var(--dom-${t?.domain || 'default'})`}}/>{t?.title}</span>;
            }},
            { key: 'confidence', label: 'Conf.', width: '80px', render: (v) => <span style={{fontFamily:'var(--font-mono)'}}>{v.toFixed(2)}</span> },
            { key: 'created', label: 'Created', width: '120px', render: (v) => <span style={{fontFamily:'var(--font-mono)', fontSize:11.5, color:'rgb(var(--canvas-fg-3))'}}>{v}</span> },
          ]}
  ...
  return (
    <div data-screen-label="06 Data · Individuals">
      <PageHeader
        ...
        actions={<><Button variant="ghost"><Icon name="filter" size={13}/> Filter</Button><Button variant="primary"><Icon name="plus" size={13}/> New individual</Button></>}
      />
      <div style={{height:18}}/>
      <Panel noPadding>
        <Table
          rowKey="id"
          columns={[
            { key: 'id', label: 'Identifier', width: '180px', render: (v) => <span style={{fontFamily:'var(--font-mono)', fontSize:12.5}}>{v}</span> },
            { key: 'title', label: 'Title', render: (v) => <span style={{fontWeight: 500}}>{v}</span> },
            { key: 'description', label: 'Description', render: (v) => <span style={{color:'rgb(var(--canvas-fg-3))', fontSize:12.5}}>{v}</span> },
            { key: 'class_ids', label: 'Classes', width: '200px', render: (v) => (
              <div style={{display:'flex', flexWrap:'wrap', gap:4}}>
                {v.map((cid) => {
                  const c = data.classes.find((x) => x.id === cid);
                  return c && <Chip key={cid} variant="neutral">{c.title}</Chip>;
                })}
              </div>
            )},
            { key: 'source', label: 'Source', width: '100px', render: (v) => <span style={{fontFamily:'var(--font-mono)', fontSize:11.5, color:'rgb(var(--canvas-fg-3))'}}>{v}</span> },
            { key: 'confidence', label: 'Conf.', width: '80px', render: (v) => <span style={{fontFamily:'var(--font-mono)'}}>{v.toFixed(2)}</span> },
            { key: 'version', label: 'Ver', width: '60px', render: (v) => <VersionPill>{v}</VersionPill> },
          ]}
...
function CSPipelines({ onNav }) {
  const data = D();
  const ICON_REMAP = { reference: 'link', sparkle: 'zap', doc: 'file', database: 'hardDrive' };
  // Per-stage tone now ships natively via FlowNode.color (source→extract→resolve→write).
  const STAGE_TONES = ['cyan', 'violet', 'amber', 'emerald'];
  const toHeimdallPipeline = (p) => ({
    id: p.id, name: p.name, description: p.description, status: p.status, target: p.target, tags: p.tags,
    lastRun: p.lastRun,
    flow: p.flow.map((s, i) => ({ id: `${p.id}-${i}`, name: s.name, label: s.kind, icon: ICON_REMAP[s.ic] || s.ic, color: STAGE_TONES[i % STAGE_TONES.length] })),
    recent: p.recent || { ingested: 0, created: 0, updated: 0, errors: 0 },
  });
  return (
    <div data-screen-label="07 Pipelines">
      <PageHeader
        ...
        actions={<><Button variant="ghost"><Icon name="reload" size={13}/> Refresh</Button><Button variant="primary"><Icon name="plus" size={13}/> New pipeline</Button></>}
      />
      <div style={{height:18}}/>
      <div style={{display:'flex', flexDirection:'column', gap: 14}}>
        {data.pipelines.map((p) => (
          <PipelineCard key={p.id} pipeline={toHeimdallPipeline(p)} onRun={() => {}} onCancel={() => {}} onOptions={() => {}} />
        ))}
      </div>
    </div>
  );
...
function CSStub({ route }) {
  return (
    <div data-screen-label={`Stub · ${route}`}>
      <PageHeader
        title={route.split('/').map((s) => s[0].toUpperCase()+s.slice(1)).join(' · ')}
        idChip={`/${route}`}
      ...
      >
        <p style={{margin: 0, color:'rgb(var(--canvas-fg-2))', fontSize: 13.5, lineHeight: 1.7}}>
          The shell, sidebar, topbar, and statusbar around this canvas are
          all real <code>@tinkermonkey/heimdall-ui</code> components. So is every
          chip, button, table, and panel on the other pages. The dev team can
          take the JSX from any rendered page and paste it into the production
          codebase with only the imports to add at the top of the file.
        </p>
      </Panel>
    </div>
  );
