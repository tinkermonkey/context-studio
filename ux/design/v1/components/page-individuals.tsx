        ...
        <div>
          <div className="page-head-meta">
            ...
            <span className="muted mono" style={{ fontSize: 11 }}>
              {filtered.length} of {D.individuals.length}
            </span>
          </div>
          <h1>
            Individuals <span className="id-tag">/data/individuals</span>
          </h1>
          <div className="subtitle">
            Concrete data nodes. Membership in parent classes is an ordered list — the first parent
            wins on inherited-property conflicts.
          </div>
        </div>
      ...
      <div className="filter-bar">
        ...
        <div className="seg">
          {[
            ...
            <button
              key={v}
              className={"seg-btn" + (parents === v ? " active" : "")}
              onClick={() => setParents(v)}
            >
              {l}
            </button>
          ))}
          ...
          return (
            <div
              ...
              style={{
                ...
                borderLeft:
                  i.id === selectedId ? "2px solid var(--accent-primary)" : "2px solid transparent",
                paddingLeft: 14,
              }}
            >
              <div>
...
function IndividualDetail({ individual }) {
    ...
    return (
      ...
      >
        <div className="empty">
          ...
          <div className="d">
            Click a row to inspect identity, membership, inherited properties, and relationships.
          </div>
        </div>
      </div>
    );
  ...
  const allMembership = individual.class_ids
    .map((cid) => D.classes.find((c) => c.id === cid))
    .filter(Boolean);
  const rels = D.relationships.filter(
    (r) => r.source_id === individual.id || r.target_id === individual.id,
  );
    ...
    <div className="inspector">
      ...
      <div className="inspector-body">
        ...
        <div className="kv-grid">
          ...
          <div className="v">
            <span className="version-pill">{individual.confidence.toFixed(2)}</span>
          </div>
        ...
        <div style={{ padding: "14px 16px", borderBottom: "1px solid var(--canvas-border)" }}>
          <div className="between" style={{ marginBottom: 10 }}>
            <div className="row gap-12">
              ...
              <span className="muted mono" style={{ fontSize: 11 }}>
                {individual.class_ids.length}
              </span>
            </div>
            <button className="btn btn-ghost btn-xs">
              <Icon name="plus" size={11} /> Add parent
            </button>
          </div>
            ...
            {allMembership.map((c, idx) => (
              <div
                ...
                style={{
                  ...
                  borderRadius: 4,
                  border: idx === 0 ? "1px solid rgba(251,191,36,0.22)" : "1px solid transparent",
                }}
              >
                <span className="mono muted" style={{ fontSize: 11, width: 16 }}>
                  {idx + 1}
                </span>
                ...
                <button
                  ...
                  disabled={individual.class_ids.length === 1}
                >
                  <Icon name="x" size={10} />
                </button>
              </div>
        ...
        <div style={{ padding: "14px 16px", borderBottom: "1px solid var(--canvas-border)" }}>
          <div className="between" style={{ marginBottom: 10 }}>
            ...
            <span className="muted mono" style={{ fontSize: 10.5 }}>
              GET /individuals/{individual.id}/inherited-properties
            </span>
          </div>
            ...
            <tbody>
              {inherited.map((p, i) => (
                <tr key={i}>
                  ...
                  <td>
                    <span className="kg-node-pill sm" data-domain={primary?.domain || "default"}>
                      <span className="swatch"></span>
                      {p.inherited_from}
                    </span>
                  </td>
        ...
        <div style={{ padding: "14px 16px" }}>
          <div className="between" style={{ marginBottom: 10 }}>
            ...
            <span className="muted mono" style={{ fontSize: 10.5 }}>
              cascade-deleted with this individual
            </span>
          </div>
          {rels.length === 0 ? (
              ...
              {rels.map((r) => {
                ...
                return (
                  <div
                    key={r.id}
                    className="row gap-12"
                    style={{ padding: "6px 0", borderBottom: "1px solid var(--canvas-border)" }}
                  >
                    <span
                      style={{ color: isSource ? "var(--accent-primary)" : "var(--canvas-fg-1)" }}
                    >
                      {isSource ? individual.title : other?.title}
                    </span>
