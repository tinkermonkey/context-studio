    ...
    <div className="pipeline-card">
      <div className="pipeline-card-head">
        ...
        <div className="row gap-12" style={{flexShrink: 0}}>
          {p.status === "running" ? (
            <button className="btn btn-ghost btn-sm"><Icon name="pause" size={11} /> Cancel</button>
          ) : (
            <button className="btn btn-accent btn-sm" onClick={() => onRun && onRun(p)}>
              <Icon name="play" size={11} /> Run
            </button>
          )}
          <button className="btn btn-ghost btn-sm btn-icon" title="More"><Icon name="more" size={13} /></button>
        </div>
      </div>
      ...
      <div className="pipeline-card-flow">
        {p.flow.flatMap((n, i, arr) => [
          <div key={"n" + i} className="flow-node" data-kind={n.kind}>
            <div className="ic"><Icon name={n.ic} size={13} /></div>
            <div>
