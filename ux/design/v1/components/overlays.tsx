...
function CommandPalette({ onClose, onNav }) {
  ...
  const items = useMemo(() => {
    ...
    const recent = [
      {
        kind: "Recent",
        label: "BRCA1 — individual",
        hint: "opened 4m ago",
        route: "data/individuals",
        icon: "data",
      },
      {
        kind: "Recent",
      ...
      {
        kind: "Recent",
        label: "Genomics — concept scheme",
        hint: "opened 27m ago",
        route: "schema/schemes",
        icon: "schema",
      },
    ];
    const entityHits = q
    ...
    const all = q ? [...entityHits, ...navItems, ...actions] : [...recent, ...navItems, ...actions];
    if (!q) return all;
    ...
    <div className="overlay-backdrop" onClick={onClose}>
      ...
      >
          ...
          {items.length === 0 && (
            <div style={{ padding: "30px 16px", textAlign: "center", color: "var(--shell-fg-3)" }}>
              ...
              <div style={{ marginTop: 4, fontSize: 12 }}>
                Try a class, an individual, or a command verb.
              </div>
            </div>
          )}
          {grouped.flatMap(([kind, list]) => [
            <div key={kind + ":head"} className="palette-section">
              {kind === "Go" ? "Jump to" : kind === "Recent" ? "Recent" : kind}
            </div>,
            ...list.map((it) => {
        ...
        <div className="palette-foot">
          ...
          <span>
            <span className="kbd-mini">tab</span> autocomplete
          </span>
      ...
      >
        ...
        <div className="ws-dialog-actions">
          ...
          <button className="ws-action">
            ...
            <div className="d">
              Initialize a fresh <span className="mono">.context-studio/</span> directory.
            </div>
          </button>
          <button className="ws-action">
            <div className="ic">
        ...
        <div className="ws-list">
          {recent.map((w) => (
            <button
              key={w.name}
              className={"ws-recent-row" + (w.current ? " open" : "")}
              onClick={onClose}
            >
              <div className="mark">
    ...
    <div className="toast-stack">
      {toasts.map((t) => (
        <div key={t.id} className={"toast " + (t.kind || "")}>
          <Icon
            name={t.kind === "success" ? "check" : t.kind === "fail" ? "alert" : "sparkle"}
            size={14}
          />
          <div style={{ flex: 1 }}>
            <div>{t.title}</div>
