...
function SchemaPage({ route, onNav, onOpenNewClass }) {
  ...
  if (route === "schema/classes")       return <ClassesPage onNav={onNav} onOpenNewClass={onOpenNewClass} />;
...
function ClassesPage({ onNav, onOpenNewClass }) {
  ...
  const filtered = useMemo(() => D.classes.filter((c) => {
      ...
      <div className="page-head">
        <div>
          <div className="page-head-meta">
            ...
            <span className="muted mono" style={{fontSize:11}}>{D.classes.length} classes · {D.concept_schemes.length} schemes · {D.taxonomies.length} taxonomies</span>
          </div>
          ...
          <div className="subtitle">
            ...
            inherits from a parent class, and carries data-property definitions populated by pipelines or curators.
          </div>
        </div>
        <div className="page-actions">
          <button className="btn"><Icon name="ext" size={13} /> Export</button>
                ...
                {filtered.map((c) => {
                  ...
                  return (
                    <tr key={c.id} className={selectedId === c.id ? "selected" : ""} onClick={() => setSelectedId(c.id)}>
                      <td>
                        <span className="kg-node-pill sm" data-domain={c.domain}><span className="swatch"></span></span>
                      </td>
                      <td className="mono">{c.id}</td>
                      <td><span className="row-link">{c.title}</span></td>
                      <td className="muted" style={{fontSize: 12.5}}>{c.description}</td>
                      <td><span className="chip mono">{sch?.title}</span></td>
                      <td className="num">{c.individuals}</td>
                      <td style={{textAlign:"right"}}>
                        <div className="row-actions">
                          <button className="btn btn-ghost btn-icon" title="Edit"><Icon name="edit" size={12}/></button>
                          <button className="btn btn-ghost btn-icon" title="More"><Icon name="more" size={12}/></button>
                        </div>
                      </td>
                    </tr>
                  );
    ...
    <div className="inspector">
      ...
      <div className="inspector-body">
        ...
        <div className="kv-grid">
          ...
          <div className="v"><span className="kg-node-pill sm" data-domain={cls.domain}><span className="swatch"></span>{cls.domain}</span></div>
          <div className="k">Scheme</div>
          <div className="v"><span className="kg-node-pill sm" data-domain={sch.domain}><span className="swatch"></span>{sch.title}</span><span className="mono muted" style={{fontSize: 11}}>{sch.id}</span></div>
          <div className="k">Taxonomy</div>
          <div className="v"><span className="kg-node-pill sm" data-domain={tax.domain}><span className="swatch"></span>{tax.title}</span><span className="mono muted" style={{fontSize: 11}}>{tax.id}</span></div>
          <div className="k">Parent class</div>
          <div className="v">{parent ? (
            <span className="kg-node-pill sm" data-domain={parent.domain}><span className="swatch"></span>{parent.title}</span>
          ) : <em className="muted">— root —</em>}</div>
          ...
          <div className="v" style={{flexDirection: "row", flexWrap: "wrap", gap: 4}}>
            ...
            {children.map((c) => (
              <span key={c.id} className="kg-node-pill sm" data-domain={c.domain}><span className="swatch"></span>{c.title}</span>
            ))}
          </div>
        </div>
        ...
        <div style={{padding: "14px 16px", borderBottom: "1px solid var(--canvas-border)"}}>
          <div className="row between" style={{marginBottom: 8}}>
            <div className="row gap-12">
              ...
              <span className="muted mono" style={{fontSize: 11}}>{props.length}</span>
            </div>
            <button className="btn btn-ghost btn-xs"><Icon name="plus" size={11}/> Add</button>
          </div>
          <table className="tbl" style={{fontSize: 12}}>
            <thead><tr><th>Identifier</th><th>Title</th><th style={{width: 80}}>Used by</th></tr></thead>
            <tbody>
              {props.map((p) => (
                <tr key={p.id}>
        ...
        <div style={{padding: "14px 16px", borderBottom: "1px solid var(--canvas-border)"}}>
          <div className="row between" style={{marginBottom: 8}}>
            <div className="row gap-12">
              ...
              <span className="muted mono" style={{fontSize: 11}}>{cls.individuals} total</span>
            </div>
            <button className="btn btn-ghost btn-xs">Show all <Icon name="arrow" size={10}/></button>
          </div>
          {indSample.length === 0 && <em className="muted" style={{fontSize:12}}>— no individuals yet —</em>}
          <div style={{display: "flex", flexDirection: "column", gap: 4}}>
            {indSample.map((i) => (
              <div key={i.id} className="row gap-12" style={{padding: "4px 0"}}>
                <span style={{flex:1, fontSize: 12.5}}>{i.title}</span>
                <span className="mono muted" style={{fontSize: 11}}>{i.id}</span>
  ...
  return (
    <div className="canvas-inner" data-screen-label="02 Schema · Taxonomies">
      <div className="page-head">
        <div>
          <div className="page-head-meta">
            <span className="chip cyan"><span className="dot"></span>schema</span>
            <span className="id-tag">node_type · taxonomy</span>
            <span className="muted mono" style={{fontSize:11}}>{D.taxonomies.length} taxonomies · {D.concept_schemes.length} schemes total</span>
          </div>
          <h1>Taxonomies <span className="id-tag">/schema/taxonomies</span></h1>
          <div className="subtitle">A taxonomy is the top-level domain. It groups concept schemes — each scheme is a focused area of knowledge.</div>
        </div>
        <div className="page-actions">
          <button className="btn"><Icon name="ext" size={13} /> Export</button>
          <button className="btn btn-primary"><Icon name="plus" size={13} /> New taxonomy</button>
        </div>
      </div>
      ...
      <div className="panel">
        <table className="tbl">
          ...
          <tbody>
            {D.taxonomies.map((t) => (
              <tr key={t.id}>
                <td><span className="kg-node-pill sm" data-domain={t.domain}><span className="swatch"></span></span></td>
                <td className="mono">{t.id}</td>
                <td><span className="row-link" style={{fontWeight: 600}}>{t.title}</span></td>
                <td className="muted" style={{fontSize: 12.5}}>{t.description}</td>
                <td><span className="chip mono">{D.concept_schemes.filter((s) => s.taxonomy_id === t.id).length}</span></td>
                <td className="num">{t.classes}</td>
                <td className="num">{t.individuals}</td>
                <td className="mono muted" style={{fontSize: 11.5}}>{t.updated}</td>
                <td style={{textAlign:"right"}}>
    ...
    <div className="canvas-inner" data-screen-label="03 Schema · Concept schemes">
      <div className="page-head">
        <div>
          ...
          <h1>Concept schemes <span className="id-tag">/schema/schemes</span></h1>
          <div className="subtitle">A concept scheme is a focused vocabulary inside a taxonomy. Classes belong to exactly one scheme.</div>
        </div>
        <div className="page-actions">
          <button className="btn"><Icon name="ext" size={13} /> Export</button>
          <button className="btn btn-primary"><Icon name="plus" size={13} /> New scheme</button>
        </div>
      </div>
        ...
        <table className="tbl">
          ...
          <tbody>
            {D.concept_schemes.map((s) => {
              ...
              return (
                <tr key={s.id}>
                  <td><span className="kg-node-pill sm" data-domain={s.domain}><span className="swatch"></span></span></td>
                  <td className="mono">{s.id}</td>
                  <td><span className="row-link" style={{fontWeight:600}}>{s.title}</span></td>
                  <td className="muted" style={{fontSize: 12.5}}>{s.description}</td>
                  <td><span className="kg-node-pill sm" data-domain={tax.domain}><span className="swatch"></span>{tax.title}</span></td>
...
function PropertiesPage({ onNav }) {
  ...
  const relevanceChip = (r) =>
    r === true  ? <span className="chip emerald"><span className="dot"></span>relevant</span> :
    r === false ? <span className="chip rose"><span className="dot"></span>irrelevant</span> :
                  <span className="chip gray"><span className="dot"></span>unevaluated</span>;
  return (
    <div className="canvas-inner" data-screen-label="04 Schema · Properties">
      <div className="page-head">
        <div>
          ...
          <h1>Properties <span className="id-tag">/schema/properties</span></h1>
          <div className="subtitle">Property definitions are the named predicates used by relationships. A tri-state relevance flag drives which appear in the inferred graph.</div>
        </div>
        <div className="page-actions">
          <button className="btn"><Icon name="filter" size={13} /> Filter</button>
          <button className="btn btn-primary"><Icon name="plus" size={13} /> New property</button>
        </div>
      </div>
        ...
        <table className="tbl">
          ...
          <tbody>
            {D.property_definitions.map((p) => (
              <tr key={p.id}>
                <td className="mono"><span className="row-link">{p.identifier}</span></td>
                <td>{p.title}</td>
                <td className="muted" style={{fontSize: 12.5}}>{p.description}</td>
                <td>{relevanceChip(p.is_relevant)}</td>
                <td className="mono muted" style={{fontSize: 11.5}}>{p.origin}</td>
                <td className="num">{p.used_by}</td>
...
function RelationshipsPage({ onNav }) {
  const D = window.CS_DATA;
  const lookup = (id) => D.classes.find((c) => c.id === id) || D.individuals.find((i) => i.id === id);
  return (
    <div className="canvas-inner" data-screen-label="05 Schema · Relationships">
      <div className="page-head">
        <div>
          ...
          <h1>Relationships <span className="id-tag">/schema/relationships</span></h1>
          <div className="subtitle">A relationship is a typed triple <span className="mono">(s, p, o)</span> between two graph nodes. Source and confidence are tracked separately from the data.</div>
        </div>
        <div className="page-actions">
          <button className="btn"><Icon name="ext" size={13} /> Export</button>
          <button className="btn btn-primary"><Icon name="plus" size={13} /> New relationship</button>
        </div>
      </div>
            ...
            {D.relationships.map((r) => {
              ...
              return (
                <tr key={r.id}>
                  ...
                  <td>
                    <span className="kg-node-pill sm" data-domain={s?.domain || "default"}><span className="swatch"></span>{s?.title}</span>
                  </td>
                  <td className="mono"><span className="row-link">— {p?.identifier} →</span></td>
                  <td>
                    <span className="kg-node-pill sm" data-domain={t?.domain || "default"}><span className="swatch"></span>{t?.title}</span>
                  </td>
                  <td className="mono muted" style={{fontSize: 11.5}}>{r.source}</td>
                  <td className="num">{r.confidence.toFixed(2)}</td>
                  <td className="mono muted" style={{fontSize: 11.5}}>{r.created}</td>
                  <td style={{textAlign:"right"}}>
                    <button className="btn btn-ghost btn-icon"><Icon name="more" size={12}/></button>
                  </td>
                </tr>
