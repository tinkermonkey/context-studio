...
class APIError extends Error {
  constructor(error_code, detail, status = 400) {
    super(detail); this.error_code = error_code; this.detail = detail; this.status = status;
  }
}
...
function createStore() {
  ...
  const createTaxonomy = (data) => {
    ...
    const t = { id: makeId('tax'), node_type: 'taxonomy', domain: data.domain || 'default', version: 0, ...data };
    state.taxonomies = [t, ...state.taxonomies]; notify(); return t;
  };
  const updateTaxonomy = (id, patch) => {
    ...
    state.taxonomies = state.taxonomies.map((x) => x.id === id ? next : x); notify(); return next;
  };
  const deleteTaxonomy = (id) => {
    if (state.concept_schemes.some((s) => s.taxonomy_id === id))
      throw new APIError('has_children', 'Cannot delete a taxonomy that contains concept schemes', 409);
    state.taxonomies = state.taxonomies.filter((x) => x.id !== id); notify();
  };
  ...
  const createScheme = (data) => {
    ...
    const s = { id: makeId('sch'), node_type: 'concept_scheme', domain: tax.domain, version: 0, ...data };
    state.concept_schemes = [s, ...state.concept_schemes]; notify(); return s;
  };
  const updateScheme = (id, patch) => {
    ...
    state.concept_schemes = state.concept_schemes.map((x) => x.id === id ? next : x); notify(); return next;
  };
  const deleteScheme = (id) => {
    if (state.classes.some((c) => c.concept_scheme_id === id))
      throw new APIError('has_children', 'Cannot delete a concept scheme that contains classes', 409);
    state.concept_schemes = state.concept_schemes.filter((x) => x.id !== id); notify();
  };

  // ---------- Class CRUD + tree ----------
  const createClass = (data) => {
    requireTitle(data.title);
    if (!data.concept_scheme_id) throw new APIError('invalid_input', 'concept_scheme_id is required', 400);
    ...
    if (data.parent_class_id) {
      ...
      if (parent.concept_scheme_id !== scheme.id)
        throw new APIError('invalid_input', 'parent_class_id must belong to the same concept scheme', 400);
    }
    const cls = {
      id: makeId('cls'), node_type: 'class', taxonomy_id: scheme.taxonomy_id, domain: scheme.domain,
      parent_class_id: null, structural_property_id: null,
      external_references: [], lexical_senses: [], data_properties: [],
      version: 0, ...data,
    };
    state.classes = [cls, ...state.classes]; notify(); return cls;
  };
  const updateClass = (id, patch) => {
    ...
    state.classes = state.classes.map((x) => x.id === id ? next : x); notify(); return next;
  };
  const deleteClass = (id) => {
    if (state.classes.some((c) => c.parent_class_id === id))
      throw new APIError('has_children', 'Cannot delete a class with children — reparent or delete them first', 409);
    ...
    if (state.relationships.some((r) => r.source_id === id || r.target_id === id))
      throw new APIError('has_relationships', 'Cannot delete a class still used in relationships', 409);
    state.classes = state.classes.filter((x) => x.id !== id); notify();
  };
  ...
  const moveClass = (id, target_scheme_id) => {
    ...
    if (target.taxonomy_id !== c.taxonomy_id)
      throw new APIError('invalid_input', 'A class cannot be moved to a scheme in a different taxonomy', 400);
    const next = { ...c, concept_scheme_id: target.id, parent_class_id: null, version: c.version + 1 };
    state.classes = state.classes.map((x) => x.id === id ? next : x); notify(); return next;
  };
  ...
  const titleConflict = (title, class_ids, ignoreIndividualId = null) => {
    ...
    for (const cid of class_ids) {
      const clash = state.individuals.find((i) =>
  ...
  const createIndividual = (data) => {
    ...
    if (conflict) {
      ...
      throw new APIError('duplicate_title',
        '"' + data.title + '" already exists in class ' + (cls?.title || conflict.class_id), 409);
    }
    const now = new Date().toISOString();
    const ind = {
      id: makeId('ind'), node_type: 'individual',
    ...
    state.individuals = [ind, ...state.individuals]; notify(); return ind;
  };
  ...
  const updateIndividual = (id, patch) => {
    ...
    if (!sameTitle) {
      ...
      if (conflict) {
        const cls = state.classes.find((c) => c.id === conflict.class_id);
        throw new APIError('duplicate_title',
          '"' + nextTitle + '" already exists in class ' + (cls?.title || conflict.class_id), 409);
      }
    }
    const now = new Date().toISOString();
    const next = { ...i, title: nextTitle, description: nextDesc,
      last_modified: now, updated: now.slice(0, 10), version: i.version + 1 };
    state.individuals = state.individuals.map((x) => x.id === id ? next : x); notify(); return next;
  };
  ...
  const addIndividualClass = (id, class_id) => {
    ...
    if (conflict) {
      const cls = state.classes.find((c) => c.id === conflict.class_id);
      throw new APIError('duplicate_title',
        '"' + i.title + '" already exists in class ' + (cls?.title || conflict.class_id), 409);
    }
    const now = new Date().toISOString();
    const next = { ...i, class_ids: [...i.class_ids, class_id],
      last_modified: now, updated: now.slice(0, 10), version: i.version + 1 };
    state.individuals = state.individuals.map((x) => x.id === id ? next : x); notify(); return next;
  };
  ...
  const removeIndividualClass = (id, class_id) => {
    ...
    if (i.class_ids.length === 1)
      throw new APIError('invalid_input',
        'Cannot remove the last parent class — an individual must always have at least one', 400);
    const now = new Date().toISOString();
    const next = { ...i, class_ids: i.class_ids.filter((c) => c !== class_id),
      last_modified: now, updated: now.slice(0, 10), version: i.version + 1 };
    state.individuals = state.individuals.map((x) => x.id === id ? next : x); notify(); return next;
  };
  ...
  const replaceIndividualClasses = (id, class_ids) => {
    ...
    if (!sameSet)
      throw new APIError('invalid_input',
        'Reorder list must match the current class set exactly — use add/remove for changes', 400);
    if (class_ids.every((c, idx) => c === i.class_ids[idx])) return { ...i, _noop: true };
    const now = new Date().toISOString();
    const next = { ...i, class_ids: [...class_ids],
      last_modified: now, updated: now.slice(0, 10), version: i.version + 1 };
    state.individuals = state.individuals.map((x) => x.id === id ? next : x); notify(); return next;
  };
  ...
  const getInheritedProperties = (id) => {
    ...
    for (const cid of i.class_ids) {
      ...
      for (const dp of (cls.data_properties || [])) {
        const key = dp.key;
        if (!seen.has(key)) seen.set(key, { ...dp, identifier: key,
          inherited_from_class_id: cls.id, inherited_from_class_title: cls.title });
      }
    }
  ...
  const deleteIndividual = (id) => {
    const i = findOrThrow(state.individuals, id, 'Individual');
    const cascadedRels = state.relationships.filter((r) => r.source_id === id || r.target_id === id);
    state.relationships = state.relationships.filter((r) => r.source_id !== id && r.target_id !== id);
  ...
  const createPropertyDefinition = (data) => {
    ...
    if (state.property_definitions.some((p) => p.identifier === ident))
      throw new APIError('duplicate', 'A property with identifier "' + ident + '" already exists', 409);
    const p = { id: makeId('prop'), identifier: ident, title: data.title || ident, description: '', is_relevant: null, origin: 'manual', version: 0, ...data, identifier: ident };
    state.property_definitions = [p, ...state.property_definitions]; notify(); return p;
  };
  const updatePropertyDefinition = (id, patch) => {
    ...
    state.property_definitions = state.property_definitions.map((x) => x.id === id ? next : x); notify(); return next;
  };
  const deletePropertyDefinition = (id) => {
    ...
    state.property_definitions = state.property_definitions.filter((x) => x.id !== id); notify();
  };

  // ---------- Relationship CRUD ----------
  // Auto-creates a PropertyDefinition if relationship_type doesn't match an existing one.
  const findEntity = (id) => {
    return state.classes.find((c) => c.id === id) || state.individuals.find((i) => i.id === id) || null;
  };
  const createRelationship = (data) => {
    if (!data.source_id || !data.target_id)
      throw new APIError('invalid_input', 'source_id and target_id are required', 400);
    if (data.source_id === data.target_id)
      throw new APIError('invalid_input', 'Self-loops are not allowed (source_id == target_id)', 400);
    ...
    if (!property_definition_id) {
      ...
      if (!prop) {
        prop = { id: makeId('prop'), identifier: ident,
          title: ident.split('_').map((w) => w[0].toUpperCase() + w.slice(1)).join(' '),
          description: 'Auto-created from relationship', is_relevant: null,
          origin: 'auto', version: 0 };
    ...
    if (state.relationships.some((r) => r.source_id === data.source_id && r.target_id === data.target_id && r.property_definition_id === property_definition_id))
      throw new APIError('duplicate', 'A relationship with this (source, target, property) triple already exists', 409);

    const rel = { id: makeId('rel'), source_id: data.source_id, target_id: data.target_id,
      property_definition_id, source: data.source || 'manual', confidence: data.confidence ?? 1.0,
  ...
  const deleteRelationship = (id) => {
    state.relationships = state.relationships.filter((r) => r.id !== id); notify();
  };
  ...
  return {
    ...
    actions: {
      ...
      createScheme, updateScheme, deleteScheme,
      createClass, updateClass, deleteClass, moveClass,
      createIndividual, updateIndividual, deleteIndividual,
      addIndividualClass, removeIndividualClass, replaceIndividualClasses,
      getInheritedProperties, individualRelationships,
...
function StoreProvider({ children }) {
  ...
  const actions = useMemo(() => {
    ...
    const wrap = (name, fn, successMsg) => (...args) => {
      try {
        ...
        if (e instanceof APIError) {
          pushToast({ kind: 'error', title: 'HTTP ' + e.status + ' · ' + e.error_code, body: e.detail, duration: 6000 });
        } else { pushToast({ kind: 'error', title: 'Unexpected error', body: e.message }); }
        throw e;
      }
    };
    return {
      createTaxonomy:           wrap('createTaxonomy', raw.createTaxonomy, (r) => ({ title: 'Taxonomy created', body: r.title })),
      updateTaxonomy:           wrap('updateTaxonomy', raw.updateTaxonomy, (r) => ({ title: 'Taxonomy updated', body: r.title + ' · v' + r.version })),
      deleteTaxonomy:           wrap('deleteTaxonomy', raw.deleteTaxonomy, { title: 'Taxonomy deleted' }),
      createScheme:             wrap('createScheme', raw.createScheme, (r) => ({ title: 'Concept scheme created', body: r.title })),
      updateScheme:             wrap('updateScheme', raw.updateScheme, (r) => ({ title: 'Concept scheme updated', body: r.title + ' · v' + r.version })),
      deleteScheme:             wrap('deleteScheme', raw.deleteScheme, { title: 'Concept scheme deleted' }),
      createClass:              wrap('createClass', raw.createClass, (r) => ({ title: 'Class created', body: r.title })),
      updateClass:              wrap('updateClass', raw.updateClass, (r) => ({ title: 'Class updated', body: r.title + ' · v' + r.version })),
      ...
      createIndividual:         wrap('createIndividual', raw.createIndividual, (r) => ({ title: 'Individual created', body: r.title })),
      updateIndividual:         wrap('updateIndividual', raw.updateIndividual, (r) => r._noop ? null : ({ title: 'Individual updated', body: r.title + ' · v' + r.version })),
      addIndividualClass:       wrap('addIndividualClass', raw.addIndividualClass, (r) => ({ title: 'Parent class added', body: r.title + ' · ' + r.class_ids.length + ' parents' })),
      removeIndividualClass:    wrap('removeIndividualClass', raw.removeIndividualClass, (r) => ({ title: 'Parent class removed', body: r.title + ' · ' + r.class_ids.length + ' parents' })),
      replaceIndividualClasses: wrap('replaceIndividualClasses', raw.replaceIndividualClasses, (r) => r._noop ? null : ({ title: 'Class order updated', body: r.title })),
      deleteIndividual:         wrap('deleteIndividual', raw.deleteIndividual, (r) => ({ title: 'Individual deleted', body: r.cascaded_relationships.length ? 'Cascaded ' + r.cascaded_relationships.length + ' relationship' + (r.cascaded_relationships.length === 1 ? '' : 's') : null })),
      getInheritedProperties:   raw.getInheritedProperties,
      individualRelationships:  raw.individualRelationships,
      createPropertyDefinition: wrap('createPropertyDefinition', raw.createPropertyDefinition, (r) => ({ title: 'Property created', body: r.identifier })),
      updatePropertyDefinition: wrap('updatePropertyDefinition', raw.updatePropertyDefinition, (r) => ({ title: 'Property updated', body: r.identifier + ' · v' + r.version })),
      deletePropertyDefinition: wrap('deletePropertyDefinition', raw.deletePropertyDefinition, { title: 'Property deleted' }),
      createRelationship:       wrap('createRelationship', raw.createRelationship, (r) => ({ title: 'Relationship created', body: r.auto_created ? 'Auto-created property "' + r.auto_created.identifier + '"' : null })),
      deleteRelationship:       wrap('deleteRelationship', raw.deleteRelationship, { title: 'Relationship deleted' }),
    };
  }, [pushToast]);
  ...
  const value = useMemo(() => ({
    ...
    actions,
  }), [actions, toasts]); // toasts in deps just to retrigger; state mutates in place + force()

  return (
    <StoreCtx.Provider value={value}>
      <ToastCtx.Provider value={{ toasts, pushToast, dismissToast }}>
        {children}
      </ToastCtx.Provider>
    </StoreCtx.Provider>
  );
