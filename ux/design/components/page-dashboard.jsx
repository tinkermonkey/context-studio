    ...
    <div className="canvas-inner">
      <div className="page-head">
        <div>
          ...
          <div className="subtitle">Curate knowledge graphs for retrieval-augmented generation and agents.</div>
        </div>
      ...
      <div className="stat-grid" style={{marginBottom: 22}}>
        <StatTile label="Taxonomies" value={D.taxonomies.length} color="cyan"
          meta={<span className="muted">3 active · 0 archived</span>} />
        <StatTile label="Classes" value={totalClasses} color="violet"
          meta={<><span className="delta-up">▲ 4</span><span className="muted">this week</span></>} />
        <StatTile label="Individuals" value={totalIndividuals} color="emerald"
          meta={<><span className="delta-up">▲ 38</span><span className="muted">last run</span></>} />
        <StatTile label="Pipelines" value={`${runningPipelines}/${D.pipelines.length}`} color="amber"
          meta={<span className="status-line"><span className="pulse"></span><span className="muted">1 running</span></span>} />
      </div>
        ...
        <div className="panel">
          <div className="panel-head">
            ...
            <div className="row gap-12">
              <span className="muted mono" style={{fontSize:11, whiteSpace:'nowrap'}}>3 tax · {totalClasses} cls</span>
              <button className="btn btn-ghost btn-sm" onClick={() => onNav('schema/classes')}>Open <Icon name="arrow"/></button>
            </div>
          </div>
            ...
            {D.activity.map((a, i) => (
              <div className={'activity-item ' + a.kind} key={i}>
                ...
                <div className="content">
                  <div className="head">
                    <span className="muted mono" style={{textTransform:'uppercase', fontSize:10, marginRight: 6, letterSpacing:'0.06em'}}>{a.kind}</span>
                    <b>{a.what}</b> — {a.subject}
                  </div>
                  <div className="meta-line">{a.meta} · by {a.who}</div>
                </div>
      ...
      <div className="panel" style={{marginBottom: 22}}>
        <div className="panel-head">
          ...
          <button className="btn btn-ghost btn-sm" onClick={() => onNav('pipelines/all')}>All pipelines <Icon name="arrow"/></button>
        </div>
        ...
        <div className="qa-grid">
          {[
            ...
            <div key={r} className="qa" onClick={() => onNav(r)}>
              <div className="qa-icon"><Icon name={ic} size={16}/></div>
              <div className="qa-body"><div className="n">{n}</div><div className="d">{d}</div></div>
            </div>
          ))}
        </div>
      </div>
