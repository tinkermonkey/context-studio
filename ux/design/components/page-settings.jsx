...
const CREDENTIAL_FIELD_NAMES = ['openai_api_key', 'anthropic_api_key', 's3_access_key', 's3_secret_key'];
...
const SECTIONS = [
  ...
  { id: 'database',          name: 'Database',           icon: 'database',  desc: 'Local store and operations DB paths.' },
  ...
  { id: 'embedding',         name: 'Embedding model',    icon: 'layers',    desc: 'Vector model and inference device.' },
  { id: 'reference_sources', name: 'Reference sources',  icon: 'reference', desc: 'External corpora — toggles & rate limits.' },
...
function StatusBadge({ status }) {
  ...
  return <span className={'chip ' + v.cls}><span className="dot"></span>{v.label}</span>;
}
    ...
    <Field label={label} hint={hint}>
      {replacing ? (
        <div className="cred-row replacing">
          ...
          />
          <button type="button" className="btn btn-ghost btn-sm" onClick={onClear}>Cancel</button>
        </div>
      ) : (
  ...
  return (
    <>
      <div className="form-grid two">
        ...
        <Field label="Port">
          <NumberInput value={value.port} min={1} max={65535} onChange={(v) => onChange({ port: v })} />
        </Field>
      </div>
      <Field label="CORS origins" hint="one per line">
        <TextArea
          ...
          onChange={(v) => onChange({ cors_origins: v.split('\n').map((s) => s.trim()).filter(Boolean) })}
        />
      </Field>
      <Field label="Request timeout" hint="seconds">
        <NumberInput value={value.request_timeout_seconds} min={1} max={600} onChange={(v) => onChange({ request_timeout_seconds: v })} />
      </Field>
    </>
  );
}
  ...
  return (
    <>
      <Field label="Local DB path" hint="primary store, includes schema + individuals">
        <TextInput mono value={value.local_db_path} onChange={(v) => onChange({ local_db_path: v })} />
      </Field>
      <Field label="Operations DB path" hint="task ledger, audit trail">
        <TextInput mono value={value.operations_db_path} onChange={(v) => onChange({ operations_db_path: v })} />
      </Field>
      <div className="form-grid two">
        <Field label="Connection pool size">
          <NumberInput value={value.pool_size} min={1} max={64} onChange={(v) => onChange({ pool_size: v })} />
        </Field>
        <Field label="Statement timeout (ms)">
          <NumberInput value={value.statement_timeout_ms} min={100} max={60000} step={100} onChange={(v) => onChange({ statement_timeout_ms: v })} />
        </Field>
      </div>
    </>
  );
...
function LLMForm({ value, onChange, drafts, setDraft }) {
  ...
  const models = {
    anthropic: ['claude-3-5-sonnet-20241022', 'claude-3-5-haiku-20241022', 'claude-opus-4-1-20250805'],
    ...
    <>
      ...
      <div className="form-grid two">
        <Field label="Default provider">
          <Select value={value.default_provider} onChange={(v) => onChange({ default_provider: v })} options={providers} />
        </Field>
        <Field label="Default model">
          <Select value={value.default_model} onChange={(v) => onChange({ default_model: v })} options={opts} />
        </Field>
      </div>
      <Field label="Request timeout" hint="seconds — provider call timeout">
        <NumberInput value={value.request_timeout_seconds} min={5} max={300} onChange={(v) => onChange({ request_timeout_seconds: v })} />
      </Field>
    </>
  );
}
  ...
  return (
    <>
      <Field label="spaCy model">
        <Select value={value.model_name} onChange={(v) => onChange({ model_name: v })} options={models} />
      </Field>
      <div className="form-grid two">
        <Field label="Batch size" hint="docs per spaCy pipe call">
          <NumberInput value={value.batch_size} min={1} max={1024} onChange={(v) => onChange({ batch_size: v })} />
        </Field>
        <Field label="Pretrained NER" hint="ents from spaCy stats model">
          <ToggleField value={value.enable_pretrained_ner} onChange={(v) => onChange({ enable_pretrained_ner: v })} />
        </Field>
      </div>
    </>
  );
    ...
    <>
      ...
      <div className="form-grid two">
        <Field label="Inference device">
          <Select value={value.device} onChange={(v) => onChange({ device: v })} options={devices} />
        </Field>
        <Field label="Batch size">
          <NumberInput value={value.batch_size} min={1} max={512} onChange={(v) => onChange({ batch_size: v })} />
        </Field>
      </div>
  ...
  return (
    <>
        ...
        {allSources.map((s) => {
          ...
          return (
            <div key={s.id} className={'ref-source-row' + (on ? ' on' : '')}>
              <button type="button" className={'src-toggle ' + (on ? 'on' : 'off')} onClick={() => toggle(s.id)} aria-label={on ? 'disable' : 'enable'}>
                <span className="src-toggle-dot"></span>
              </button>
              <span className="kg-node" data-domain={s.domain} style={{ height: 24, fontSize: 12 }}>
                <span className="swatch"></span>{s.label}
              </span>
      ...
      <Field label="Cache TTL" hint="seconds — applies to all enabled sources">
        <NumberInput value={value.cache_ttl_seconds} min={0} max={2592000} step={60} onChange={(v) => onChange({ cache_ttl_seconds: v })} />
      </Field>
    </>
  );
}
  ...
  return (
    <>
      <Field label="Level">
        <div className="seg">
          {levels.map((lv) => (
            <button key={lv} type="button" className={'seg-pill' + (value.level === lv ? ' active' : '')} onClick={() => onChange({ level: lv })}>
              {lv}
            </button>
          ))}
        </div>
      ...
      <div className="form-grid two">
        ...
        <Field label="Max file size (MB)" hint="rotate at threshold">
          <NumberInput value={value.max_file_mb} min={1} max={10000} onChange={(v) => onChange({ max_file_mb: v })} />
        </Field>
      </div>
    </>
  );
  ...
  if (!configured) {
    return (
      <div className="empty-section">
        <div className="empty-section-icon"><Icon name="ext" size={20}/></div>
  ...
  return (
    <>
      ...
      <Field label="Schedule" hint="cron expression — UTC">
        <TextInput mono value={value.schedule_cron} onChange={(v) => onChange({ schedule_cron: v })} />
      </Field>
    </>
  );
}
...
function SectionEditor({ section, value, onSave, onResetSection, syncConfigured, onConfigureSync }) {
  const [draft, setDraft] = useState(value);
  const [credDrafts, setCredDrafts] = useState({});
  useEffect(() => { setDraft(value); setCredDrafts({}); }, [section.id, value]);
  ...
  const handleDiscard = () => { setDraft(value); setCredDrafts({}); };

  const formProps = { value: draft, onChange: patch };
  let body;
  switch (section.id) {
    case 'server':            body = <ServerForm {...formProps} />; break;
    case 'database':          body = <DatabaseForm {...formProps} />; break;
    case 'llm':               body = <LLMForm {...formProps} drafts={credDrafts} setDraft={setCred} />; break;
    case 'nlp':               body = <NLPForm {...formProps} />; break;
    case 'embedding':         body = <EmbeddingForm {...formProps} />; break;
    case 'reference_sources': body = <ReferenceSourcesForm {...formProps} />; break;
    case 'logging':           body = <LoggingForm {...formProps} />; break;
    case 'sync':              body = <SyncForm {...formProps} drafts={credDrafts} setDraft={setCred} configured={syncConfigured} onConfigure={onConfigureSync} />; break;
    default: body = null;
  }
  ...
  return (
    <div className="cfg-pane">
      <div className="cfg-pane-head">
        <div>
          <div className="cfg-pane-title">
            <span className="cfg-pane-icon"><Icon name={section.icon} size={14}/></span>
      ...
      <div className="cfg-pane-foot">
        ...
        {dirty && <span className="dirty-flag mono"><span className="dot"></span>unsaved changes</span>}
        <button type="button" className="btn btn-ghost btn-sm" disabled={!dirty} onClick={handleDiscard}>Discard</button>
    ...
    <>
      <div className={'health-banner banner-' + health.status}>
        <div className="health-banner-l">
          <div className="health-banner-icon">
            ...
            <Icon name={health.status === 'healthy' ? 'check' : health.status === 'degraded' ? 'alert' : 'x'} size={20} />
          </div>
          <div>
            <div className="health-banner-status">
              <StatusBadge status={health.status} />
              <span className="health-banner-up mono">uptime {fmtUptime(health.uptime_seconds)}</span>
              <span className="health-banner-checked mono muted">checked {fmtRelative(health.checked_at)}</span>
            </div>
            <div className="health-banner-title">
              {health.status === 'healthy' && 'All components are operating normally.'}
              {health.status === 'degraded' && `${health.issues.length} ${health.issues.length === 1 ? 'issue' : 'issues'} — service is up but functionality is reduced.`}
      ...
      <div className="health-grid">
        ...
        <ComponentCheck
          ...
          detail={health.nlp_pipeline_ready ? "spaCy model 'en_core_web_sm' loaded" : 'pipeline not initialised'}
        ...
        <ComponentCheck
          ...
          detail={health.llm_providers_available.length > 0
      ...
      {health.issues.length > 0 && (
        <div className="issues-panel">
          <div className="issues-head">
            ...
            <span>{health.issues.length} active {health.issues.length === 1 ? 'issue' : 'issues'}</span>
            <span className="muted mono" style={{ fontSize: 11 }}>safe to surface · from <code>SystemHealthResponse.issues</code></span>
          </div>
    ...
    <div className="task-state-line">
      {TASK_STATES.map((s, i) => (
        <React.Fragment key={s}>
          <span className={'tsl-node ' + s + (reached(s) ? ' reached' : '') + (status === s ? ' current' : '')}>
            <span className="dot"></span><span className="lbl">{s}</span>
          </span>
          {i < TASK_STATES.length - 1 && (
            <span className={'tsl-edge ' + ((status === 'failed' && s === 'running') ? 'fork-down' : '') + (reached(TASK_STATES[i + 1]) ? ' reached' : '')}></span>
          )}
        </React.Fragment>
      ))}
    </div>
    ...
    <div className="task-drawer">
      ...
      <div className="task-drawer-body">
        <div className="task-drawer-row">
          ...
          <div className="td-val"><span className={'task-status-pill ' + task.status}><span className="dot"></span>{task.status}</span></div>
        </div>
        <div className="task-drawer-row">
          <div className="td-label mono">state machine</div>
          <div className="td-val"><TaskStateLine status={task.status} /></div>
        </div>
        <div className="task-drawer-row">
          <div className="td-label mono">created_at</div>
          <div className="td-val mono">{task.created_at} <span className="muted">· {fmtRelative(task.created_at)}</span></div>
        </div>
        <div className="task-drawer-row">
          <div className="td-label mono">started_at</div>
          <div className="td-val mono">{task.started_at || <span className="muted">null</span>}</div>
        </div>
        <div className="task-drawer-row">
          <div className="td-label mono">completed_at</div>
          <div className="td-val mono">{task.completed_at || <span className="muted">null</span>}</div>
        </div>
        <div className="task-drawer-row">
        ...
        {task.result && (
          <div className="task-drawer-row">
            ...
            <div className="td-val"><pre className="task-pre">{JSON.stringify(task.result, null, 2)}</pre></div>
          </div>
        )}
      </div>
  ...
  return (
    <div className="cfg-pane">
      <div className="cfg-pane-head">
        <div>
          <div className="cfg-pane-title">
            <span className="cfg-pane-icon"><Icon name="workflow" size={14}/></span>
            <span>Background tasks</span>
            <span className="chip gray">in-memory</span>
          </div>
          <div className="cfg-pane-sub">
            Tasks live for the lifetime of the server process — they are not persisted and are lost on restart.
            Created server-side by long-running operations; there is no API to start or cancel from the client.
          </div>
        </div>
        <div className="cfg-pane-endpoints">
      ...
      <div className="task-list-wrap">
        <table className="t task-table">
          ...
          <tbody>
            {list.map((t) => (
              <tr key={t.id} className={picked?.id === t.id ? 'picked' : ''} onClick={() => setPicked(t)}>
                <td>
                  <div className="task-name">{t.name}</div>
                  <div className="task-id mono">{t.id.slice(0, 8)}…{t.id.slice(-4)}</div>
                </td>
                <td><span className={'task-status-pill ' + t.status}><span className="dot"></span>{t.status}</span></td>
                <td className="mono muted" style={{ fontSize: 11.5 }}>{fmtRelative(t.created_at)}</td>
                <td className="mono" style={{ fontSize: 12 }}>{fmtDuration(t.started_at, t.completed_at)}</td>
                <td>
                  {t.error && <span className="task-cell-err mono">{t.error}</span>}
                  {t.result && <span className="task-cell-ok mono">{Object.entries(t.result).slice(0, 2).map(([k, v]) => `${k}=${v}`).join(' · ')}</span>}
                  {!t.error && !t.result && <span className="muted mono" style={{ fontSize: 11.5 }}>—</span>}
                </td>
                <td className="row-actions"><Icon name="arrow" size={12}/></td>
              </tr>
            ))}
            {list.length === 0 && (
              <tr><td colSpan={6} style={{ padding: '24px', textAlign: 'center', color: 'var(--canvas-fg-3)' }}>No tasks in this state.</td></tr>
            )}
          </tbody>
        </table>
      </div>
    ...
    <div className="cfg-pane">
      <div className="cfg-pane-head">
        <div>
          <div className="cfg-pane-title danger">
            <span className="cfg-pane-icon danger"><Icon name="alert" size={14}/></span>
            <span>Reset to defaults</span>
          </div>
          <div className="cfg-pane-sub">
            Resets every non-credential value across all sections to factory defaults. The four credential fields
            <span className="mono"> openai_api_key</span>, <span className="mono">anthropic_api_key</span>,
            <span className="mono"> s3_access_key</span>, <span className="mono">s3_secret_key</span> are preserved.
          </div>
        </div>
        <div className="cfg-pane-endpoints">
      ...
      <div className="cfg-pane-body">
        <div className="danger-card">
          <div className="danger-card-icon"><Icon name="alert" size={18}/></div>
          <div className="danger-card-body">
            ...
            <div className="danger-card-desc">
              Server, database paths, NLP/embedding models, reference-source toggles, logging, and sync schedule
              will all snap back to defaults. Active background tasks are unaffected. Credentials are preserved.
            </div>
      ...
      <ConfirmModal
        ...
        onConfirm={() => { setConfirm(false); onReset(); }}
...
function SettingsPage() {
  ...
  const refreshHealth = () => {
    ...
    toast.pushToast?.({ kind: 'success', title: 'Health re-checked', body: 'GET /health · all components polled.' });
  };
  ...
  const resetSection = (sectionId) => {
    ...
    toast.pushToast?.({ kind: 'success', title: 'Section reset', body: `${SECTIONS.find((s) => s.id === sectionId).name} restored to defaults.` });
  };
  ...
  const resetAll = () => {
    ...
    setConfig(next);
    toast.pushToast?.({ kind: 'success', title: 'Configuration reset', body: 'All non-credential values restored. Credentials preserved.' });
    setActive('overview');
  };

  const configureSync = () => {
    setSyncConfigured(true);
    toast.pushToast?.({ kind: 'success', title: 'Sync initialised', body: 'PATCH /configuration/sync is now valid.' });
  };
    ...
    <div className="canvas-inner cfg-canvas">
      <div className="page-head">
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
            <span className="chip cyan"><span className="dot"></span>workspace · default</span>
            <StatusBadge status={health.status} />
          </div>
          <h1>Configuration <span className="id-tag">/api/v1/admin</span></h1>
          <div className="subtitle">System health, eight configuration sections, and the in-memory background-task queue.</div>
        </div>
        <div className="page-actions">
          <button className="btn btn-ghost"><Icon name="ext"/> Export config</button>
          <button className="btn btn-ghost" onClick={refreshHealth}><Icon name="refresh"/> Re-check health</button>
        </div>
      </div>
      ...
      <div className="cfg-layout">
          ...
          <div className="cfg-rail-section">
            {NAV.map((n) => {
              ...
              return (
                <button
                  key={n.id}
                  className={'cfg-rail-item' + (isActive ? ' active' : '') + (n.tone === 'danger' ? ' danger' : '')}
                  onClick={() => setActive(n.id)}
                >
                  <span className="cfg-rail-ic"><Icon name={n.icon} size={14}/></span>
        ...
        <div className="cfg-content">
          {active === 'overview' && (
            <HealthOverview health={health} tasks={tasks} onRefresh={refreshHealth} onJump={(t) => setActive(t)} />
          )}
