...
function PipelinesPage({ onNav, route }) {
  ...
  if (route === "pipelines/flavors")
    return (
      <StubPage
        route={route}
        title="Pipeline flavors"
        subtitle="Pipeline templates — the kinds of source → extract → resolve → write chains the studio supports."
      />
    );
...
function PipelinesAllPage({ onNav }) {
  ...
  const counts = D.pipelines.reduce((a, p) => {
    ...
    <div className="canvas-inner" data-screen-label="09 Pipelines">
        ...
        <div>
          <div className="page-head-meta">
            <span className="chip amber">
              <span className="dot"></span>pipelines
            </span>
            <span className="id-tag">{D.pipelines.length} pipelines</span>
            <span className="muted mono" style={{ fontSize: 11 }}>
              {counts.running || 0} running · {counts.idle || 0} idle · {counts.success || 0} ok ·{" "}
              {counts.failed || 0} failed
            </span>
          </div>
          <h1>
            Pipelines <span className="id-tag">/pipelines/all</span>
          </h1>
          <div className="subtitle">
            A pipeline is a source → extract → resolve → write chain. Each run materializes
            individuals (or schema) from a reference source. Runs are append-only — partial writes
            are kept on failure.
          </div>
        </div>
      ...
      <div className="filter-bar">
        <div className="seg">
          {[
            ...
            >
              {l}{" "}
              {v !== "all" && (
                <span className="mono muted" style={{ marginLeft: 4 }}>
                  {counts[v] || 0}
                </span>
              )}
            </button>
        ...
        <span className="muted mono" style={{ fontSize: 11, marginLeft: "auto" }}>
          showing {filtered.length} of {D.pipelines.length}
        </span>
      </div>
    ...
    <div className="canvas-inner" data-screen-label="10 Pipelines · Runs">
      <div className="page-head">
        <div>
          ...
          <h1>
            Run history <span className="id-tag">/pipelines/runs</span>
          </h1>
          <div className="subtitle">
            All runs across all pipelines. Append-only — older runs are archived to{" "}
            <span className="mono">.context-studio/runs/</span>.
          </div>
        </div>
        <div className="page-actions">
          <button className="btn">
          ...
          <tbody>
            {runs.map((r) => (
              <tr key={r.id}>
                ...
                <td>
                  <span className="chip mono">{r.trigger}</span>
                </td>
...
function ReferencePage({ route, onNav }) {
  if (route === "reference/grounding")
    return (
      <StubPage
        route={route}
        title="Grounding workflows"
        subtitle="Multi-step pipelines that combine a reference source with extraction and resolution to ground a class or individual in real-world data."
      />
    );
    ...
    <div className="canvas-inner" data-screen-label="11 External Reference · Sources">
      <div className="page-head">
        <div>
          <div className="page-head-meta">
            <span className="chip cyan">
              <span className="dot"></span>reference
            </span>
            <span className="id-tag">{D.reference_sources.length} sources</span>
          </div>
          <h1>
            Reference sources <span className="id-tag">/reference/sources</span>
          </h1>
          <div className="subtitle">
            External APIs, datasets, and document corpora that pipelines pull from. Each source can
            power one or more pipelines.
          </div>
        </div>
        <div className="page-actions">
          <button className="btn">
            <Icon name="refresh" size={13} /> Health check
          </button>
          <button className="btn btn-primary">
          ...
          <table className="tbl">
            ...
            <tbody>
              {D.reference_sources.map((s) => (
                <tr key={s.id}>
                  <td>
                    <Icon name={kindIcon(s.kind)} size={14} className="muted" />
                  </td>
        ...
        <div className="stack">
          <div className="panel">
            ...
            <div
              ...
              style={{ display: "flex", flexDirection: "column", gap: 10, fontSize: 12.5 }}
            >
              <div>
                External references resolve canonical identifiers for classes and individuals.
              </div>
...
function CheckboxDropdown({ label, summary, children, width }) {
  ...
  return (
    <div className="dd" ref={ref}>
      ...
      {open && (
        <div className="fd-panel" style={width ? { minWidth: width } : null}>
          {children}
        </div>
      )}
    </div>
...
function GraphViewPage({ onNav }) {
  ...
  const edges = showOpts.edges
    ? D.relationships
        .filter((r) => nodes[r.source_id] && nodes[r.target_id])
        .map((r) => {
          const s = nodes[r.source_id],
            t = nodes[r.target_id];
  ...
  const domainSummary = (() => {
    const on = Object.entries(domains)
      .filter(([_, v]) => v)
      .map(([k]) => k);
        ...
        <div>
          <div className="page-head-meta">
            <span className="chip violet">
              <span className="dot"></span>graph
            </span>
            <span className="muted mono" style={{ fontSize: 11 }}>
              {D.classes.length} nodes · {D.relationships.length} edges · 3 domains
            </span>
          </div>
          <h1>
            Graph view <span className="id-tag">/graph</span>
          </h1>
          <div className="subtitle">
            Force-directed layout of classes (large nodes) and individuals (small nodes). Edges are
            typed relationships. Drag to reposition, scroll to zoom.
          </div>
        </div>
        <div className="page-actions">
          <button className="btn">
            <Icon name="refresh" size={13} /> Re-layout
          </button>
          <button className="btn btn-primary">
            <Icon name="ext" size={13} /> Export PNG
          </button>
        </div>
      </div>
        ...
        <CheckboxDropdown label="Domain" summary={domainSummary}>
          ...
          {[
            ...
            <div
              key={k}
              className="fd-row"
              onClick={() => setDomains((d) => ({ ...d, [k]: !d[k] }))}
            >
              <span className={"fd-checkbox" + (domains[k] ? " on" : "")}>
                {domains[k] && <Icon name="check" size={10} />}
              </span>
              <span className="kg-node-pill sm" data-domain={k}>
      ...
      <div className="graph-shell">
        ...
        <div className="graph-canvas">
          ...
          <div className="graph-toolbar">
            <button title="Pan" className="active">
          ...
          >
              ...
              return (
                <g key={e.id}>
                  <line
                    ...
                    stroke="#475569"
                    strokeWidth="1.2"
                    strokeDasharray={e.source === "manual" ? "" : "3 3"}
            ...
            {Object.values(nodes).map(({ x, y, cls }) => {
              ...
              return (
                <g
                  key={cls.id}
                  onClick={() => setSelectedNode(cls.id)}
          ...
          <div className="graph-legend">
            <div className="lg-row">
              <span className="sw" style={{ background: "#10B981" }}></span>life
            </div>
            <div className="lg-row">
              <span className="sw" style={{ background: "#FBBF24" }}></span>climate
            </div>
            <div className="lg-row">
              <span className="sw" style={{ background: "#818CF8" }}></span>software
            </div>
            <div
              className="lg-row"
              style={{ marginTop: 4, paddingTop: 4, borderTop: "1px solid var(--canvas-border)" }}
            >
              <svg width="22" height="6">
                <line x1="0" y1="3" x2="22" y2="3" stroke="#475569" strokeWidth="1.5" />
              </svg>
              manual
            </div>
            <div className="lg-row">
        ...
        <div className="panel" style={{ minHeight: 0, overflow: "auto" }}>
          <div className="panel-head">
            <div className="panel-title">
              <span className="kg-node-pill sm" data-domain={selected?.domain}>
                <span className="swatch"></span>
              </span>
              Selection
            </div>
          ...
          {selected && (
            <div>
              <div className="kv">
                ...
                <div className="v">
                  <span className="version-pill">v{selected.version}</span>
                </div>
              </div>
        ...
        <div>
          <div className="page-head-meta">
            ...
            <span className="id-tag">
              /workspace/{window.CS_DATA.workspace.name}/.context-studio
            </span>
          </div>
          <h1>
            Configuration <span className="id-tag">/settings</span>
          </h1>
          <div className="subtitle">
            Workspace-level configuration. Changes are written to{" "}
            <span className="mono">.context-studio/config.yaml</span> and synced on next push.
          </div>
        </div>
        <div className="page-actions">
          <button className="btn">
    ...
    <div className="split-3-1" style={{ alignItems: "start" }}>
      <div className="panel">
        ...
        <div className="panel-body" style={{ display: "grid", gap: 16 }}>
          <div className="field">
            <div className="lab">
              Name<span className="hint-r">snake_case</span>
            </div>
            <input className="input mono" defaultValue={D.workspace.name} />
            <div className="hint">
              The canonical workspace identifier. Used in URLs, exports, and the daemon API.
            </div>
          </div>
          <div className="field">
            <div className="lab">
              Local path<span className="hint-r">absolute or ~/</span>
            </div>
          ...
          <div className="field">
            ...
            <div className="row gap-12">
              ...
              <button className="btn btn-sm">
                <Icon name="refresh" size={11} /> Test
              </button>
            </div>
            <div className="hint">
              Where the local graph process listens. Restart required if changed.
            </div>
          </div>
        </div>
    ...
    <div className="config-grid">
      {[
        [
          "pipeline",
          "Default flavor",
          "claude-haiku · embeddings · neo4j-write",
          "Used for new pipelines unless overridden.",
        ],
        [
          "sparkle",
          "Model providers",
          "openai · anthropic · local-llama",
          "Order = priority on rate-limit fallback.",
        ],
    ...
    <div className="config-grid">
      {[
        ...
        [
          ...
          "Embeddings cache",
          "sqlite · 224 MB",
          "Reused across pipelines for resolve steps.",
        ],
      ].map(([ic, t, v, d]) => (
        <div key={t} className="config-tile">
          <div className="ic">
        ...
        <tbody>
          {members.map((m) => (
            <tr key={m.who}>
              ...
              <td>
                <span className="chip mono">{m.role}</span>
              </td>
    ...
    <div className="canvas-inner" data-screen-label={"99 " + (title || labels.join(" / "))}>
      <div className="page-head">
        <div>
          ...
          <div className="subtitle">
            {subtitle ||
              "This surface is wired in the prototype but doesn't have a richer mock yet — open the production app."}
          </div>
        </div>
      </div>
      <div className="panel">
        <div className="empty" style={{ padding: 60 }}>
          ...
          <div className="d">
            This page exists in the navigation. The detail view ships in the production studio.
          </div>
        </div>
      </div>
    </div>
