...
interface BezierPathResult {
  d: string
  mid: Point
  angle: number
}

function rectEdgePoint(cx: number, cy: number, w: number, h: number, tx: number, ty: number): Point {
  const dx = tx - cx
  const dy = ty - cy
...
function bezierPath(p1: Point, p2: Point, curvature: number = 0.28): BezierPathResult {
  const dx = p2.x - p1.x
  const dy = p2.y - p1.y
...
window.rectEdgePoint = rectEdgePoint;
