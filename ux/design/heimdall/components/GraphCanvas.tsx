...
interface BaseGraphNodeComponentProps {
  id: string;
  label: string;
  selected?: boolean;
  ...
  const markerUrl =
    variant === "hot"
      ...
      : variant === "irrelevant"
        ? `url(#${markerRoseId})`
        : `url(#${markerId})`;
  const classNames = ["graph-edge", variant !== "default" && `graph-edge--${variant}`]
    .filter(Boolean)
    .join(" ");
  return (
    <g
    ...
    >
      ...
      {label && (
        ...
        >
          ...
          />
          <text x={label.length * 3.3 + 7} y="12" className="graph-edge__label-text">
            {label}
          </text>
        </g>
      )}
...
interface GraphCanvasProps extends Omit<React.HTMLAttributes<HTMLDivElement>, "onSelect"> {
  nodes: GraphNodeData[];
  edges?: GraphEdge[];
...
const GraphCanvas = React.forwardRef<HTMLDivElement, GraphCanvasProps>(
  (
    {
      nodes = [],
      edges = [],
      ...
      >
        ...
        >
          <defs>
            ...
            >
              <circle cx="0" cy="0" r={Math.min(1, 0.5 * zoom)} className="graph-grid-dot" />
            </pattern>
          </defs>
