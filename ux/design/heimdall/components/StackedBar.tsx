...
interface StackedBarProps extends Omit<React.SVGAttributes<SVGSVGElement>, "children"> {
...
const StackedBar = React.forwardRef<SVGSVGElement, StackedBarProps>(
  (
    {
      stacks,
      colors,
          ...
          return (
            <g key={si}>
              {s.parts.map((p, pi) => {
                ...
                acc += ph;
                return (
                  <rect key={pi} x={x} y={y} width={bw} height={ph} fill={cs[pi % cs.length]} />
                );
              })}
              {axes && (
                <text
