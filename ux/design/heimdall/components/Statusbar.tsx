    ...
    switch (item.kind) {
      ...
      case "icon":
        return (
          <div
            key={index}
            className={`statusbar__item ${item.mono ? "statusbar__item--mono" : ""}`}
...
const Statusbar = React.forwardRef<HTMLDivElement, StatusbarProps>(
  ({ left, center, right, className = "", ...props }, ref) => {
    ...
    const renderSlot = (
      content: React.ReactNode | StatusbarItem[] | undefined,
    ): React.ReactNode => {
    ...
    return (
      <div ref={ref} role="status" className={classNames} {...props}>
        {left && (
          <div className="statusbar__slot statusbar__slot--left statusbar__left">
            {renderSlot(left)}
          </div>
        )}
        {center && (
          <div className="statusbar__slot statusbar__slot--center">{renderSlot(center)}</div>
        )}
        {right && (
          <div className="statusbar__slot statusbar__slot--right statusbar__right">
            {renderSlot(right)}
          </div>
        )}
      </div>
