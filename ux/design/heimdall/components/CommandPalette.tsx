...
const CommandPalette = React.forwardRef<HTMLDivElement, CommandPaletteProps>(
  (
    ...
    return (
      <div ref={paletteRef} className="command-palette-backdrop" onClick={handleBackdropClick}>
        <div className="command-palette" role="dialog" aria-modal="true" aria-labelledby={titleId}>
          <span id={titleId} className="command-palette__sr-title">
            Command palette
          </span>
          <div className="command-palette__header">
            ...
            <input
              ...
              aria-activedescendant={
                filtered[selectedIndex] ? `${listboxId}-${filtered[selectedIndex].id}` : undefined
              }
          ...
          <div id={listboxId} role="listbox" className="command-palette__list">
            {filtered.length === 0 ? (
              ...
              groups.map((group) => (
                ...
                >
                  {group.name && (
                    <div className="command-palette__group-header" aria-hidden="true">
                      {group.name}
                    </div>
                  )}
