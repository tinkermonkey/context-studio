interface GraphEdgeProps extends React.SVGAttributes<SVGGElement> {
  id: string;
  sourceId: string;
  targetId: string;
  label?: string;
...
const GraphEdge = React.forwardRef<SVGGElement, GraphEdgeProps>(
  ({ id, sourceId, targetId, label, variant = "default", className = "", ...props }, ref) => {
    ...
    const classNames = ["graph-edge", variant !== "default" && `graph-edge--${variant}`, className]
      .filter(Boolean)
      .join(" ");
