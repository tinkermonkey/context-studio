type SegmentKind = "ok" | "warn" | "error" | "idle" | "info";
...
interface StatusTimelineProps extends Omit<React.SVGAttributes<SVGSVGElement>, "children"> {
  tracks: StatusTrack[];
  range?: [number, number];
  width?: number;
  height?: number;
...
const StatusTimeline = React.forwardRef<SVGSVGElement, StatusTimelineProps>(
  (
    {
      tracks,
      range = [0, 100],
    ...
    const barH = Math.min(rowH * 0.6, 14);

    const xAt = (v: number) => pad.left + ((v - range[0]) / (range[1] - range[0] || 1)) * innerW;
