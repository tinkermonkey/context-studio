...
- **Heimdall tokens store raw RGB channel values** (e.g., `--canvas-fg-1: 11 18 32`) and must be wrapped in `rgb()` when used in CSS (`color: rgb(var(--canvas-fg-1))`). Using `var(--canvas-fg-1)` directly produces invalid CSS. The only exceptions are aliases defined in `app-overrides.css` (e.g., `--accent-cyan`) which already include the `rgb()` call.
- **CSS cascade layer ordering is critical.** `src/index.css` starts with `@layer properties, theme, base, components, utilities, heimdall, app, graph;` — this declaration must remain first. It ensures Heimdall's component styles and app overrides win over Tailwind's base reset (`border: 0px solid`). Removing or reordering this line silently kills all Heimdall component borders and backgrounds.
- **`--accent-primary` is Heimdall amber, not cyan.** Use `rgb(var(--status-cyan))` or the `--accent-cyan` alias from `app-overrides.css` for cyan accents.
