...
function App() {
  ...
  }, [tweaks.accent, tweaks.shellTone, tweaks.windowChrome, tweaks.showStatusBar, tweaks.darkCanvas]);
  ...
  let page;
  switch (route) {
    case 'dashboard': page = <DashboardPage onNav={nav} />; break;
    case 'schema/taxonomies': page = <TaxonomiesPage />; break;
    case 'schema/schemes': page = <ConceptSchemesPage />; break;
    case 'schema/classes': page = <ClassesPage />; break;
    case 'schema/properties': page = <PropertiesPage />; break;
    case 'schema/relationships': page = <RelationshipsPage />; break;
    case 'data/individuals': page = <IndividualsPage />; break;
    case 'data/datasets': page = <IndividualsPage />; break;
    case 'pipelines/all':
    case 'pipelines/runs':
    case 'pipelines/flavors': page = <PipelinesPage />; break;
    case 'reference/sources':
    case 'reference/grounding': page = <ReferencePage />; break;
    case 'settings': page = window.SettingsPage ? <window.SettingsPage /> : null; break;
    default: page = <DashboardPage onNav={nav} />;
  }

  return (
    <div className={'desktop-frame' + (tweaks.windowChrome ? ' with-chrome' : '')}>
      {tweaks.windowChrome && <Titlebar onPalette={() => setPaletteOpen(true)} onSwitchWs={() => setWorkspaceOpen(true)} />}

      <div className={'app-shell' + (collapsed ? ' collapsed' : '')}>
        <Sidebar route={route} onNav={nav} collapsed={collapsed} onToggle={() => setCollapsed(!collapsed)} />
        <div className="workspace">
          <Topbar route={route} onPalette={() => setPaletteOpen(true)} onSwitchWs={() => setWorkspaceOpen(true)} />
          <div className="canvas-area">{page}</div>
          {tweaks.showStatusBar && <Statusbar route={route} />}
        </div>
      </div>

      {paletteOpen && <CommandPalette onClose={() => setPaletteOpen(false)} onNav={(r) => { nav(r); setPaletteOpen(false); }} />}
      ...
      {window.TweaksPanel && (
        <window.TweaksPanel title="Tweaks" defaultOpen={false}>
          <window.TweakSection title="Studio appearance">
            <window.TweakToggle label="Dark canvas" value={tweaks.darkCanvas} onChange={(v) => setTweak('darkCanvas', v)} />
            ...
            <window.TweakRadio label="Shell tone" value={tweaks.shellTone} onChange={(v) => setTweak('shellTone', v)}
              options={[
          ...
          <window.TweakSection title="Desktop chrome">
            <window.TweakToggle label="Show window titlebar" value={tweaks.windowChrome} onChange={(v) => setTweak('windowChrome', v)} />
            <window.TweakToggle label="Show status bar" value={tweaks.showStatusBar} onChange={(v) => setTweak('showStatusBar', v)} />
          </window.TweakSection>
          <window.TweakSection title="Quick actions">
            <window.TweakButton label="Open command palette (⌘K)" onClick={() => setPaletteOpen(true)} />
            <window.TweakButton label="Switch workspace" onClick={() => setWorkspaceOpen(true)} />
            <window.TweakButton label={collapsed ? 'Expand sidebar' : 'Collapse sidebar'} onClick={() => setCollapsed(!collapsed)} />
          </window.TweakSection>
        </window.TweaksPanel>
      )}
    </div>
