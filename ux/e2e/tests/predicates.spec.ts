...
test.describe('Predicate Management', () => {
  ...
  test('should display the predicates table', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="predicate-table-toolbar"]')).toBeVisible();
    await expect(page.locator('[data-testid="predicate-add-button"]')).toBeVisible();
  });
  ...
  test('should create a new predicate', async ({ page }) => {
    ...
    await expect(page.getByRole('dialog', { name: 'Create New Predicate' })).toBeVisible();

    // Fill in the form
    await page.fill('[data-testid="predicate-title-input"]', predicateTitle);
    await page.fill('[data-testid="predicate-definition-input"]', predicateDefinition);

    // The identifier should be auto-generated, but we can verify it's there
    const identifierValue = await page.locator('[data-testid="predicate-identifier-input"]').inputValue();
    ...
    await expect(page.getByRole('dialog', { name: 'Create New Predicate' })).not.toBeVisible();

    // Verify predicate appears in the table
    await expect(page.locator('[data-testid="predicate-table"]')).toContainText(predicateTitle);

    // Verify predicate exists in backend
    const response = await apiRequest<{ data: any[] }>(page, '/api/predicates');
    const createdPredicate = response.data.find((p: any) => p.title === predicateTitle);
  ...
  test('should create a predicate with custom identifier', async ({ page }) => {
    ...
    await page.fill('[data-testid="predicate-title-input"]', predicateTitle);
    await page.fill('[data-testid="predicate-identifier-input"]', customIdentifier);
  ...
  test('should edit a predicate through the table', async ({ page }) => {
    ...
    await page.locator(`[data-testid="predicate-row-${predicateId}"]`).dblclick();

    // Wait for edit modal
    await expect(page.getByRole('dialog', { name: 'Edit Predicate' })).toBeVisible();

    // Clear and update the title
    await page.fill('[data-testid="predicate-title-input"]', updatedTitle);
    await page.fill('[data-testid="predicate-definition-input"]', updatedDefinition);

    // Submit
    await page.click('[data-testid="predicate-submit-button"]');

    // Wait for modal to close
    await expect(page.getByRole('dialog', { name: 'Edit Predicate' })).not.toBeVisible();

    // Verify updated values appear in table
    await expect(page.locator('[data-testid="predicate-table"]')).toContainText(updatedTitle);

    // Verify backend was updated
    const response = await apiRequest<any>(page, `/api/predicates/${predicateId}`);
  ...
  test('should delete a predicate', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="predicate-table"]')).not.toContainText(predicateTitle);
  ...
  test('should search for predicates', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="predicate-table"]')).toContainText(predicate1Title);
    await expect(page.locator('[data-testid="predicate-table"]')).toContainText(predicate2Title);
    await expect(page.locator('[data-testid="predicate-table"]')).toContainText(predicate3Title);
    ...
    await expect(page.locator('[data-testid="predicate-table"]')).toContainText(predicate2Title);
    await expect(page.locator('[data-testid="predicate-table"]')).not.toContainText(predicate1Title);
    await expect(page.locator('[data-testid="predicate-table"]')).not.toContainText(predicate3Title);
  ...
  test('should handle multiple predicate selections and bulk delete', async ({ page }) => {
    ...
    await page.locator(`[data-testid="predicate-row-${predicate1.id}"]`).locator('input[type="checkbox"]').check();
    await page.locator(`[data-testid="predicate-row-${predicate2.id}"]`).locator('input[type="checkbox"]').check();
    ...
    await expect(page.locator('[data-testid="predicate-table"]')).not.toContainText(`Bulk Delete 1 ${timestamp}`);
    await expect(page.locator('[data-testid="predicate-table"]')).not.toContainText(`Bulk Delete 2 ${timestamp}`);
  });

  test('should cancel predicate creation', async ({ page }) => {
    // Get current predicate count
    const beforeResponse = await apiRequest<{ data: any[], total: number }>(page, '/api/predicates');
    ...
    // Wait for create modal
    const createModal = page.getByRole('dialog', { name: 'Create New Predicate' });
    await expect(createModal).toBeVisible();

    // Fill in some data but don't submit
    await page.fill('[data-testid="predicate-title-input"]', 'Test Predicate Not Submitted');
    ...
    const afterResponse = await apiRequest<{ data: any[], total: number }>(page, '/api/predicates');
    const afterCount = afterResponse.total;

    expect(afterCount).toBe(beforeCount);

    // Verify the specific test predicate was not created
    const testPredicate = afterResponse.data.find((p: any) => p.title === 'Test Predicate Not Submitted');
  ...
  test('should validate identifier format', async ({ page }) => {
    ...
    await page.fill('[data-testid="predicate-identifier-input"]', 'invalid-identifier!@#');
    ...
    const modalBody = page.getByRole('dialog', { name: 'Create New Predicate' });
