...
test.describe('RAG Experiments', () => {
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    const hasEndpoint = await endpointExists(page, '/api/rag-experiments/paragraphs');
    await context.close();
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await expect(page.getByText('RAG Pipeline Experiments')).toBeVisible({ timeout: 10000 });
  });
  ...
  test('should load RAG experiments page', async ({ page }) => {
    ...
    // Verify description
    await expect(page.getByText(/Test and compare RAG pipeline performance/i)).toBeVisible();

    // Verify tabs are present
    await expect(page.getByRole('tab', { name: /Test Paragraphs/i })).toBeVisible();
  ...
  test('should display test paragraph list', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="test-paragraph-list"]')).toBeVisible({ timeout: 10000 });

    // Verify the create button is visible
    await expect(page.locator('[data-testid="test-paragraph-create-button"]')).toBeVisible();
  });
  ...
  test('should create a new test paragraph', async ({ page }) => {
    ...
    const createButton = page.locator('[data-testid="test-paragraph-create-button"]');
    ...
    await expect(page.locator('[data-testid="test-paragraph-editor"]')).toBeVisible({ timeout: 5000 });

    // Fill in the paragraph text
    await page.fill('[data-testid="test-paragraph-text-input"]', paragraphText);

    // Verify submit button is enabled
    const submitButton = page.locator('[data-testid="test-paragraph-submit-button"]');
    ...
    await expect(page.locator('[data-testid="test-paragraph-editor"]')).toBeVisible({ timeout: 3000 });

    // Wait for the paragraph to appear in the table
    await expect(page.locator('[data-testid="test-paragraph-table"]')).toContainText('E2E Test Paragraph', { timeout: 5000 });

    // Verify paragraph exists in backend
    const response = await apiRequest<{ paragraphs: any[] }>(page, '/api/rag-experiments/paragraphs?limit=100');
    const createdParagraph = response.paragraphs.find((p: any) => p.text.includes('E2E Test Paragraph'));
  ...
  test('should edit an existing test paragraph', async ({ page }) => {
    ...
    const createResponse = await apiRequest<any>(page, '/api/rag-experiments/paragraphs', {
      method: 'POST',
      body: {
        text: originalText,
      },
    ...
    const editButton = page.locator(`[data-testid="test-paragraph-edit-button-${paragraphId}"]`);
    ...
    const inputValue = await page.locator('[data-testid="test-paragraph-text-input"]').inputValue();
    ...
    await expect(page.locator('[data-testid="test-paragraph-table"]')).toContainText('(Updated)', { timeout: 5000 });

    // Verify paragraph updated in backend
    const response = await apiRequest<any>(page, `/api/rag-experiments/paragraphs/${paragraphId}`);
  ...
  test('should delete a test paragraph', async ({ page }) => {
    ...
    const deleteButton = page.locator(`[data-testid="test-paragraph-delete-button-${paragraphId}"]`);
    ...
    await expect(page.locator('[data-testid="test-paragraph-table"]')).not.toContainText(testText, { timeout: 5000 });

    // Verify paragraph deleted in backend
    const response = await page.request.fetch(`http://localhost:8888/api/rag-experiments/paragraphs/${paragraphId}`);
  ...
  test('should cancel paragraph creation', async ({ page }) => {
    ...
    // Type some text
    await page.fill('[data-testid="test-paragraph-text-input"]', 'This should be cancelled');
  ...
  test('should display annotation selector for existing paragraph', async ({ page }) => {
    // Create a paragraph via API
    ...
    // Click edit button
    const editButton = page.locator(`[data-testid="test-paragraph-edit-button-${createResponse.id}"]`);
    await expect(editButton).toBeVisible({ timeout: 10000 });
    await editButton.click();
    ...
    await expect(page.locator('[data-testid="annotation-selector"]')).toBeVisible({ timeout: 10000 });
  });
  ...
  test('should navigate to Run Tests tab', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="pipeline-test-runner"]')).toBeVisible({ timeout: 5000 });
  });
  ...
  test('should disable run button when no paragraphs selected', async ({ page }) => {
    // Navigate to Run Tests tab
    const runTestsTab = page.getByRole('tab', { name: /Run Tests/i });
    await runTestsTab.click();
    ...
    const hasDeselect = await deselectAllButton.isVisible({ timeout: 2000 }).catch(() => false);
  ...
  test('should display test results tab', async ({ page }) => {
    ...
    await expect(page.getByText(/No test results yet/i)).toBeVisible({ timeout: 5000 });
  });

  test('should validate required fields in paragraph editor', async ({ page }) => {
  ...
  test('should display annotation count in paragraph list', async ({ page }) => {
    // Create a paragraph via API
    ...
    await expect(page.locator('[data-testid="test-paragraph-table"]')).toContainText(/annotation/i, { timeout: 5000 });
  });
});
