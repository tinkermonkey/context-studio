...
test.describe('Term Management', () => {
  ...
  test('should display the terms table', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="term-table-toolbar"]')).toBeVisible();
  ...
  test('should create a new term with domain selection', async ({ page }) => {
    ...
    await expect(page.getByRole('dialog', { name: 'Create New Term' })).toBeVisible();
    ...
    const domainSelector = page.locator('[data-testid="term-domain-selector"]').locator('button');
    ...
    const domainResponse = await apiRequest<any>(page, `/api/structure_nodes/${testDomainId}`);
    const testDomainTitle = domainResponse.title;

    // Find the option that contains our test domain title
    const domainOption = page.locator(`div[role="option"]:has-text("${testDomainTitle}")`);
    ...
    await expect(page.getByRole('dialog', { name: 'Create New Term' })).not.toBeVisible();

    // Verify term appears in the table
    await expect(page.locator('[data-testid="term-table"]')).toContainText(termTitle);

    // Verify term exists in backend with correct domain association
    const response = await apiRequest<{ data: any[] }>(page, '/api/structure_nodes?node_type=term');
  ...
  test('should edit a term through the table', async ({ page }) => {
    ...
    // Wait for modal to close
    await expect(page.getByRole('dialog', { name: 'Edit Term' })).not.toBeVisible();

    // Verify updated values appear in table
    await expect(page.locator('[data-testid="term-table"]')).toContainText(updatedTitle);

    // Verify backend was updated
    const response = await apiRequest<any>(page, `/api/structure_nodes/${termId}`);
  ...
  test('should delete a term', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="term-table"]')).not.toContainText(termTitle);
  ...
  test('should search for terms', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="term-table"]')).toContainText(term1Title);
    await expect(page.locator('[data-testid="term-table"]')).toContainText(term2Title);
    await expect(page.locator('[data-testid="term-table"]')).toContainText(term3Title);
    ...
    await expect(page.locator('[data-testid="term-table"]')).toContainText(term2Title);
    await expect(page.locator('[data-testid="term-table"]')).not.toContainText(term1Title);
    await expect(page.locator('[data-testid="term-table"]')).not.toContainText(term3Title);
  ...
  test('should navigate to term detail view', async ({ page }) => {
    ...
    // Wait for navigation
    await page.waitForURL(`**/app/structure_nodes/${termId}`, { timeout: 15000 });
    await page.waitForLoadState('networkidle');
    ...
    await expect(page.locator('body')).toContainText(termTitle, { timeout: 10000 });
  });
  ...
  test('should cancel term creation', async ({ page }) => {
    ...
    await page.fill('[data-testid="term-title-input"]', 'Test Term Not Submitted');
    ...
    const testTerm = afterResponse.data.find((n: any) => n.title === 'Test Term Not Submitted');
  ...
  test('should validate required fields', async ({ page }) => {
    ...
    const definitionInput = page.locator('[data-testid="term-definition-input"]');
    await expect(definitionInput).toHaveAttribute('required');
  });

  test('should handle multiple term selections and bulk delete', async ({ page }) => {
    ...
    // Select both terms
    await page.locator(`[data-testid="term-row-${term1.id}"]`).locator('input[type="checkbox"]').check();
    await page.locator(`[data-testid="term-row-${term2.id}"]`).locator('input[type="checkbox"]').check();
    ...
    await expect(page.locator('[data-testid="term-table"]')).not.toContainText(`Bulk Delete 1 ${timestamp}`);
    await expect(page.locator('[data-testid="term-table"]')).not.toContainText(`Bulk Delete 2 ${timestamp}`);
  });

  test('should filter terms by domain', async ({ page }) => {
    // Create a second domain
    const domain2Response = await apiRequest<any>(page, '/api/structure_nodes', {
      method: 'POST',
      body: {
    ...
    await expect(page.locator('[data-testid="term-table"]')).toBeVisible({ timeout: 10000 });

    // Verify term1 is visible and term2 is not
    await expect(page.locator('[data-testid="term-table"]')).toContainText(term1Title, { timeout: 10000 });
    await expect(page.locator('[data-testid="term-table"]')).not.toContainText(term2Title);
    ...
    await expect(page.locator('[data-testid="term-table"]')).toContainText(term2Title, { timeout: 10000 });
    await expect(page.locator('[data-testid="term-table"]')).not.toContainText(term1Title);
  });
  ...
  test('should handle hierarchical filtering by both layer and domain', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="term-table"]')).toContainText(`Term D1 ${timestamp}`);
    await expect(page.locator('[data-testid="term-table"]')).not.toContainText(`Term D2 ${timestamp}`);
  });
});
