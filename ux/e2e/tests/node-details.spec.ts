...
test.describe("Structure Node Detail View", () => {
  ...
  test("should load and display layer detail page", async ({ page }) => {
    ...
    const definitionSection = page.locator(
      '[data-testid="node-detail-definition-section"]',
    );
    await expect(definitionSection).toBeVisible();
    await expect(definitionSection).toContainText(
      "Test layer for node detail tests",
    );
  });
  ...
  test("should load and display domain detail page", async ({ page }) => {
    ...
    await expect(definitionSection).toBeVisible();
    await expect(definitionSection).toContainText(
      "Test domain for node detail tests",
    );
  });
  ...
  test("should load and display term detail page", async ({ page }) => {
    ...
    await expect(definitionSection).toBeVisible();
    await expect(definitionSection).toContainText(
      "Test term for node detail tests",
    );
  });
  ...
  test("should open edit modal from detail page", async ({ page }) => {
    ...
    await expect(
      editModal.locator('[data-testid="domain-title-input"]'),
    ).toBeVisible();
    await expect(
      editModal.locator('[data-testid="domain-definition-input"]'),
    ).toBeVisible();
  });
  ...
  test("should display correct hierarchy for each node type", async ({
    page,
  }) => {
  ...
  test("should persist word sense selections across page reload", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator('[data-testid="nlp-analysis-panel"]'),
    ...
    await page.waitForSelector('[data-testid="nlp-concept-chart"]', {
      timeout: 10000,
    });
    ...
    const persistedSense = page
      .locator('[data-testid="persisted-word-sense"]')
      .first();
    await expect(persistedSense).toBeVisible({ timeout: 5000 });

    // Verify save button is NOT visible (not dirty)
    await expect(
      page.locator('button:has-text("Save Word Senses")'),
    ).not.toBeVisible();
  });
  ...
  test("should add and remove reference node links", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="reference-node-panel"]'),
    ).toBeVisible({ timeout: 10000 });

    // Initially should show "no references" message
    await expect(page.locator("text=/no reference.*associated/i")).toBeVisible({
      timeout: 5000,
    });
    ...
    await page.waitForSelector('[data-testid="reference-search-result"]', {
      timeout: 15000,
    });

    // Click first search result to add it
    const firstResult = page
      .locator('[data-testid="reference-search-result"]')
      .first();
    ...
    await page.waitForResponse(
      (resp) =>
        resp.url().includes("/reference_links") &&
    ...
    await expect(
      page.locator('[data-testid="reference-link-item"]'),
    ).toBeVisible({ timeout: 5000 });

    // Verify "no references" message is gone
    await expect(
      page.locator("text=/no reference.*associated/i"),
    ).not.toBeVisible();
    ...
    const removeButton = page
      .locator('[data-testid="remove-reference-button"]')
      .first();
