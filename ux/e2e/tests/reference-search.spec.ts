...
import { apiRequest, waitForElement, waitForAnyCondition, mockReferenceAPIs } from '../fixtures/test-helpers';
...
test.describe('Reference Data Search', () => {
  ...
  test('should load reference search page', async ({ page }) => {
    ...
    await expect(page.getByText('Search across multiple reference sources')).toBeVisible();

    // Verify search input is visible
    await expect(page.locator('[data-testid="reference-search-input"]')).toBeVisible();

    // Verify search button is visible
    await expect(page.locator('[data-testid="reference-search-button"]')).toBeVisible();

    // Verify source filter selector is visible
    await expect(page.locator('[data-testid="reference-source-filter"]')).toBeVisible();
  });
  ...
  test('should show validation message for short query', async ({ page }) => {
    ...
    await expect(page.getByText('Search query must be at least 2 characters long')).toBeVisible();

    // Verify search button is disabled
    await expect(page.locator('[data-testid="reference-search-button"]')).toBeDisabled();
  });
  ...
  test('should enable search with valid query', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="reference-search-button"]')).toBeEnabled();
  });
  ...
  test('should perform search and display results', async ({ page }) => {
    ...
    const resultIndex = await waitForAnyCondition(
      page,
      [
        async () => await page.locator('[data-testid="reference-results-list"]').isVisible(),
        async () => await page.getByText('No results found').isVisible(),
        async () => await page.getByText('Search failed').isVisible(),
        async () => await page.getByText('Searching across reference sources').isVisible(),
      ],
    ...
    if (resultIndex === 0 || resultIndex === 3) {
      ...
      await expect(page.getByRole('tab', { name: /Graph View|List View/i })).toBeVisible({ timeout: 5000 });
    }
  });
  ...
  test('should search on Enter key press', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="reference-search-input"]')).toBeDisabled({ timeout: 2000 });
  ...
  test('should disable search while searching', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="reference-search-input"]')).toBeDisabled({ timeout: 1000 });
  });
  ...
  test('should toggle between graph and list views', async ({ page }) => {
    ...
    await waitForAnyCondition(
      page,
      [
        async () => await page.getByRole('tab', { name: /Graph View/i }).isVisible(),
        async () => await page.getByRole('tab', { name: /List View/i }).isVisible(),
      ],
      15000
    );
    ...
    await expect(page.locator('[data-testid="reference-results-list"]')).toBeVisible({ timeout: 5000 });
  ...
  test('should show empty state when no search performed', async ({ page }) => {
    ...
    await expect(page.getByText('Enter a search term and select sources to begin searching')).toBeVisible();
  });
  ...
  test('should handle navigation to node details', async ({ page }) => {
    ...
    if (hasResults === 0) {
      ...
      await expect(page.locator('[data-testid="reference-results-list"]')).toBeVisible();

      // Try to find and click on a result card
      const resultCards = page.locator('[data-testid="reference-results-list"] [class*="Card"]');
  ...
  test('should display source status indicators during search', async ({ page }) => {
    // Type a search term
    await page.fill('[data-testid="reference-search-input"]', 'computer');
    ...
    const badges = page.locator('[class*="badge"], [class*="Badge"], span[class*="bg-blue"], span[class*="bg-green"]');
  ...
  test('should maintain search term in input after search', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="reference-search-input"]')).toBeEnabled({ timeout: 15000 });

    // Verify search term is still in the input
    const inputValue = await page.locator('[data-testid="reference-search-input"]').inputValue();
  ...
  test('should show error state when no sources selected', async ({ page }) => {
    ...
    if (await sourceSelectorButton.isVisible()) {
      ...
      const deselectButton = page.getByRole('button', { name: /deselect|none/i });
      if (await deselectButton.isVisible({ timeout: 1000 }).catch(() => false)) {
        ...
        await expect(page.getByText('Please select at least one reference source')).toBeVisible();
      }
    }
  });
});
