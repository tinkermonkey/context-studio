...
test.describe.skip("Pipeline Configuration", () => {
  ...
  test("should display pipeline type selection page", async ({ page }) => {
    ...
    await expect(
      page.getByText(/Choose which pipeline type you want to manage/i),
    ).toBeVisible();
  ...
  test("should navigate to specific pipeline type", async ({ page }) => {
    ...
    // Wait for navigation
    await page.waitForURL("**/app/config/pipelines/suggest_layer_definition", {
      timeout: 10000,
    });
    await page.waitForLoadState("networkidle");
    ...
    expect(page.url()).toContain(
      "/app/config/pipelines/suggest_layer_definition",
    );

    // Verify page title
    await expect(
      page.getByText("Layer Definitions Pipeline Flavors"),
  ...
  test("should list pipeline flavors for a type", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="pipeline-flavors-list"]'),
    ).toBeVisible({ timeout: 10000 });

    // Verify create button is visible
    await expect(
      page.locator('[data-testid="pipeline-flavor-create-button"]'),
    ).toBeVisible();

    // Verify flavors container is visible
    await expect(
      page.locator('[data-testid="pipeline-flavors-container"]'),
    ).toBeVisible();

    // Verify at least the default flavor exists using test ID
    await expect(
      page.locator('[data-testid="pipeline-flavor-title-default"]'),
    ).toBeVisible();
  });
  ...
  test("should display flavor details in list", async ({ page }) => {
    ...
    const flavorsContainer = page.locator(
      '[data-testid="pipeline-flavors-container"]',
    );
  ...
  test("should navigate to create new flavor page", async ({ page }) => {
    ...
    const createButton = page.locator(
      '[data-testid="pipeline-flavor-create-button"]',
    );
    ...
    await page.waitForURL(`**/app/config/pipelines/${pipelineType}/create`, {
      timeout: 10000,
    });
    await page.waitForLoadState("networkidle");
    expect(page.url()).toContain(
    ...
    await expect(page.getByText(/Create New.*Layer.*Flavor/i)).toBeVisible({
      timeout: 10000,
    });
  });
  ...
  test("should not allow editing default flavor", async ({ page }) => {
    ...
    const defaultFlavorCard = page.locator(
      '[data-testid="pipeline-flavor-card-default"]',
    );
    await expect(defaultFlavorCard).toBeVisible();

    // Verify edit button is disabled for default flavor
    const editButton = page.locator(
      '[data-testid="pipeline-flavor-edit-button-default"]',
    );
    await expect(editButton).toBeDisabled();

    // Verify delete button is disabled for default flavor
    const deleteButton = page.locator(
      '[data-testid="pipeline-flavor-delete-button-default"]',
    );
    await expect(deleteButton).toBeDisabled();
  });

  test("should navigate to edit flavor page for custom flavors", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator(`[data-testid="pipeline-flavor-card-${flavorId}"]`),
    ).toBeVisible({ timeout: 10000 });

    // Click edit button
    const editButton = page.locator(
      `[data-testid="pipeline-flavor-edit-button-${flavorId}"]`,
    );
  ...
  test("should navigate to test flavor page", async ({ page }) => {
    ...
    if (flavorId) {
      ...
      const testButton = page.locator(
        `[data-testid="pipeline-flavor-test-button-${flavorId}"]`,
      );
      ...
      expect(page.url()).toContain(
        `/app/config/pipelines/${pipelineType}/test/`,
      );
      await expect(page.getByText("Flavor Tester")).toBeVisible({
        timeout: 5000,
      });
    }
  });
  ...
  test("should delete a custom flavor", async ({ page }) => {
    ...
    const flavorCard = page.locator(
      `[data-testid="pipeline-flavor-card-${flavorId}"]`,
    );
    ...
    // Click delete button
    const deleteButton = page.locator(
      `[data-testid="pipeline-flavor-delete-button-${flavorId}"]`,
    );
    ...
    const deleteModal = page.locator(
      '[data-testid="pipeline-flavor-delete-modal"]',
    );
  ...
  test("should navigate between different pipeline types", async ({ page }) => {
    ...
    await page.waitForURL("**/app/config/pipelines/suggest_domain_definition", {
      timeout: 10000,
    });
    await expect(
      page.getByText("Domain Definitions Pipeline Flavors"),
    ).toBeVisible({ timeout: 10000 });
    ...
    await page.waitForURL("**/app/config/pipelines/suggest_term_definition", {
      timeout: 10000,
    });
    await expect(
      page.getByText("Term Definitions Pipeline Flavors"),
    ).toBeVisible({ timeout: 10000 });
  });
  ...
  test("should display flavor enabled/disabled status", async ({ page }) => {
    ...
    const statusElement = page.locator(
      `[data-testid="pipeline-flavor-status-${flavorId}"]`,
    );
  ...
  test("should handle multiple custom flavors", async ({ page }) => {
    ...
    await expect(
      flavorsContainer.getByText(/Provider: openai/).first(),
    ).toBeVisible();
    await expect(
      flavorsContainer.getByText(/Provider: anthropic/).first(),
    ).toBeVisible();
  });
});
