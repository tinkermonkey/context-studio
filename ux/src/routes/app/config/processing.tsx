...
const NLP_MODELS = [
  { value: "en_core_web_sm", label: "English Small (en_core_web_sm)", size: "15MB" },
  { value: "en_core_web_md", label: "English Medium (en_core_web_md)", size: "40MB" },
  { value: "en_core_web_lg", label: "English Large (en_core_web_lg)", size: "750MB" },
  { value: "en_core_web_trf", label: "English Transformer (en_core_web_trf)", size: "560MB" }
];
    ...
    <div className="p-6">
      ...
      <div className="mb-6">
        <p className="text-gray-600">
          Configure NLP processing and pipeline defaults for knowledge graph operations.
        </p>
      </div>
...
function NLPConfigurationSection({
  ...
  const currentModel = NLP_MODELS.find(m => m.value === config.model_name) || NLP_MODELS[0];
    ...
    <div className="space-y-6">
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">
              Currently using: <span className="font-medium">{currentModel.label}</span> ({currentModel.size})
            </p>
          </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600">Automatically download missing spaCy models</p>
            </div>
            <Checkbox
              id="auto_download"
              checked={config.auto_download_models ?? true}
              onChange={(e) => onUpdate('auto_download_models', e.target.checked)}
              disabled={isUpdating}
            />
          </div>
          ...
          <div>
            ...
            <TextInput
              ...
              onChange={(e) => onUpdate('download_timeout', parseInt(e.target.value))}
              disabled={isUpdating}
            />
          </div>
        </div>
      ...
      <Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <Label htmlFor="max_text_length">Max Text Length (characters)</Label>
            <TextInput
              id="max_text_length"
              type="number"
              value={config.max_text_length || 512}
              onChange={(e) => onUpdate('max_text_length', parseInt(e.target.value))}
              disabled={isUpdating}
            />
            <p className="text-sm text-gray-600 mt-1">
          ...
          <div>
            ...
            <TextInput
              ...
              onChange={(e) => onUpdate('request_timeout', parseInt(e.target.value))}
              disabled={isUpdating}
            />
          </div>
        </div>
        ...
        <div className="mt-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600">Skip processing nodes with empty text</p>
            </div>
            <Checkbox
              id="filter_missing_text"
              checked={config.filter_missing_text ?? true}
              onChange={(e) => onUpdate('filter_missing_text', e.target.checked)}
              disabled={isUpdating}
            />
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div>
            ...
            <TextInput
              ...
              onChange={(e) => onUpdate('edge_weight_filter', parseFloat(e.target.value))}
              disabled={isUpdating}
            />
            <p className="text-sm text-gray-600 mt-1">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600 mt-2">
              {enabledRelations.length} of {CONCEPTNET_RELATIONS.length} relations enabled
            </p>
          </div>
        </div>
      </Card>
...
function PipelineDefaultsSection({
  ...
  const currentModel = enabledModels.find(m => m.model_name === config.model_name);
  ...
  return (
    <div className="space-y-6">
      <Alert color="info">
        ...
        <span className="ml-2">
          These settings provide defaults for new pipeline flavors. Individual flavors can override these settings.
        </span>
      </Alert>
          ...
          <div>
            ...
            {currentModel && (
              <div className="flex items-center gap-2 mt-2">
                <Badge color="info" size="sm">{currentModel.provider_type}</Badge>
                <Badge color={currentModel.cost_tier === 'low' ? 'success' : currentModel.cost_tier === 'medium' ? 'warning' : 'failure'} size="sm">
      ...
      <Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            <Label htmlFor="max_text_length">Max Input Length (characters)</Label>
            <TextInput
              id="max_text_length"
              type="number"
          ...
          <div>
            ...
            <TextInput
              ...
              onChange={(e) => onUpdate('retry_attempts', parseInt(e.target.value))}
              disabled={isUpdating}
            />
          </div>
        </div>
      ...
      {enabledModels.length === 0 && (
        <Alert color="warning">
          ...
          <span className="ml-2">
            No LLM models are currently enabled. Visit the LLM Models configuration to enable models.
          </span>
        </Alert>
      )}
    </div>
