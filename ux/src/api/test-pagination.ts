...
export async function testPaginationFeatures() {
  try {
    ...
    const layersResponse: PaginatedResponse<LayerOut> =
      await layerService.listPageWithMetadata({
        skip: 0,
        limit: 10,
        sort: "title",
      });
    ...
    console.log("Layers pagination response:", {
      ...
      hasMorePages:
        layersResponse.skip + layersResponse.limit < layersResponse.total,
    });

    // Test domains pagination with metadata
    const domainsResponse: PaginatedResponse<DomainOut> =
      await domainService.listPageWithMetadata({
        skip: 0,
        limit: 10,
        sort: "title",
      });
    ...
    console.log("Domains pagination response:", {
      ...
      hasMorePages:
        domainsResponse.skip + domainsResponse.limit < domainsResponse.total,
    });

    // Test terms pagination with metadata
    const termsResponse: PaginatedResponse<TermOut> =
      await termService.listPageWithMetadata({
        skip: 0,
        limit: 10,
        sort: "title",
      });
    ...
    console.log("Terms pagination response:", {
      ...
      hasMorePages:
        termsResponse.skip + termsResponse.limit < termsResponse.total,
    });
