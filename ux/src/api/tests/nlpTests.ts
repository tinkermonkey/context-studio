...
export const testNLPFeatures = async () => {
  const testText =
    "Barack Obama, the former President of the United States, visited Paris, France last year.";
  ...
  try {
    ...
    result.tokens?.forEach((token, index) => {
      if (index < 5) {
        ...
        if (token.wordnet) {
          console.log(
            `     - WordNet synsets: ${token.wordnet.synsets?.length || 0}`,
          );
          console.log(
            `     - WordNet definitions: ${token.wordnet.definitions?.length || 0}`,
          );
        }

        if (token.concepcy) {
          console.log(
            `     - Concepcy score: ${token.concepcy.score || "N/A"}`,
          );
          console.log(
