...
import { nlpService, ProxyStatusResponse, ProxyMonitoringResponse, ProxyConfiguration, ProxyConfigureResponse } from '@/api/services/nlp';
...
export const useConfigureNLPProxy = (
  options?: UseMutationOptions<ProxyConfigureResponse, Error, ProxyConfiguration>
) => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (config: ProxyConfiguration) => nlpService.configureProxy(config),
    onSuccess: () => {
