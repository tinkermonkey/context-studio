...
type UpdateDatasetDirectoryRequest = components['schemas']['UpdateDatasetDirectoryRequest'];
...
export const useDeleteDataset = (
  ...
  return useMutation({
    ...
    onSuccess: (_, deletedId) => {
      ...
      queryClient.removeQueries({ queryKey: createQueryKey(QUERY_KEYS.DATASETS, deletedId) });
      // Invalidate datasets list
      queryClient.invalidateQueries({ queryKey: createQueryKey(QUERY_KEYS.DATASETS) });
      // Invalidate active dataset in case it was the deleted one
      queryClient.invalidateQueries({ queryKey: createQueryKey(QUERY_KEYS.DATASETS, 'active') });
    },
    ...options,
  });
};
...
export const useActivateDataset = (
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: createQueryKey(QUERY_KEYS.LAYERS) });
      queryClient.invalidateQueries({ queryKey: createQueryKey(QUERY_KEYS.DOMAINS) });
      queryClient.invalidateQueries({ queryKey: createQueryKey(QUERY_KEYS.TERMS) });
      queryClient.invalidateQueries({ queryKey: createQueryKey(QUERY_KEYS.RELATIONSHIPS) });
      queryClient.invalidateQueries({ queryKey: createQueryKey(QUERY_KEYS.GRAPH) });
    },
    ...options,
  });
};
...
export const useForgetDataset = (
  ...
  return useMutation({
    ...
    onSuccess: (_, forgottenId) => {
      // Remove the specific dataset from cache
      queryClient.removeQueries({ queryKey: createQueryKey(QUERY_KEYS.DATASETS, forgottenId) });
      // Invalidate datasets list
      queryClient.invalidateQueries({ queryKey: createQueryKey(QUERY_KEYS.DATASETS) });
...
export const useAddExistingDataset = (
  options?: UseMutationOptions<DatasetResponse, Error, AddExistingDatasetRequest>
) => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (data: AddExistingDatasetRequest) => datasetService.addExisting(data),
    onSuccess: () => {
      // Invalidate and refetch datasets list
      queryClient.invalidateQueries({ queryKey: createQueryKey(QUERY_KEYS.DATASETS) });
    },
...
export const useUpdateDatasetsDirectory = (
  options?: UseMutationOptions<{ success: boolean; message: string }, Error, UpdateDatasetDirectoryRequest>
) => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (data: UpdateDatasetDirectoryRequest) => datasetService.updateDirectory(data),
    onSuccess: () => {
      // Invalidate directory info
      queryClient.invalidateQueries({ queryKey: createQueryKey(QUERY_KEYS.DATASETS, 'directory') });
