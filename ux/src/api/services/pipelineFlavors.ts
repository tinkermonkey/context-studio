...
export class PipelineFlavorService extends BaseService {
  ...
  async list(params?: ListFlavorsParams): Promise<PipelineFlavorListResponse> {
    return this.withErrorContext(
      () =>
        this.getResource<PipelineFlavorListResponse>(
  ...
  async create(data: CreatePipelineFlavorRequest): Promise<PipelineFlavor> {
    return this.withErrorContext(() => {
      ...
      this.sanitizeString(
        (data.llm_provider as unknown as string) || "",
        "LLM provider",
        100,
      );
      this.sanitizeString(
        (data.llm_model as unknown as string) || "",
        "LLM model",
        255,
      );
      this.sanitizeString(
        (data.system_prompt as unknown as string) || "",
        "System prompt",
        10000,
      );
      this.sanitizeString(
        (data.user_prompt as unknown as string) || "",
        "User prompt",
        10000,
      );
  ...
  async get(flavorId: string): Promise<PipelineFlavor> {
    return this.withErrorContext(() => {
      ...
      return this.getResource<PipelineFlavor>(
        `${ENDPOINTS.PIPELINE_FLAVORS}/${flavorId}`,
      );
    }, "get pipeline flavor");
  }
