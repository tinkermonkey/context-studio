...
describe("FilterBar", () => {
  ...
  describe("search input", () => {
    ...
    it("renders custom placeholder when provided", () => {
      render(<FilterBar searchValue="" onSearchChange={vi.fn()} placeholder="Search by name..." />);
      expect(screen.getByPlaceholderText("Search by name...")).toBeInTheDocument();
    });

    it("displays current search value", () => {
      render(<FilterBar searchValue="test search" onSearchChange={vi.fn()} />);
  ...
  describe("filter chips", () => {
    it("does not render chips container when no chips provided", () => {
      ...
      const chipsContainer = container.querySelector('div[style*="flex-wrap"]');
    ...
    it("renders chip elements when filterChips provided", () => {
      ...
      const { container } = render(
        <FilterBar searchValue="" onSearchChange={vi.fn()} filterChips={chips} />,
      );
    ...
    it("displays chip labels", () => {
      ...
      render(<FilterBar searchValue="" onSearchChange={vi.fn()} filterChips={chips} />);
    ...
    it("calls onRemove callback when remove button is clicked", async () => {
      ...
      const chips = [{ label: "Tag 1", value: "tag1", onRemove }];
      const user = userEvent.setup();
      render(<FilterBar searchValue="" onSearchChange={vi.fn()} filterChips={chips} />);
  ...
  describe("complete structure", () => {
    it("renders search input and chips together", () => {
      const chips = [{ label: "Important", value: "important", onRemove: vi.fn() }];
      render(<FilterBar searchValue="search term" onSearchChange={vi.fn()} filterChips={chips} />);
      expect(screen.getByTestId("schema-search-input")).toBeInTheDocument();
