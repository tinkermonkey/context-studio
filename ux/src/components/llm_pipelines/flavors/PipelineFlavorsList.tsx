...
import type { PipelineType, PipelineFlavor } from "@/api/services/pipelineFlavors";

const PIPELINE_TYPES: { value: PipelineType; label: string; description: string }[] = [
  {
...
export const PipelineFlavorsList: React.FC = () => {
  const [selectedPipeline, setSelectedPipeline] = useState<PipelineType>("suggest_layer_definition");
  const [editingFlavor, setEditingFlavor] = useState<PipelineFlavor | null>(null);
  const [testingFlavor, setTestingFlavor] = useState<PipelineFlavor | null>(null);
  const [showCreateForm, setShowCreateForm] = useState(false);

  const { data: flavorsResponse, isLoading, error } = usePipelineFlavors(
  ...
  if (error) {
    return (
      <Alert color="failure">
        <span className="font-medium">Error loading pipeline flavors:</span> {error.message}
      </Alert>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <div className="mt-3 p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
          <p className="text-sm text-gray-600 dark:text-gray-300">
            {PIPELINE_TYPES.find(p => p.value === selectedPipeline)?.description}
          </p>
        </div>
      </Card>
      ...
      <Card>
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
              {PIPELINE_TYPES.find(p => p.value === selectedPipeline)?.label} Flavors
            </h3>
            <p className="text-sm text-gray-500 dark:text-gray-400">
        ...
        {flavors.length === 0 ? (
          <div className="text-center py-8 text-gray-500 dark:text-gray-400">
            ...
            <h4 className="text-lg font-medium mb-2">No flavors configured</h4>
          ...
          <div className="space-y-4">
            {flavors.map((flavor) => (
              <div key={flavor.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="font-medium text-gray-900 dark:text-white">{flavor.title}</h4>
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                      Provider: {flavor.llm_provider} | Model: {flavor.llm_model} | Version: {flavor.version}
                    </p>
                    <p className="text-sm text-gray-600 dark:text-gray-300 mt-1">
                      {flavor.enabled ? "Enabled" : "Disabled"} | Created: {new Date(flavor.date_created).toLocaleDateString()}
                    </p>
                  </div>
                  <div className="flex gap-2">
                    <Button
                      ...
                      title={flavor.title === "Default" ? "The Default flavor cannot be edited or deleted." : undefined}
                    >
                      Edit
                    </Button>
                    <Button size="sm" color="blue" onClick={() => handleTest(flavor)}>
                      Test
                    </Button>
                  </div>
                </div>
