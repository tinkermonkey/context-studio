...
export const SimpleDefinitionExample: React.FC = () => {
  ...
  const { data: definition, isLoading, error } = useSimpleDefinition(
...
export const DefinitionWithDomainExample: React.FC = () => {
  ...
  const [domainDef, setDomainDef] = useState('A subset of artificial intelligence');

  const { data: definition, isLoading, error } = useDefinitionWithDomain(
    term.trim().length > 0 ? term : null,
...
export const ComprehensiveDefinitionExample: React.FC = () => {
  ...
  return (
    <div>
      ...
      <button 
        onClick={handleGenerate}
        disabled={generateDefinition.isPending}
      >
        {generateDefinition.isPending ? 'Generating...' : 'Generate Comprehensive Definition'}
      </button>
...
export const LLMAPIExamples: React.FC = () => {
  ...
  const examples = {
    ...
    mutation: { component: MutationDefinitionExample, title: 'Manual Generation' },
    comprehensive: { component: ComprehensiveDefinitionExample, title: 'Comprehensive' },
  };
