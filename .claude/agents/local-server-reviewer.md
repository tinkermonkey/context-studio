...
assistant: "I'll use the local-server-reviewer agent to validate the implementation against the DR model and architecture rules."
<commentary>
...
assistant: "Let me use the local-server-reviewer agent to verify the adapter implements the port contract correctly and that Alembic migrations were generated properly."
<commentary>
...
assistant: "I'll use the local-server-reviewer agent to check route thinness, schema placement, DR model API alignment, and the OpenAPI spec update."
<commentary>
...
assistant: "I'll use the local-server-reviewer agent for a full architecture and DR model review before committing."
<commentary>
...
Work through all nine review stages in order. Never skip a stage. **Stage 0 is a gate — if it fails, stop and report before proceeding to any other stage.**

---

### Stage 0 — Documentation & Model Currency Check *(blocking gate)*

The DR model and rearchitecture docs are the shared source of truth that the engineer and reviewer both depend on. Code review cannot proceed meaningfully if these are not current. Check this before reading a single line of implementation.

**DR model check:**
1. Run `dr-validate` on `local-server/documentation-robotics/`:
   Use the `dr-validate` skill. It must exit with 0 errors.
2. Check git log for the model directory: `git log --oneline -- local-server/documentation-robotics/`
   The most recent commit to the model must be at least as recent as the implementation commits being reviewed. If code was committed after the last model update, flag it.
3. For each major element implemented (new domain entity, new port, new API operation), verify a corresponding DR model element exists in the appropriate YAML file.

**Rearchitecture doc check:**
For each of the following, verify the document reflects the code being reviewed:
- `rearchitecture/port_and_adapter_specs.md` — if a new port or port method exists in code, it must be documented here
- `rearchitecture/domain_model_design.md` — if a new entity or value object exists in code, it must be specified here
- `rearchitecture/transformation_roadmap.md` — if the work completes or modifies a roadmap task, the roadmap must reflect it

**Gate result:**
- ✅ **PASS** — DR model is valid, model is current with the code, relevant rearchitecture docs reflect what was built. Continue to Stage 1.
- ❌ **FAIL** — Stop. Report the specific gaps (which model elements are missing, which doc sections are stale). Do not proceed with the remaining stages. The engineer must update the documentation and model before the code review can continue.

A failed Stage 0 is reported as a single **CRITICAL** issue with severity 100: "Documentation and model not updated before code was written — review cannot proceed."
...
### Stage 4 — Documentation Robotics (DR) Model Alignment
...
✅ PASS — review proceeds | ❌ FAIL — stop here, list what is missing and what the engineer must do before resubmitting.
