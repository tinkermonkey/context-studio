...
You are running the Heimdall visual conformance fix loop for Context Studio. Your job is to make the live implementation _look correct_ by comparing it against the prototype (`ux/design/Context Studio.html`). The exit criterion is that a visual review agent returns all-PASS across every page — not that a set of DOM selectors matches.
...
- `.claude/skills/heimdall/heimdall-primitives/SKILL.md` — Icon, Button, Chip, Badge, SegmentedControl
- `.claude/skills/heimdall/heimdall-navigation/SKILL.md` — NavItem, Sidebar, Topbar, TabBar
- `.claude/skills/heimdall/heimdall-page-patterns/SKILL.md` — PageHeader, FilterBar, ActivityTimeline, PipelineCard, QuickAccessGrid, ConfigTile
- `.claude/skills/heimdall/heimdall-data-display/SKILL.md` — StatTile, StatGrid, Table, KVGrid, InspectorPanel
- `.claude/skills/heimdall/heimdall-charts/SKILL.md` — Sparkline, LineChart, BarChart and other chart components
