...
Below is a non-react example of the sub-graph layout

[Source](https://g6.antv.antgroup.com/en/examples/scene-case/default/#sub-graph)
...
class SubGraphNode extends HTML {
  ...
  drawGraphNode(data) {
    ...
    const subGraph = new Graph({
      ...
      behaviors: [
        "zoom-canvas",
        { type: "drag-canvas", enable: (event) => event.shiftKey === true },
      ],
      autoFit: "view",
    });
...
const getSize = (d) => {
  const data = d.data;
  if (data.type === "card")
    return data.status === "expanded"
      ? [200, 100 * data.children.length]
      : [200, 100];
  else return [200, 200];
};
