...
class ChangeEventHandler:
    ...
    def create_event(self, event_type: str, record_type: RecordType, record_id: Optional[str] = None, 
        ...
        try:
            ...
            logger.debug(f"Created ChangeEvent: {event_type} {record_type.value} (id: {event.id}, record_id: {record_id})")
            return event
    ...
    def fire_change_event(self, event_type: str, record_type: RecordType, record_id: Optional[str] = None, 
                         old_data: Optional[Dict[str, Any]] = None, 
                         new_data: Optional[Dict[str, Any]] = None) -> ChangeEvent:
        """
    ...
    def fire_created_event(self, record_type: RecordType, record_id: str, record_data: Dict[str, Any]) -> ChangeEvent:
        ...
        Returns:
            The created ChangeEvent
        """
        return self.create_event('create', record_type, record_id=record_id, old_data=None, new_data=record_data)

    def fire_updated_event(self, record_type: RecordType, record_id: str, old_data: Dict[str, Any], new_data: Dict[str, Any]) -> ChangeEvent:
        """
        ...
        Returns:
            The created ChangeEvent
        """
        return self.create_event('update', record_type, record_id=record_id, old_data=old_data, new_data=new_data)

    def fire_deleted_event(self, record_type: RecordType, record_id: str, record_data: Dict[str, Any]) -> ChangeEvent:
        """
        ...
        Returns:
            The created ChangeEvent
        """
        return self.create_event('delete', record_type, record_id=record_id, old_data=record_data, new_data=None)
    ...
    def fire_structure_node_link_created_event(self, link_id: str, link_data: Dict[str, Any]) -> ChangeEvent:
        """Convenience method for structure node link creation events."""
        return self.fire_created_event(RecordType.STRUCTURE_NODE_LINK, link_id, link_data)
    ...
    def get_events_for_record(self, record_id: str, limit: int = 100) -> List[ChangeEvent]:
        """
    ...
    def mark_event_processed(self, event_id: int) -> bool:
        ...
        """
        try:
            event = self.db.query(ChangeEvent).filter(ChangeEvent.id == event_id).first()
            if event:
    ...
    def get_event_stats(self) -> Dict[str, Any]:
        ...
        processed_events = self.db.query(ChangeEvent).filter(ChangeEvent.processed == True).count()
        unprocessed_events = total_events - processed_events
        
        # Events by type
        event_types = {}
        for event_type, count in self.db.query(ChangeEvent.event_type, func.count(ChangeEvent.id)).group_by(ChangeEvent.event_type).all():
            event_types[event_type] = count
        
        # Events by record type
        record_types = {}
        for record_type, count in self.db.query(ChangeEvent.record_type, func.count(ChangeEvent.id)).group_by(ChangeEvent.record_type).all():
            record_types[record_type.value if hasattr(record_type, 'value') else record_type] = count
