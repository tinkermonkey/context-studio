...
class NodeService:
    ...
    def create_node(self, node_data: Dict[str, Any]) -> StructureNode:
        ...
        try:
            ...
            logger.info(f"Successfully created structure_node: {structure_node.id} ({structure_node.node_type})")
            ...
            self.event_handler.fire_created_event(RecordType.STRUCTURE_NODE, str(structure_node.id), node_dict)
    ...
    def update_node(self, node_id: str, node_data: Dict[str, Any]) -> StructureNode:
        ...
        structure_node = self.db.query(StructureNode).filter(StructureNode.id == node_id).first()
        ...
        if 'parent_node_id' in node_data and node_data['parent_node_id'] != structure_node.parent_node_id:
        ...
        if 'definition' in node_data:
            ...
            try:
                if node_data['definition']:
                    structure_node.definition_embedding = generate_embedding(node_data['definition'])
                else:
        ...
        if 'structural_predicate_id' in node_data:
            structure_node.structural_predicate_id = node_data['structural_predicate_id']
        ...
        try:
            ...
            from database.enums import RecordType
            self.event_handler.fire_updated_event(RecordType.STRUCTURE_NODE, str(structure_node.id), old_node_data, new_node_data)
    ...
    def delete_node(self, node_id: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Successfully deleted structure_node {node_id} and {total_children} children")
            
            # Fire structure_node deleted event using new ChangeEventHandler
            from database.enums import RecordType
            self.event_handler.fire_deleted_event(RecordType.STRUCTURE_NODE, node_id, node_data)
    ...
    def list_nodes(self, node_type: Optional[NodeType] = None, 
                   ...
                   skip: int = 0, limit: int = 50) -> List[StructureNode]:
        """
    ...
    def get_nodes(self, node_type: Optional[NodeType] = None,
                  ...
                  offset: Optional[int] = None) -> List[StructureNode]:
        """
    ...
    def get_node_children(self, node_id: str, recursive: bool = False) -> List[StructureNode]:
        """
        ...
        """
        if not recursive:
            return self.db.query(StructureNode).filter(StructureNode.parent_node_id == node_id).order_by(StructureNode.title).all()
        
        # For recursive, we need to traverse the tree
        all_children = []
        direct_children = self.db.query(StructureNode).filter(StructureNode.parent_node_id == node_id).all()
    ...
    def _validate_node_update(self, structure_node: StructureNode, node_data: Dict[str, Any]):
    ...
    def _validate_layer_update(self, structure_node: StructureNode, node_data: Dict[str, Any]):
    ...
    def _validate_domain_update(self, structure_node: StructureNode, node_data: Dict[str, Any]):
        ...
        check_title_uniqueness = (
            ...
            'parent_node_id' in node_data and node_data['parent_node_id'] != structure_node.parent_node_id
        )
    ...
    def _validate_term_update(self, structure_node: StructureNode, node_data: Dict[str, Any]):
        ...
        if check_title_uniqueness:
            title = node_data.get('title', structure_node.title)
            if self._check_title_uniqueness_in_domain(domain.id, title, exclude_id=structure_node.id):
                raise ValueError("Term title must be unique within domain")
    ...
    def _get_domain_ancestor(self, structure_node: StructureNode) -> Optional[StructureNode]:
        ...
        while current:
            ...
            if current.parent_node_id:
                current = self.db.query(StructureNode).filter(StructureNode.id == current.parent_node_id).first()
            else:
                break
        return None
    ...
    def _get_all_terms_in_domain(self, domain_id: str) -> List[str]:
        ...
        direct_terms = self.db.query(StructureNode.id).filter(
            StructureNode.node_type == NodeType.TERM,
    ...
    def _get_all_terms_under_term(self, term_id: str) -> List[str]:
        ...
        """
        child_terms = self.db.query(StructureNode.id).filter(
            StructureNode.node_type == NodeType.TERM,
    ...
    def _node_to_dict(self, structure_node: StructureNode) -> Dict[str, Any]:
        ...
        return {
            ...
            'node_type': structure_node.node_type.value if isinstance(structure_node.node_type, NodeType) else structure_node.node_type,
    ...
    def move_nodes(self, node_ids: List[str], target_parent_id: Optional[str] = None, 
                   move_children: bool = True, handle_conflicts: str = "warn") -> Dict[str, Any]:
        """
        ...
        Raises:
            ValueError: If validation fails
        """
        logger.info(f"Moving {len(node_ids)} structure_nodes to parent {target_parent_id}")
            ...
            for node_id in node_ids:
                try:
                    result = self._move_single_node(
                        node_id, target_parent_id, target_parent, 
                        move_children, handle_conflicts
                    )
    ...
    def _move_single_node(self, node_id: str, target_parent_id: Optional[str], 
                         target_parent: Optional[StructureNode], move_children: bool,
                         handle_conflicts: str) -> Dict[str, Any]:
        """
        ...
        # Check for title conflicts
        conflict_warning = self._check_move_conflicts(structure_node, target_parent_id, handle_conflicts)
        if conflict_warning:
        ...
        logger.info(f"Moved structure_node {node_id} from parent {old_parent_id} to {target_parent_id}")
    ...
    def _validate_node_move(self, structure_node: StructureNode, target_parent: Optional[StructureNode]):
        """
        ...
        if target_parent and self._would_create_cycle(structure_node.id, target_parent.id):
            raise ValueError("Move would create circular reference")

    def _check_move_conflicts(self, structure_node: StructureNode, target_parent_id: Optional[str], 
                             handle_conflicts: str) -> Optional[str]:
        """
        ...
        existing = self.db.query(StructureNode).filter(
            and_(
                ...
                StructureNode.id != structure_node.id  # Exclude the structure_node being moved
            )
        ).first()
        ...
        if existing:
            ...
            if handle_conflicts == "error":
                ...
                while self.db.query(StructureNode).filter(
                    and_(
                        StructureNode.parent_node_id == target_parent_id,
