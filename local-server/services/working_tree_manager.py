...
class WorkingTreeManager:
    ...
    def initialize_entity_in_working_tree(
        ...
        if existing:
            logger.warning(f"Entity {entity_type}:{entity_id} already exists in working tree")
            return existing
        
        # Verify the version exists
        version_exists = self.db.execute(text("""
            SELECT 1 FROM entity_versions WHERE id = :version_id
        """), {"version_id": initial_version_id}).fetchone()
        ...
        try:
            ...
            self.db.rollback()
            logger.error(f"Failed to initialize {entity_type}:{entity_id} in working tree: {e}")
            raise RuntimeError(f"Database error during working tree initialization: {e}") from e
    ...
    def update_current_version(
        ...
        version_exists = self.db.execute(text("""
            SELECT 1 FROM entity_versions WHERE id = :version_id
        """), {"version_id": new_version_id}).fetchone()
        ...
        try:
            ...
            self.db.rollback()
            logger.error(f"Failed to update current version for {entity_type}:{entity_id}: {e}")
    ...
    def get_working_tree_entry(self, entity_type: str, entity_id: str) -> Optional[WorkingTreeEntry]:
        """
        ...
        try:
            ...
            logger.error(f"Failed to retrieve working tree entry for {entity_type}:{entity_id}: {e}")
            raise RuntimeError(f"Database error during working tree retrieval: {e}") from e
    ...
    def get_working_changes(self) -> List[WorkingTreeEntry]:
        ...
        try:
            ...
            raise RuntimeError(f"Database error during working changes retrieval: {e}") from e
    ...
    def get_staged_entities(self) -> List[WorkingTreeEntry]:
        ...
        try:
            ...
            raise RuntimeError(f"Database error during staged entities retrieval: {e}") from e
    ...
    def get_working_tree_status(self) -> WorkingTreeStatus:
        ...
        try:
            ...
            logger.debug(f"Working tree status: {len(entries)} total, {modified_count} modified, {staged_count} staged")
            ...
            raise RuntimeError(f"Database error during working tree status retrieval: {e}") from e
