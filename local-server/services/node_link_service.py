...
class NodeLinkService:
    ...
    def create_link(self, link_data: Dict[str, Any]) -> StructureNodeLink:
        ...
        """
        logger.info(f"Creating structure_node link: {link_data.get('source_node_id')} -> {link_data.get('target_node_id')}")
        ...
        try:
            ...
            self.event_handler.fire_created_event(RecordType.STRUCTURE_NODE_LINK, str(link.id), link_dict)
    ...
    def update_link(self, link_id: str, link_data: Dict[str, Any]) -> StructureNodeLink:
        ...
        # Get existing link
        link = self.db.query(StructureNodeLink).filter(StructureNodeLink.id == link_id).first()
        if not link:
        ...
        try:
            ...
            from database.enums import RecordType
            self.event_handler.fire_updated_event(RecordType.STRUCTURE_NODE_LINK, str(link.id), old_link_data, new_link_data)
    ...
    def delete_link(self, link_id: str) -> bool:
        ...
        try:
            ...
            from database.enums import RecordType
            self.event_handler.fire_deleted_event(RecordType.STRUCTURE_NODE_LINK, link_id, link_data)
    ...
    def get_link(self, link_id: str) -> Optional[StructureNodeLink]:
        ...
        """
        return self.db.query(StructureNodeLink).filter(StructureNodeLink.id == link_id).first()
    
    def list_links(self, source_node_id: Optional[str] = None,
                   target_node_id: Optional[str] = None,
                   predicate: Optional[str] = None,
                   skip: int = 0, limit: int = 50) -> List[StructureNodeLink]:
        """
        ...
        return query.order_by(StructureNodeLink.created_at.desc()).offset(skip).limit(limit).all()
    ...
    def get_node_links(self, node_id: str, direction: str = "both") -> List[StructureNodeLink]:
        """
    ...
    def get_linked_nodes(self, node_id: str, direction: str = "both", 
                        node_type: Optional[NodeType] = None) -> List[StructureNode]:
        """
        ...
        query = self.db.query(StructureNode).filter(StructureNode.id.in_(linked_node_ids))
    ...
    def validate_link_compatibility(self, source_node_id: str, target_node_id: str) -> bool:
        ...
        # Get structure_nodes
        source = self.db.query(StructureNode).filter(StructureNode.id == source_node_id).first()
        target = self.db.query(StructureNode).filter(StructureNode.id == target_node_id).first()
    ...
    def get_link_statistics(self) -> Dict[str, Any]:
        ...
        for (predicate,) in predicates:
            count = self.db.query(StructureNodeLink).filter(StructureNodeLink.predicate == predicate).count()
            predicate_stats[predicate] = count
        
        # Links by structure_node type (source)
        node_type_stats = {}
        for node_type in NodeType:
            count = (self.db.query(StructureNodeLink)
                    .join(StructureNode, StructureNodeLink.source_node_id == StructureNode.id)
