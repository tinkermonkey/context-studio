...
class ChangeAnalyticsEngine:
    ...
    def _create_local_views(self):
        ...
        try:
            ...
            if active_dataset:
                sqlite_path = dataset_manager.get_dataset_file_path(
                ...
                logger.info(
                    "Created local analytical views successfully using SQLite scan"
                )
            else:
        ...
        try:
            if self.s3_config and "bucket" in self.s3_config:
                ...
                if not impact_result.empty:
                    ...
                    base_analysis["impact_by_type"] = (
                        type_result.to_dict("records") if not type_result.empty else []
                    )
    ...
    def get_branch_analytics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not branch_result.empty:
                ...
                analytics["branch_statistics"] = self._get_fallback_branch_analytics()[
                    "branch_statistics"
                ]
            ...
            if not mr_result.empty:
                ...
                analytics["merge_request_statistics"] = (
                    self._get_fallback_branch_analytics()["merge_request_statistics"]
                )
    ...
    def get_comprehensive_change_trends(self, days: int = 90) -> Dict[str, Any]:
        ...
        try:
            if self.s3_config and "bucket" in self.s3_config:
                ...
                return {
                    "daily_trends": (
                        trends_result.to_dict("records")
                        if not trends_result.empty
                        else []
                    ),
                    "peak_hours": (
                        peak_result.to_dict("records") if not peak_result.empty else []
                    ),
    ...
    def get_system_performance_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return (
                performance_data
                if performance_data
                else self._get_fallback_performance_metrics()
            )
            ...
            if self.s3_config and "bucket" in self.s3_config:
                ...
                return {
                    "collaboration_networks": (
                        collaboration_result.to_dict("records")
                        if not collaboration_result.empty
                        else []
                    ),
                    "team_productivity": (
                        productivity_result.to_dict("records")
                        if not productivity_result.empty
                        else []
                    ),
                    "analysis_period_days": days,
                }
    ...
    def generate_executive_summary(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_changes_per_user = (
                user_activity["total_changes"].mean() if not user_activity.empty else 0
            )
            most_active_entity = (
                entity_hotspots.iloc[0]["entity_type"]
                if not entity_hotspots.empty
                else "N/A"
            )
            ...
            return {
                ...
                "collaboration_health": {
                    "total_branches": branch_analytics.get("branch_statistics", {}).get(
                        "total_branches", 0
                    ),
                    "merge_requests": branch_analytics.get(
                        "merge_request_statistics", {}
                    ).get("total_merge_requests", 0),
                ...
                "system_health": {
                    "high_severity_conflicts": conflict_metrics.get(
                        "high_severity_conflicts", 0
                    ),
                    "avg_resolution_time_hours": conflict_metrics.get(
                        "avg_resolution_time_hours", 0
                    ),
                    "sync_performance": performance_metrics.get("sync_performance", {}),
                },
                "top_entities": (
                    entity_hotspots.head(5).to_dict("records")
                    if not entity_hotspots.empty
                    else []
                ),
