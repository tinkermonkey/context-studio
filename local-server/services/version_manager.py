...
class VersionManager:
    ...
    def create_version(
        ...
        if parent_version_id:
            parent_exists = self.db.execute(text("""
                SELECT 1 FROM entity_versions WHERE id = :parent_id
            """), {"parent_id": parent_version_id}).fetchone()
        ...
        try:
            ...
            logger.info(f"Created version {next_version} for {entity_type}:{entity_id} (id: {version_id})")
    ...
    def get_entity_versions(self, entity_type: str, entity_id: str) -> List[EntityVersion]:
        ...
        try:
            ...
            logger.debug(f"Retrieved {len(versions)} versions for {entity_type}:{entity_id}")
            return versions
            
        except Exception as e:
            logger.error(f"Failed to retrieve versions for {entity_type}:{entity_id}: {e}")
    ...
    def get_version_by_number(
        ...
        """
        logger.debug(f"Retrieving version {version_number} for {entity_type}:{entity_id}")
        ...
        try:
            ...
            logger.debug(f"Version {version_number} not found for {entity_type}:{entity_id}")
            return None
            
        except Exception as e:
            logger.error(f"Failed to retrieve version {version_number} for {entity_type}:{entity_id}: {e}")
            raise RuntimeError(f"Database error during version retrieval: {e}") from e
    ...
    def get_current_version(self, entity_type: str, entity_id: str) -> Optional[EntityVersion]:
        ...
        try:
            ...
            logger.error(f"Failed to retrieve current version for {entity_type}:{entity_id}: {e}")
            raise RuntimeError(f"Database error during version retrieval: {e}") from e
    ...
    def rollback_to_version(
        ...
        Raises:
            ...
            RuntimeError: If database operation fails
        """
        logger.info(f"Rolling back {entity_type}:{entity_id} to version {target_version_number}")
        
        # Get target version
        target_version = self.get_version_by_number(entity_type, entity_id, target_version_number)
