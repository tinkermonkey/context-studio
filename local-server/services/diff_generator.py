...
class DiffGenerator:
    ...
    def __init__(self, version_manager: VersionManager, working_tree_manager: WorkingTreeManager):
        """
    ...
    def generate_version_diff(
        ...
        """
        logger.info(f"Generating version diff for {entity_type}:{entity_id} "
    ...
    def generate_working_diff(self, entity_type: str, entity_id: str) -> EntityDiff:
        ...
        working_entry = self.working_tree_manager.get_working_tree_entry(entity_type, entity_id)
        if not working_entry:
            raise ValueError(f"Entity {entity_type}:{entity_id} not found in working tree")
        
        # Get canonical version content
        canonical_version = self._get_version_by_id(working_entry.canonical_version_id)
        if not canonical_version:
            raise ValueError(f"Canonical version {working_entry.canonical_version_id} not found")
        
        # Get current working version content
        current_version = self._get_version_by_id(working_entry.current_version_id)
        if not current_version:
            raise ValueError(f"Current version {working_entry.current_version_id} not found")
    ...
    def generate_all_working_diffs(self) -> List[EntityDiff]:
        ...
        for entry in working_changes:
            try:
                ...
                logger.error(f"Failed to generate diff for {entry.entity_type}:{entry.entity_id}: {e}")
                continue
    ...
    def generate_commit_preview(self) -> List[EntityDiff]:
        ...
        for entry in staged_entities:
            try:
                ...
                logger.error(f"Failed to generate commit preview for {entry.entity_type}:{entry.entity_id}: {e}")
                continue
    ...
    def get_change_summary(self, entity_type: str, entity_id: str) -> DiffSummary:
        ...
        try:
            ...
            logger.error(f"Failed to get change summary for {entity_type}:{entity_id}: {e}")
            return DiffSummary()
    
    def format_diff_for_display(self, diff: EntityDiff, format_type: str = "summary") -> Dict[str, Any]:
        """
        ...
        if format_type == "summary":
            ...
            return {
                ...
                "before_version": diff.before_version.version_number if diff.before_version else None,
    ...
    def _get_version_by_id(self, version_id: str) -> Optional[EntityVersion]:
        ...
        try:
            result = self.version_manager.db.execute(text("""
    ...
    def _create_diff_summary(self, summary_dict: Dict[str, int]) -> DiffSummary:
        ...
        added = summary_dict.get('dictionary_item_added', 0) + summary_dict.get('iterable_item_added', 0)
        removed = summary_dict.get('dictionary_item_removed', 0) + summary_dict.get('iterable_item_removed', 0)
    ...
    def _format_diff_summary(self, diff: EntityDiff) -> Dict[str, Any]:
        ...
        return {
            ...
            "versions": {
                "before": diff.before_version.version_number if diff.before_version else None,
