...
class VersioningService:
    ...
    def approve_proposal(self, proposal_id: str) -> Proposal:
        ...
        if changeset is None:
            raise VersionNotFoundError(
                f"Changeset {proposal.changeset_id} for proposal {proposal_id} not"
                " found"
            )
    ...
    def detect_conflicts(
        self,
        proposal_id: str,
        resolutions: Optional[dict[str, dict[str, object]]] = None,
    ) -> ConflictReport:
        """
        ...
        _logger.info(
            "Conflict detection complete (proposal_id=%s, conflicts_found=%d,"
            " resolved=%d)",
            proposal_id,
    ...
    def auto_resolve(
        ...
        _logger.info(
            "Auto-resolved conflicts using %s strategy (proposal_id=%s,"
    ...
    def _create_merge_versions(
        ...
        if requested_ids != returned_ids:
            ...
            error_msg = f"Missing events for changeset {changeset.id}: {missing}"
            _logger.error(error_msg)
    ...
    def _create_entity_version(
        ...
        _logger.debug(
            "Built entity version (entity_id=%s, version=%d, state=%s,"
            " parent_version=%s)",
            entity_id,
            new_version_num,
            version_state.value,
    ...
    def merge_proposal(self, proposal_id: str) -> MergeResult:
        ...
        if report.has_conflicts and not report.all_resolved:
            error_msg = (
                f"Proposal {proposal_id} has {len(report.conflicts)} unresolved"
                " conflicts"
            )
            _logger.error(error_msg)
            raise ConflictResolutionError(error_msg)
    ...
    def pull_changes(self) -> SyncResult:
        ...
        for change_event in events:
            try:
                ...
                error_msg = (
                    f"Failed to record change for entity {change_event.entity_id}:"
                    f" {str(e)}"
                )
    ...
    def on_changeset_merged(self, event: ChangesetMerged) -> None:
        ...
        """
        _logger.info(
            "ChangesetMerged event handled (changeset_id=%s, proposal_id=%s,"
            " events_applied=%d)",
            event.changeset_id,
            event.proposal_id,
