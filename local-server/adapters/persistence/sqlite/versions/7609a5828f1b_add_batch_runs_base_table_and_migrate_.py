...
def upgrade() -> None:
    ...
    if insert_result.rowcount != import_run_count:
        raise Exception(
            f"Data migration integrity error: expected to insert {import_run_count}"
            f" rows, but inserted {insert_result.rowcount}"
        )
    ...
    if update_result.rowcount != events_with_import_run:
        raise Exception(
            "Data migration integrity error: expected to update"
            f" {events_with_import_run} rows, but updated {update_result.rowcount}"
        )
...
def downgrade() -> None:
    ...
    op.get_bind().execute(sa.text("""
        UPDATE import_runs
        SET affected_entity_ids = (
            SELECT affected_entity_ids FROM batch_runs
            WHERE batch_runs.id = import_runs.id
        )
        WHERE EXISTS (
            SELECT 1 FROM batch_runs WHERE batch_runs.id = import_runs.id
        )
    """))
