...
from api.models.change_events import ChangeEventOut, ChangeEventUpdate, RecordTypeEnum  # noqa: E501
...
def to_change_event_out(event: ChangeEvent) -> ChangeEventOut:
    ...
    return ChangeEventOut(
        ...
        record_type=event.record_type.value if hasattr(event.record_type, 'value') else str(event.record_type),  # noqa: E501
...
def list_change_events(
    ...
    limit: int = Query(50, ge=1, le=1000, description="Maximum number of events to return"),  # noqa: E501
    record_type: Optional[RecordTypeEnum] = Query(None, description="Filter by record type"),  # noqa: E501
    record_id: Optional[str] = Query(None, description="Filter by record ID"),
    event_type: Optional[str] = Query(None, description="Filter by event type (create, update, delete)"),  # noqa: E501
    processed: Optional[bool] = Query(None, description="Filter by processed status"),  # noqa: E501
    ...
    try:
        ...
        if record_type is not None:
            ...
            query = query.filter(ChangeEvent.record_type == RecordType(record_type.value))  # noqa: E501
        ...
        raise HTTPException(status_code=500, detail=f"Error listing change events: {str(e)}")  # noqa: E501
...
def get_change_event(
    ...
    """
    try:
        event = db.query(ChangeEvent).filter(ChangeEvent.id == event_id).first()  # noqa: E501

        if not event:
            raise HTTPException(status_code=404, detail=f"Change event {event_id} not found")  # noqa: E501
        ...
        logger.error(f"Error getting change event {event_id}: {e}", exc_info=True)  # noqa: E501
        raise HTTPException(status_code=500, detail=f"Error getting change event: {str(e)}")  # noqa: E501
...
def update_change_event(
    ...
    try:
        ...
        logger.error(f"Error updating change event {event_id}: {e}", exc_info=True)  # noqa: E501
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Error updating change event: {str(e)}")  # noqa: E501
