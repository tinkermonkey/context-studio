...
This module provides administrative endpoints to monitor service factory performance,
...
async def get_service_factory_stats(
    ...
    try:
        ...
        logger.info(f"Service factory stats requested - {stats['total_cache_entries']} entries")
        return stats
    except Exception as e:
        logger.error(f"Error getting service factory stats: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to get service factory stats: {str(e)}")
...
async def get_service_factory_health(
    ...
    try:
        ...
        if health['status'] != 'healthy':
            logger.warning(f"Service factory health issues detected: {health['issues']}")
        
        return health
    except Exception as e:
        logger.error(f"Error getting service factory health: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to get service factory health: {str(e)}")
...
async def clear_service_factory_cache(
    ...
    try:
        ...
        logger.info(f"Service factory cache cleared - removed {entries_before} entries")
        ...
        raise HTTPException(status_code=500, detail=f"Failed to clear service factory cache: {str(e)}")


@router.post("/cache/cleanup", summary="Force cleanup of expired cache entries")
async def cleanup_service_factory_cache(
    ...
    try:
        ...
        logger.info(f"Service factory cache cleanup completed - removed {expired_count} expired entries")
        ...
        raise HTTPException(status_code=500, detail=f"Failed to cleanup service factory cache: {str(e)}")


@router.get("/metrics/{service_type}", summary="Get metrics for specific service type")
async def get_service_type_metrics(
    ...
    try:
        ...
        logger.error(f"Error getting metrics for service type {service_type}: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to get metrics for service type: {str(e)}")
...
async def get_service_factory_dashboard(
    ...
    try:
        ...
        return {
            ...
            "performance": {
                "overall_hit_rate_percent": performance['overall_cache_hit_rate_percent'],
                "total_services_created": performance['total_services_created'],
