...
This module implements comprehensive analytics API endpoints for change analytics,
...
- GET /api/analytics/collaboration-metrics - Get collaboration effectiveness metrics
- GET /api/analytics/change-impact/{changeset_id} - Get impact analysis for specific changeset
...
- GET /api/analytics/collaboration-insights - Get advanced collaboration insights
...
def get_change_impact_analysis(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get change impact analysis: {str(e)}")
...
def get_change_trends(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get change trends: {str(e)}")
...
def get_collaboration_insights(
    ...
    try:
        insights = analytics_engine.get_advanced_collaboration_insights(days=days)
        return CollaborationInsightsOut.model_validate(insights)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to get collaboration insights: {str(e)}")
...
def get_conflict_resolution_metrics(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get conflict metrics: {str(e)}")
...
def get_analytics_health(
    ...
    try:
        ...
        s3_configured = bool(analytics_engine.s3_config and 'bucket' in analytics_engine.s3_config)
...
def export_analytics_csv(
    ...
    try:
        ...
        return StreamingResponse(
            ...
            media_type="text/csv",
            headers={"Content-Disposition": f"attachment; filename={report_type}_analytics_{days}days.csv"}
        )
...
def get_dashboard_metrics(
    ...
    try:
        ...
        change_summary = analytics_engine.get_change_summary(days=1)  # Last 24 hours
        user_activity = analytics_engine.get_user_activity_report(days=7)  # Last week
        ...
        return {
            ...
            "metrics": {
                ...
                "active_users_week": len(user_activity) if not user_activity.empty else 0,
                "unresolved_conflicts": (
                    conflict_metrics.get("total_conflicts", 0) - 
                    conflict_metrics.get("resolved_conflicts", 0)
                ),
                "avg_sync_time": performance_metrics.get("sync_performance", {}).get("avg_sync_time_minutes", 0),
                "system_status": "healthy" if analytics_engine.duckdb.connection else "degraded"
            }
        }
    except Exception as e:
