...
def get_change_summary(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get change summary: {str(e)}")  # noqa: E501


@router.get("/user-activity", response_model=List[UserActivityOut])
def get_user_activity_report(
    user_id: Optional[str] = Query(None, description="Specific user ID to analyze"),  # noqa: E501
    days: int = Query(30, ge=1, le=365, description="Number of days to analyze"),  # noqa: E501
    limit: int = Query(100, ge=1, le=1000, description="Maximum number of users to return"),  # noqa: E501
    analytics_engine=Depends(get_analytics_engine)
):
    ...
    try:
        ...
        return [UserActivityOut.model_validate(row.to_dict()) for _, row in limited_df.iterrows()]  # noqa: E501
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to get user activity: {str(e)}")  # noqa: E501


@router.get("/entity-hotspots", response_model=List[EntityHotspotOut])
def get_entity_hotspots(
    limit: int = Query(50, ge=1, le=500, description="Maximum number of hotspots to return"),  # noqa: E501
    analytics_engine=Depends(get_analytics_engine)
):
    ...
    try:
        ...
        return [EntityHotspotOut.model_validate(row.to_dict()) for _, row in hotspots_df.iterrows()]  # noqa: E501
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to get entity hotspots: {str(e)}")  # noqa: E501


@router.get("/collaboration-metrics", response_model=CollaborationMetricsOut)
def get_collaboration_metrics(
    days: int = Query(60, ge=1, le=365, description="Number of days to analyze"),  # noqa: E501
    analytics_engine=Depends(get_analytics_engine)
):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get collaboration metrics: {str(e)}")  # noqa: E501
...
def get_change_impact_analysis(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get change impact analysis: {str(e)}")  # noqa: E501


@router.get("/trends", response_model=TrendAnalysisOut)
def get_change_trends(
    days: int = Query(90, ge=1, le=365, description="Number of days to analyze"),  # noqa: E501
    analytics_engine=Depends(get_analytics_engine)
):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get change trends: {str(e)}")  # noqa: E501
...
def get_system_performance_metrics(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get performance metrics: {str(e)}")  # noqa: E501
...
def get_collaboration_insights(
    ...
    try:
        insights = analytics_engine.get_advanced_collaboration_insights(days=days)  # noqa: E501
        return CollaborationInsightsOut.model_validate(insights)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to get collaboration insights: {str(e)}")  # noqa: E501
...
def get_executive_summary(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to generate executive summary: {str(e)}")  # noqa: E501
...
def get_conflict_resolution_metrics(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get conflict metrics: {str(e)}")  # noqa: E501
...
def get_analytics_health(
    ...
    try:
        ...
        s3_configured = bool(analytics_engine.s3_config and 'bucket' in analytics_engine.s3_config)  # noqa: E501
        ...
        if duckdb_available:
            try:
                ...
                active_views = view_check_result.iloc[0]['view_count'] if not view_check_result.empty else 0  # noqa: E501
            except Exception:
                active_views = 0
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get analytics health: {str(e)}")  # noqa: E501
...
def export_analytics_csv(
    report_type: str = Path(..., description="Type of report to export (summary, user-activity, hotspots, trends)"),  # noqa: E501
    days: int = Query(30, ge=1, le=365, description="Number of days to analyze"),  # noqa: E501
    analytics_engine=Depends(get_analytics_engine)
):
    ...
    try:
        ...
        if report_type == "user-activity":
            ...
            trends = analytics_engine.get_comprehensive_change_trends(days=days)  # noqa: E501
            df = pd.DataFrame(trends.get("daily_trends", []))
        else:
            raise HTTPException(status_code=400, detail=f"Unsupported report type: {report_type}")  # noqa: E501
        ...
        return StreamingResponse(
            ...
            media_type="text/csv",
            headers={"Content-Disposition": f"attachment; filename={report_type}_analytics_{days}days.csv"}  # noqa: E501
        )
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to export analytics: {str(e)}")  # noqa: E501
...
def get_dashboard_metrics(
    refresh_interval: int = Query(300, ge=60, le=3600, description="Refresh interval in seconds"),  # noqa: E501
    analytics_engine=Depends(get_analytics_engine)
):
    """Get real-time metrics suitable for dashboard display."""
    try:
        # Get key metrics for dashboard
        change_summary = analytics_engine.get_change_summary(days=1)  # Last 24 hours  # noqa: E501
        user_activity = analytics_engine.get_user_activity_report(days=7)  # Last week  # noqa: E501
        ...
        return {
            ...
            "metrics": {
                ...
                "active_users_week": len(user_activity) if not user_activity.empty else 0,  # noqa: E501
                "unresolved_conflicts": (
                    conflict_metrics.get("total_conflicts", 0) -
                    conflict_metrics.get("resolved_conflicts", 0)
                ),
                "avg_sync_time": performance_metrics.get("sync_performance", {}).get("avg_sync_time_minutes", 0),  # noqa: E501
                "system_status": "healthy" if analytics_engine.duckdb.connection else "degraded"  # noqa: E501
            }
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to get dashboard metrics: {str(e)}")  # noqa: E501
