...
This document details the technical design for normalizing the database structure of `layers`, `domains`, and `terms` into a single `nodes` table, along with related system changes.
...
CREATE INDEX idx_node_events_processed ON node_events(processed);
```
...
def upgrade():
    """
    Migration 006: Normalize layers, domains, terms into nodes table
    """
    ...
    # when nodes are inserted with title_embedding and definition_embedding fields populated
...
def validate_migration():
    ...
    node_count = connection.execute("SELECT COUNT(*) FROM nodes").scalar()
    
    if node_count != total_original:
        raise MigrationError(f"Record count mismatch: {node_count} nodes vs {total_original} original records")
    
    # Count validation for node_links
    term_relationships_count = connection.execute("SELECT COUNT(*) FROM term_relationships").scalar()
    node_links_count = connection.execute("SELECT COUNT(*) FROM node_links").scalar()
    
    if node_links_count != term_relationships_count:
        raise MigrationError(f"Link count mismatch: {node_links_count} node_links vs {term_relationships_count} term_relationships")
    
    # Count validation for node_events  
    graph_events_count = connection.execute("SELECT COUNT(*) FROM graph_events").scalar()
    node_events_count = connection.execute("SELECT COUNT(*) FROM node_events").scalar()
    
    if node_events_count != graph_events_count:
        raise MigrationError(f"Event count mismatch: {node_events_count} node_events vs {graph_events_count} graph_events")
    ...
    if invalid_parents > 0:
        raise MigrationError(f"Found {invalid_parents} nodes with invalid parent references")
    
    # Validate vector embeddings virtual table is populated
    nodes_vec_count = connection.execute("SELECT COUNT(*) FROM nodes_vec").scalar()
    ...
    if nodes_vec_count != nodes_with_embeddings_count:
        raise MigrationError(f"Vector table count mismatch: {nodes_vec_count} vs {nodes_with_embeddings_count} nodes with embeddings")
```
...
class NodeService:
    ...
    def create_node(self, node_data: Dict[str, Any]) -> Node:
        ...
        # Fire event
        self._fire_node_event('create', node_type.value, None, self._node_to_dict(node))
...
### 5.2 New API Endpoints (`api/nodes.py`)
...
def create_node(
    ...
    """Create a new node"""
    try:
        created_node = node_service.create_node(node.dict())
...
def search_nodes(
    ...
    # but works across all node types or filtered by node_type
    pass
...
```python
class GraphService:
    """Updated to work with unified nodes table"""
    ...
    def _build_graph(self):
        """Build graph structure from nodes table"""
        self.nodes = {}
        self.edges = {}
        ...
        # Load node links
        links = self.db.query(NodeLink).all()
        for link in links:
...
class NodeEventHandler:
    """Handle node events and maintain backward compatibility"""
    
    def __init__(self, db: Session):
        self.db = db
    
    def fire_node_event(self, event_type: str, node_type: str, old_data: dict, new_data: dict):
        """Fire unified node event and legacy events for backward compatibility"""
        # Create unified node event
        event = NodeEvent(
...
def test_migration_data_integrity():
    ...
    assert post_counts['nodes'] == sum(pre_counts.values())
...
def test_nodes_api_crud():
    """Test complete CRUD operations on nodes API"""
