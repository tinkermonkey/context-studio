...
class TestIndividualCRUD:
    ...
    def test_create_individual_with_multiple_classes(self, client, individual_test_class):
        ...
        class2_response = client.post(f"/api/schemes/{individual_test_class['scheme_id']}/classes", json={
            "title": "Second Class"
        })
        class2_id = class2_response.json()["id"]
    ...
    def test_create_individual_with_duplicate_title_same_class_fails(self, client, individual_test_class):
        """POST /api/individuals with duplicate title in same class returns 409."""
        # Create first individual
        response1 = client.post("/api/individuals", json={
            "class_ids": individual_test_class["class_id"],
        ...
        response2 = client.post("/api/individuals", json={
            "class_ids": individual_test_class["class_id"],
            "title": "Duplicate Title Individual"
        })
    ...
    def test_remove_last_parent_class_fails(self, client, individual_test_class):
        ...
        response = client.delete(f"/api/individuals/{individual_id}/classes/{individual_test_class['class_id']}")
        assert response.status_code == status.HTTP_400_BAD_REQUEST
    ...
    def test_reorder_individual_classes(self, client, individual_test_class):
        ...
        response = client.put(f"/api/individuals/{individual_id}/classes", json={
            "class_ids": [class2_id, individual_test_class["class_id"]]
        })
        assert response.status_code == status.HTTP_200_OK
        body = response.json()
