...
def test_configuration_error_on_invalid_save():
    """Test that ConfigurationError is raised for save errors."""
    with tempfile.TemporaryDirectory() as tmpdir:
        config_file = os.path.join(tmpdir, "config.json")
        ...
        # Create an AppConfiguration with invalid section data
        # (missing required fields) to trigger a validation error during save
        config = AppConfiguration(
            sections={"invalid_section": {"missing": "field"}}
        )
