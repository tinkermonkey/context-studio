...
class TestEnvironment:
    ...
    def __init__(self, temp_dir: Optional[str] = None, config_overrides: Optional[Dict[str, Any]] = None):
        """
    ...
    def setup(self) -> Tuple[Settings, Engine, sessionmaker]:
        ...
        self.settings = self.config_manager.get_test_settings(self.config_overrides)
        
        # Create primary database with migrations
        self.primary_engine, self.primary_session_local, self.primary_db_path = (
...
@contextmanager
def isolated_test_environment(config_overrides: Optional[Dict[str, Any]] = None, temp_dir: Optional[str] = None):
    """
    ...
    Example:
        with isolated_test_environment({"database.check_same_thread": False}) as env:
...
@contextmanager
def isolated_test_session(config_overrides: Optional[Dict[str, Any]] = None, temp_dir: Optional[str] = None):
    """
    Context manager for creating an isolated test session with complete environment.
    
    This is a convenience context manager that sets up a complete test environment
...
class TestEnvironmentBuilder:
    ...
    def with_config_dict(self, overrides: Dict[str, Any]) -> "TestEnvironmentBuilder":
    ...
    def context(self):
        ...
        with isolated_test_environment(self.config_overrides, self.temp_dir) as env:
            yield env
...
def verify_no_test_artifacts():
    ...
    if db_files:
        raise AssertionError(f"Test database files found in root directory: {db_files}")
    
    # Check for test configuration files
    config_files = list(current_dir.glob("*test*.json"))
    config_files.extend(current_dir.glob("test_config.*"))
    if config_files:
        raise AssertionError(f"Test configuration files found in root directory: {config_files}")
    
    # Check for temporary directories
    temp_dirs = [d for d in current_dir.iterdir() if d.is_dir() and "test_env_" in d.name]
    if temp_dirs:
        raise AssertionError(f"Test temporary directories found in root directory: {temp_dirs}")
