...
def test_kg_db_with_content():
    ...
    try:
        ...
        terms_data = [
            ...
            ("Cellular Respiration", "Process converting glucose to ATP energy"),
            ("DNA Replication", "Copying genetic information before cell division"),
...
class TestMultiParagraphPerformance:
    ...
    @pytest.mark.asyncio
    async def test_single_paragraph_performance(self, test_kg_db_with_content, test_ops_db):
        ...
        try:
            ...
            # Performance targets
            assert elapsed < 15.0, f"Single paragraph took {elapsed:.2f}s (target: <15s)"
    ...
    @pytest.mark.asyncio
    async def test_two_paragraph_performance(self, test_kg_db_with_content, test_ops_db):
        ...
        try:
            ...
            assert response.request_id is not None
            assert elapsed < 30.0, f"2 paragraphs took {elapsed:.2f}s (target: <30s)"
    ...
    @pytest.mark.asyncio
    async def test_three_paragraph_performance(self, test_kg_db_with_content, test_ops_db):
        ...
        try:
            ...
            assert response.request_id is not None
            assert elapsed < 60.0, f"3 paragraphs took {elapsed:.2f}s (target: <60s)"
    ...
    @pytest.mark.asyncio
    async def test_five_paragraph_performance(self, test_kg_db_with_content, test_ops_db):
        ...
        try:
            ...
            assert elapsed < 120.0, f"5 paragraphs took {elapsed:.2f}s (MAX budget: <120s)"

            print(f"\n5 paragraphs: {elapsed:.2f}s total, {response.metrics.total_sentences} sentences")
            print(f"  Layer 0: {response.metrics.kg_layer.execution_time_ms}ms")
            print(f"  Layer 1: {response.metrics.llm_layer.execution_time_ms}ms")
            print(f"  Layer 2: {response.metrics.nlp_layer.execution_time_ms}ms")
            print(f"  Layer 3: {response.metrics.web_layer.execution_time_ms}ms")
    ...
    @pytest.mark.asyncio
    async def test_domain_specific_biology(self, test_kg_db_with_content, test_ops_db):
    ...
    @pytest.mark.asyncio
    async def test_varying_paragraph_length(self, test_kg_db_with_content, test_ops_db):
        ...
        try:
            ...
            start = time.time()
            response = await service.extract_entities(mixed_text, enable_trace=False)
            elapsed = time.time() - start

            assert response.request_id is not None
            assert elapsed < 45.0, f"Mixed paragraphs took {elapsed:.2f}s (target: <45s)"
    ...
    @pytest.mark.asyncio
    async def test_performance_consistency_across_runs(self, test_kg_db_with_content, test_ops_db):
