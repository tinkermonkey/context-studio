...
def test_throughput_performance(client):
    ...
    logger.info(f"  Layers creation:   {timings['layers'] - timings['start']:.2f}s  ({1000*(timings['layers']-timings['start'])/num_layers:.1f} ms/op)")
    logger.info(f"  Domains creation:  {timings['domains'] - timings['layers']:.2f}s  ({1000*(timings['domains']-timings['layers'])/num_domains:.1f} ms/op)")
    logger.info(f"  Terms creation:    {timings['terms'] - timings['domains']:.2f}s  ({1000*(timings['terms']-timings['domains'])/num_terms:.1f} ms/op)")
