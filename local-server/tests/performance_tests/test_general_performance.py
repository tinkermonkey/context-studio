...
engine = init_db(
    database_url=SQLALCHEMY_DATABASE_URL, connect_args={"check_same_thread": False}
)
...
def test_general_performance(client):
    ...
    def print_mem_usage(label):
        ...
        logger.info(
            f"[PERF][MEM] {label}: Python RSS={mem_mb:.2f} MB, SQLite DB={db_size_mb:.2f} MB"
        )
    ...
    start = time.time()
    logger.info(
        f"[PERF] Starting performance test: {num_layers} layers, {num_domains_per_layer} domains/layer, {num_terms_per_domain} terms/domain, {num_relationships_per_term} relationships/term"
    )
    ...
    logger.info(
        f"[PERF] Created {num_layers} layers in {timings['layers'] - start:.2f}s"
    )
    ...
    for li, layer in enumerate(layers):
        for j in range(num_domains_per_layer):
            domain = create_domain(
                client, layer_id=layer["id"], title=f"PerfDomain_{layer['id']}_{j}"
            )
            domains.append(domain)
            domain_count += 1
            if (domain_count) % 10 == 0 or (
                domain_count
            ) == num_layers * num_domains_per_layer:
                logger.info(
                    f"[PERF] Created {domain_count}/{num_layers * num_domains_per_layer} domains..."
                )
    timings["domains"] = time.time()
    logger.info(
        f"[PERF] Created {len(domains)} domains in {timings['domains'] - timings['layers']:.2f}s"
    )
    ...
    for di, domain in enumerate(domains):
        ...
        for k in range(num_terms_per_domain):
            term = create_term(
                client, domain=domain, name=f"PerfTerm_{domain['id']}_{k}"
            )
    ...
    logger.info(
        f"[PERF] Created {total_terms} terms in {timings['terms'] - timings['domains']:.2f}s"
    )
        ...
        for idx, term in enumerate(domain_terms):
            for r in range(num_relationships_per_term):
                ...
                create_relationship(
                    ...
                    target_term_id=term_ids[target_idx],
                )
                relationships_created += 1
                if (
                    relationships_created % 1000 == 0
                    or relationships_created == total_relationships
                ):
                    logger.info(
                        f"[PERF] Created {relationships_created}/{total_relationships} relationships..."
                    )
    timings["relationships"] = time.time()
    logger.info(
        f"[PERF] Created {total_relationships} relationships in {timings['relationships'] - timings['terms']:.2f}s"
    )
    ...
    layer_search_titles = (
        layer_search_titles * ((num_vector_searches // len(layer_search_titles)) + 1)
    )[:num_vector_searches]
    search_times = []
    total_results = 0
    last_log = 0
    for i, search_title in enumerate(layer_search_titles, 1):
        ...
        if percent // 10 > last_log:
            ms_per_search = (
                (sum(search_times) / len(search_times)) * 1000 if search_times else 0
            )
            logger.info(
                f"[PERF] [Layers] Vector search progress: {percent}% ({i}/{num_vector_searches}), avg {ms_per_search:.2f} ms/search so far"
            )
            last_log = percent // 10
    avg_search_time = sum(search_times) / len(search_times)
    timings["vector_search_layers"] = time.time()
    logger.info(
        f"[PERF] [Layers] Ran {num_vector_searches} vector searches, avg {avg_search_time:.4f}s/search, total results: {total_results}"
    )

    # Domains (use ANN if available, else fallback to normal search)
    domain_search_titles = [d["title"] for d in domains]
    domain_search_titles = (
        domain_search_titles * ((num_vector_searches // len(domain_search_titles)) + 1)
    )[:num_vector_searches]
    domain_search_times = []
    ...
    for i, search_title in enumerate(domain_search_titles, 1):
        ...
        if percent // 10 > last_log:
            ms_per_search = (
                (sum(domain_search_times) / len(domain_search_times)) * 1000
                if domain_search_times
                else 0
            )
            logger.info(
                f"[PERF] [Domains] Vector search progress: {percent}% ({i}/{num_vector_searches}), avg {ms_per_search:.2f} ms/search so far"
            )
            last_log = percent // 10
    avg_domain_search_time = sum(domain_search_times) / len(domain_search_times)
    timings["vector_search_domains"] = time.time()
    logger.info(
        f"[PERF] [Domains] Ran {num_vector_searches} vector searches, avg {avg_domain_search_time:.4f}s/search, total results: {domain_total_results}"
    )
    ...
    term_search_titles = (
        term_search_titles * ((num_vector_searches // len(term_search_titles)) + 1)
    )[:num_vector_searches]
    term_search_times = []
    ...
    for i, search_title in enumerate(term_search_titles, 1):
        ...
        if percent // 10 > last_log:
            ms_per_search = (
                (sum(term_search_times) / len(term_search_times)) * 1000
                if term_search_times
                else 0
            )
            logger.info(
                f"[PERF] [Terms] Vector search progress: {percent}% ({i}/{num_vector_searches}), avg {ms_per_search:.2f} ms/search so far"
            )
            last_log = percent // 10
    avg_term_search_time = sum(term_search_times) / len(term_search_times)
    timings["vector_search_terms"] = time.time()
    logger.info(
        f"[PERF] [Terms] Ran {num_vector_searches} vector searches, avg {avg_term_search_time:.4f}s/search, total results: {term_total_results}"
    )
    ...
    if initial_mem is not None and final_mem is not None:
        logger.info(
            f"[PERF][MEM][SUMMARY] Python RSS delta: {final_mem - initial_mem:.2f} MB (from {initial_mem:.2f} MB to {final_mem:.2f} MB)"
        )
    if initial_db is not None and final_db is not None:
        logger.info(
            f"[PERF][MEM][SUMMARY] SQLite DB size delta: {final_db - initial_db:.2f} MB (from {initial_db:.2f} MB to {final_db:.2f} MB)"
        )
    ...
    logger.info(
        f"  Layers creation:         {timings['layers'] - timings['start']:.2f}s  ({1000*(timings['layers']-timings['start'])/n_layers:.1f} ms/op)"
    )
    logger.info(
        f"  Domains creation:        {timings['domains'] - timings['layers']:.2f}s  ({1000*(timings['domains']-timings['layers'])/n_domains:.1f} ms/op)"
    )
    logger.info(
        f"  Terms creation:          {timings['terms'] - timings['domains']:.2f}s  ({1000*(timings['terms']-timings['domains'])/n_terms:.1f} ms/op)"
    )
    logger.info(
        f"  Relationships creation:  {timings['relationships'] - timings['terms']:.2f}s  ({1000*(timings['relationships']-timings['terms'])/n_relationships:.1f} ms/op)"
    )
    logger.info(
        f"  Validation:              {timings['validation'] - timings['relationships']:.2f}s"
    )
    logger.info(
        f"  Vector search (layers):  {timings['vector_search_layers'] - timings['validation']:.2f}s  (avg {avg_search_time*1000:.2f} ms/search, {num_vector_searches} searches)"
    )
    logger.info(
        f"  Vector search (domains): {timings['vector_search_domains'] - timings['vector_search_layers']:.2f}s  (avg {avg_domain_search_time*1000:.2f} ms/search, {num_vector_searches} searches)"
    )
    logger.info(
        f"  Vector search (terms):   {timings['vector_search_terms'] - timings['vector_search_domains']:.2f}s  (avg {avg_term_search_time*1000:.2f} ms/search, {len(term_search_titles)} searches)"
    )
    logger.info(
        f"  Total:                   {timings['vector_search_terms'] - timings['start']:.2f}s\n"
    )
