...
from database.utils import (
    DatabaseManager, PoolStrategy, PoolConfiguration, ConnectionMetrics,
    get_database_manager, cleanup_database_resources
)
...
class TestDatabaseManager:
    ...
    def test_get_optimal_pool_strategy(self, database_manager):
        ...
        strategy = database_manager.get_optimal_pool_strategy("postgresql://user:pass@host/db")
    ...
    def test_create_optimized_engine_sqlite_memory(self, database_manager, memory_db_url):
        """Test optimized engine creation for SQLite in-memory database."""
        engine = database_manager.create_optimized_engine(memory_db_url, "memory_engine")
    ...
    def test_get_optimized_session_auto_create_engine(self, database_manager, test_db_url):
    ...
    def test_perform_health_check(self, database_manager, test_db_url):
        ...
        # Check structure
        required_keys = ["timestamp", "overall_status", "engines", "metrics", "warnings", "errors"]
    ...
    def test_metrics_summary(self, database_manager, test_db_url):
        ...
        required_keys = [
            "total_connections_created", "active_connections", "peak_connections",
            "pool_hits", "pool_misses", "connection_errors", "avg_connection_time_ms",
            "total_queries_executed", "avg_query_time_ms", "uptime_seconds",
    ...
    def test_get_performance_report(self, database_manager, test_db_url):
        ...
        required_keys = ["report_timestamp", "health_status", "performance_metrics",
                        "active_engines", "recommendations"]
        for key in required_keys:
...
class TestGlobalDatabaseManager:
    ...
    def test_cleanup_database_resources(self):
        ...
        assert database.utils._database_manager is None, "Global database manager should be None after cleanup"
        ...
        assert new_manager_id != original_manager_id, "Should get a new manager instance after cleanup"
        assert len(new_manager._engines) == 0, f"New manager should be clean, but has engines: {list(new_manager._engines.keys())}"
