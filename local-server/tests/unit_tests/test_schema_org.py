...
def test_manager_parse_and_populate(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr('schema_org.manager.generate_embedding', lambda text: _fake_embedding(text))
    ...
    search_res = svc.semantic_search(query='Fruit', search_type='entities', limit=5, similarity_threshold=0.0)
...
def test_refresh_handles_backup_failure(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(SchemaOrgManager, '_create_backup', lambda self: (_ for _ in ()).throw(BackupError('permission_denied')))
