...
class TestGraphMetrics:
    ...
    def test_valid_graph_metrics(self):
        ...
        metrics = GraphMetrics(
            node_count=100, edge_count=250, density=0.5, connected_components=1
        )
    ...
    def test_graph_metrics_single_component(self):
        ...
        metrics = GraphMetrics(
            node_count=5, edge_count=4, density=0.2, connected_components=1
        )
    ...
    def test_graph_metrics_single_node(self):
        ...
        metrics = GraphMetrics(
            node_count=1, edge_count=0, density=0.0, connected_components=1
        )
    ...
    def test_graph_metrics_density_boundaries(self):
        ...
        # Minimum density
        metrics = GraphMetrics(
            node_count=10, edge_count=0, density=0.0, connected_components=10
        )
    ...
    def test_graph_metrics_invalid_node_count(self):
        ...
        with pytest.raises(
            ValueError, match="node_count must be a non-negative integer"
        ):
            GraphMetrics(
                node_count=-1, edge_count=0, density=0.0, connected_components=1
            )
    ...
    def test_graph_metrics_invalid_edge_count(self):
        ...
        with pytest.raises(
            ValueError, match="edge_count must be a non-negative integer"
        ):
            GraphMetrics(
                node_count=10, edge_count=-1, density=0.5, connected_components=1
            )
    ...
    def test_graph_metrics_invalid_density(self):
        ...
        # Density > 1.0
        with pytest.raises(
            ValueError, match="density must be a number between 0.0 and 1.0"
        ):
            GraphMetrics(
                node_count=10, edge_count=25, density=1.5, connected_components=1
            )
        ...
        with pytest.raises(
            ...
            GraphMetrics(
                node_count=10, edge_count=25, density=-0.1, connected_components=1
            )
        ...
        with pytest.raises(
            ...
            GraphMetrics(
                node_count=10, edge_count=25, density=True, connected_components=1
            )

    def test_graph_metrics_invalid_connected_components(self):
        """Test that invalid connected_components raises ValueError."""
        with pytest.raises(
            ValueError, match="connected_components must be a positive integer"
        ):
            GraphMetrics(
                node_count=10, edge_count=25, density=0.5, connected_components=0
            )
...
class TestPathResult:
    ...
    def test_valid_path_result(self):
        ...
        path = PathResult(path=("node1", "node2", "node3"), length=2, weight=1.5)
    ...
    def test_path_result_single_node(self):
        ...
        path = PathResult(path=("node1",), length=0, weight=0.0)
        assert path.path == ("node1",)
        assert path.length == 0

    def test_path_result_empty_path(self):
        """Test empty path result."""
        path = PathResult(path=(), length=0, weight=0.0)
        assert path.path == ()
        assert path.length == 0
    ...
    def test_path_result_long_path(self):
        ...
        path = PathResult(path=nodes, length=9, weight=4.5)
    ...
    def test_path_result_invalid_path_type(self):
        ...
        with pytest.raises(TypeError, match="path must be a tuple"):
            PathResult(
                path=["node1", "node2"], length=1, weight=1.0  # List instead of tuple
            )
    ...
    def test_path_result_invalid_node_id(self):
        ...
        with pytest.raises(TypeError, match="All node IDs in path must be strings"):
            PathResult(
                path=(123, "node2"), length=1, weight=1.0  # Integer instead of string
            )
    ...
    def test_path_result_invalid_length(self):
        ...
        with pytest.raises(ValueError, match="length must be a non-negative integer"):
            PathResult(
                path=("node1", "node2"), length=-1, weight=1.0  # Negative length
            )
    ...
    def test_path_result_length_validation(self):
        ...
        with pytest.raises(ValueError, match="length must equal number of edges"):
            PathResult(
                path=("node1", "node2", "node3"), length=1, weight=1.5  # Should be 2
            )
    ...
    def test_path_result_invalid_weight(self):
        ...
        with pytest.raises(ValueError, match="weight must be a number"):
            PathResult(path=("node1", "node2"), length=1, weight=True)
...
class TestKnowledgeGraph:
    ...
    def test_valid_knowledge_graph(self):
        ...
        graph = KnowledgeGraph(
            ...
            edges=("edge1", "edge2"),
            metrics=GraphMetrics(
                node_count=3, edge_count=2, density=0.5, connected_components=1
            ),
        )
    ...
    def test_knowledge_graph_empty_nodes_and_edges(self):
        ...
        graph = KnowledgeGraph(taxonomy_id="tax-001", nodes=(), edges=(), metrics=None)
        assert graph.nodes == ()
        assert graph.edges == ()
    ...
    def test_knowledge_graph_invalid_taxonomy_id(self):
        ...
        with pytest.raises(ValueError, match="taxonomy_id must be a non-empty string"):
            KnowledgeGraph(taxonomy_id="", nodes=("node1",), edges=(), metrics=None)
    ...
    def test_knowledge_graph_invalid_metrics(self):
        ...
        with pytest.raises(
            TypeError, match="metrics must be None or a GraphMetrics instance"
        ):
            KnowledgeGraph(
                taxonomy_id="tax-001",
                nodes=("node1",),
