...
def test_get_events_for_record(db_session):
    ...
    event1 = handler.fire_created_event(
        RecordType.STRUCTURE_NODE, record_id, {"title": "Created"}
    )
    event2 = handler.fire_updated_event(
        RecordType.STRUCTURE_NODE, record_id, {"title": "Created"}, {"title": "Updated"}
    )
    event3 = handler.fire_deleted_event(
        RecordType.STRUCTURE_NODE, record_id, {"title": "Updated"}
    )
...
def test_get_events_for_record_with_limit(db_session):
    ...
    for i in range(5):
        handler.fire_created_event(
            RecordType.STRUCTURE_NODE, record_id, {"title": f"Event {i}"}
        )
...
def test_get_unprocessed_events_filtered_by_record_type(db_session):
    ...
    handler.fire_created_event(
        RecordType.STRUCTURE_NODE_LINK, "link-1", {"source": "node-1"}
    )
...
def test_predicate_event_creation(db_session):
    ...
    event = handler.fire_created_event(
        RecordType.PREDICATE, "test-predicate-123", predicate_data
    )
...
def test_structure_node_event_creation(db_session):
    ...
    event = handler.fire_created_event(
        RecordType.STRUCTURE_NODE, "test-node-456", node_data
    )
...
def test_event_creation_with_enum_validation(db_session):
    ...
    event = handler.fire_created_event(
        RecordType.PREDICATE, "test-predicate", {"title": "Test Predicate"}
    )
