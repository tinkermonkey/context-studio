...
def test_change_event_creation_with_record_type_enum(db_session):
    ...
    record_data = {
        "id": record_id, 
        "title": "Test Layer",
        "definition": "A test layer for the change event system"
    }
    
    # Create event using new enum-based system
    event = handler.fire_created_event(record_type, record_id, record_data)
    
    # Check event was created with correct values
...
def test_change_event_update_with_record_type_enum(db_session):
    ...
    # Test data
    record_type = RecordType.STRUCTURE_NODE
    record_id = "test-domain-456"
    old_data = {"id": record_id, "title": "Old Domain"}
    new_data = {"id": record_id, "title": "Updated Domain", "definition": "Updated definition"}
    
    # Create update event
    event = handler.fire_updated_event(record_type, record_id, old_data, new_data)
...
def test_change_event_delete_with_record_type_enum(db_session):
    ...
    # Test data
    record_type = RecordType.STRUCTURE_NODE
    record_id = "test-term-789"
    record_data = {"id": record_id, "title": "Deleted Term", "definition": "This will be deleted"}
    
    # Create delete event
    event = handler.fire_deleted_event(record_type, record_id, record_data)
...
def test_get_events_for_record(db_session):
    """Test getting events for a specific record."""
    ...
    event1 = handler.fire_created_event(RecordType.STRUCTURE_NODE, record_id, {"title": "Created"})
    event2 = handler.fire_updated_event(RecordType.STRUCTURE_NODE, record_id, {"title": "Created"}, {"title": "Updated"})
    event3 = handler.fire_deleted_event(RecordType.STRUCTURE_NODE, record_id, {"title": "Updated"})
    
    # Get events for the record
    events = handler.get_events_for_record(record_id)
    
    # Should have 3 events in reverse chronological order (newest first)
    assert len(events) == 3
...
def test_get_events_for_record_with_limit(db_session):
    """Test getting events for a record with limit."""
    ...
    # Create multiple events for the same record
    for i in range(5):
        handler.fire_created_event(RecordType.STRUCTURE_NODE, record_id, {"title": f"Event {i}"})
...
def test_get_unprocessed_events_filtered_by_record_type(db_session):
    """Test getting unprocessed events filtered by record type."""
    ...
    handler.fire_created_event(RecordType.STRUCTURE_NODE_LINK, "link-1", {"source": "node-1"})
    ...
    assert node_events[0].record_type == RecordType.STRUCTURE_NODE
...
def test_predicate_event_creation(db_session):
    """Test creating predicate-specific events."""
    ...
    predicate_data = {
        "id": "test-predicate-123",
        "identifier": "hasProperty",
        "title": "Has Property",
        "definition": "Indicates that an entity has a specific property"
    }
    
    event = handler.fire_created_event(RecordType.PREDICATE, "test-predicate-123", predicate_data)
    
    assert event.record_type == RecordType.PREDICATE
    assert event.record_id == "test-predicate-123"
    assert event.event_type == "create"
    assert event.new_data == predicate_data


def test_structure_node_event_creation(db_session):
    """Test creating structure_node events (for layers, domains, terms)."""
    
    # Create event handler
    handler = ChangeEventHandler(db_session)
    
    # Test structure_node creation (could be layer, domain, or term)
    node_data = {
        "id": "test-node-456",
        "node_type": "layer",  # This distinguishes the specific type within structure_node
        "title": "Test Layer",
        "definition": "A test layer"
    }
    
    event = handler.fire_created_event(RecordType.STRUCTURE_NODE, "test-node-456", node_data)
    
    assert event.record_type == RecordType.STRUCTURE_NODE
    assert event.record_id == "test-node-456"
    assert event.event_type == "create"
    assert event.new_data == node_data


def test_event_creation_with_enum_validation(db_session):
    """Test that ChangeEvent creation validates RecordType enum properly."""
    ...
    assert event.record_type == RecordType.PREDICATE
    assert event.record_id == "test-predicate"
    assert event.event_type == "create"
...
def test_mark_event_processed(db_session):
    ...
    # Create an event
    event = handler.fire_created_event(RecordType.STRUCTURE_NODE, "test-node", {"title": "Test"})
...
def test_get_event_stats(db_session):
    ...
    handler.fire_created_event(RecordType.STRUCTURE_NODE, "node-1", {"title": "Node 1"})
    handler.fire_created_event(RecordType.PREDICATE, "pred-1", {"title": "Predicate 1"})
    handler.fire_updated_event(RecordType.STRUCTURE_NODE, "node-1", {"title": "Node 1"}, {"title": "Node 1 Updated"})
    ...
    assert stats["processed_events"] == 0  # None marked as processed yet
...
def test_change_event_delete_with_record_type_enum(db_session):
    ...
    record_data = {"id": record_id, "title": "Deleted Predicate", "definition": "This will be deleted"}
...
def test_get_events_for_record(db_session):
    ...
    event2 = handler.fire_updated_event(record_type, record_id, {"title": "Created"}, {"title": "Updated"})
...
def test_get_unprocessed_events_filtered_by_record_type(db_session):
    ...
    handler.fire_created_event(RecordType.STRUCTURE_NODE, "node-1", {"title": "Node 1"})
    handler.fire_created_event(RecordType.STRUCTURE_NODE_LINK, "link-1", {"parent": "p1", "child": "c1"})
    handler.fire_created_event(RecordType.PREDICATE, "pred-1", {"title": "Predicate 1"})
...
def test_event_creation_with_enum_validation(db_session):
    ...
    for record_type in [RecordType.STRUCTURE_NODE, RecordType.STRUCTURE_NODE_LINK, RecordType.PREDICATE]:
        event = handler.fire_created_event(record_type, f"test-{record_type.value}", {"test": "data"})
        assert event.record_type == record_type
...
def test_get_event_stats(db_session):
    ...
    # Create some test events
    handler.fire_created_event(RecordType.STRUCTURE_NODE, "node-1", {"title": "Node 1"})
    handler.fire_updated_event(RecordType.STRUCTURE_NODE, "node-1", {"title": "Node 1"}, {"title": "Updated Node 1"})
    handler.fire_created_event(RecordType.PREDICATE, "pred-1", {"title": "Predicate 1"})
