...
class TestConfigurationNotifications:
    ...
    async def test_path_specific_listeners(self):
        ...
        server_notifications = [notif for notif in self.capture.notifications if notif[0].startswith("server.")]
        non_server_notifications = [notif for notif in self.capture.notifications if not notif[0].startswith("server.")]
    ...
    async def test_mixed_sync_async_listeners(self):
        ...
        nlp_notifications = [notif for notif in self.capture.notifications if notif[0] == "nlp.edge_weight_filter"]
    ...
    async def test_notification_error_handling(self):
        ...
        notifier.register_global_listener(self.capture.async_listener)  # This should still work
    ...
    async def test_service_specific_handlers_exist(self):
        ...
        expected_patterns = ["server.", "database.", "nlp.", "llm.", "reference_sources.", "proxy_server."]
        
        for pattern in expected_patterns:
            assert pattern in notifier.listeners, f"Missing handler for {pattern}"
            assert len(notifier.listeners[pattern]) > 0, f"No handlers registered for {pattern}"
