        ...
        try:
            for table, expected_rows in snapshot.items():
                ...
                if expected_rows:
                    ...
                    expected_sorted = sorted(expected_rows, key=lambda x: str(x[first_col]))
                    current_sorted = sorted(current_data, key=lambda x: str(x[first_col]))
...
class TestMigration006DataIntegrity:
    ...
    def test_migration_data_integrity(self, migration_harness):
        ...
        assert migration_harness.validate_embeddings_migrated(), "Embedding migration failed"
...
class TestMigration006ParentChildRelationships:
    ...
    def test_migration_parent_child_relationships(self, migration_harness):
        ...
        assert migration_harness.validate_hierarchy_preservation(), "Hierarchy was not preserved correctly"
...
class TestMigration006Rollback:
    ...
    def test_rollback_preserves_original_data(self, migration_harness):
        ...
        try:
            ...
            cursor.execute("SELECT id, title, layer_id FROM domains WHERE id = 'domain-1'")
            ...
            cursor.execute("SELECT id, title, domain_id, parent_term_id FROM terms WHERE id = 'term-2'")
