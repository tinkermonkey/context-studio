...
from llm.models import PipelineType, CreatePipelineFlavorRequest, UpdatePipelineFlavorRequest, LLMConfig, PipelineFlavor
...
class TestPipelineFlavorService:
    ...
    async def test_create_flavor_success(self, flavor_service, sample_create_request, mock_flavor_row):
        ...
        with patch('llm.flavor_service.get_pipeline_session') as mock_db:
            ...
            mock_session.execute.return_value.fetchone.side_effect = [None, mock_flavor_row]
            ...
            assert mock_session.execute.call_count >= 2  # Check existing + Insert + Select
    ...
    async def test_update_flavor_success(self, flavor_service, sample_update_request, mock_flavor_row):
        ...
        with patch('llm.flavor_service.get_pipeline_session') as mock_db:
            ...
            mock_session.execute.return_value.fetchone.side_effect = [mock_flavor_row, updated_row]
            
            result = await flavor_service.update_flavor("test-flavor-id", sample_update_request)
            ...
            # Verify database calls
            assert mock_session.execute.call_count >= 2  # Get existing + Update + Get updated
            mock_session.commit.assert_called_once()
    ...
    async def test_update_nonexistent_flavor(self, flavor_service, sample_update_request):
        ...
        with patch('llm.flavor_service.get_pipeline_session') as mock_db:
            ...
            with pytest.raises(FlavorNotFoundError) as exc_info:
                await flavor_service.update_flavor("nonexistent-id", sample_update_request)
    ...
    async def test_update_default_flavor_title_forbidden(self, flavor_service, sample_update_request):
        ...
        with patch('llm.flavor_service.get_pipeline_session') as mock_db:
            ...
            default_row = [
                "default-id", "suggest_term_definition", "Default", "openai", "gpt-3.5-turbo",
                '{"temperature": 0.0}', "System prompt", "User prompt", 1, True,
