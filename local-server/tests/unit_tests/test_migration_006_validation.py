...
def test_migration_006_creates_change_events_table(temp_db):
    ...
    expected_columns = {
        ...
        "old_data", "new_data", "timestamp", "processed"
    }
    ...
    with engine.connect() as conn:
        ...
        if len(migrated_events) > 0:
            ...
            for event in migrated_events:
                event_type, record_type, record_id, old_data, new_data, processed = event
                
                # Check record_type mapping
                assert record_type in ["structure_node", "structure_node_link", "predicate"]
                
                # If it was a node event, should be mapped to structure_node
                if record_id and ("layer" in str(new_data) or "domain" in str(new_data) or "term" in str(new_data)):
...
def test_migration_006_rollback_preserves_data(temp_db):
    ...
    with engine.connect() as conn:
        ...
        event_count_before = conn.execute(text("SELECT COUNT(*) FROM change_events")).scalar()
