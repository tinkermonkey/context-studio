...
def test_cosine_similarity_bytes_input():
    ...
    assert math.isclose(
        cosine_similarity(arr.tolist(), np.frombuffer(b, dtype=np.float32).tolist()),  # noqa: E501
        1.0,
        abs_tol=1e-6,
    )
...
def test_layer_bad_input(shared_client):
    ...
    resp = shared_client.post(
        "/api/structure_nodes/", json={"node_type": "layer", "definition": "Def"}  # noqa: E501
    )
...
def test_layer_duplicate_title(shared_client):
    resp1 = shared_client.post(
        "/api/structure_nodes/", json={"node_type": "layer", "title": "Layer X"}  # noqa: E501
    )
...
def test_update_layer_to_duplicate_title(shared_client):
    resp1 = shared_client.post(
        "/api/structure_nodes/", json={"node_type": "layer", "title": "Layer A"}  # noqa: E501
    )
    resp2 = shared_client.post(
        "/api/structure_nodes/", json={"node_type": "layer", "title": "Layer B"}  # noqa: E501
    )
    resp1.json()["id"]
    id_b = resp2.json()["id"]
    resp = shared_client.put(f"/api/structure_nodes/{id_b}", json={"title": "Layer A"})  # noqa: E501
