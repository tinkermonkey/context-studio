...
def test_selected_relation_model():
    ...
    relation_no_text = SelectedRelation(
        predicate="RelatedTo", object="another term", weight=2.5
    )
...
def test_component_term_model():
    ...
    relations = [
        SelectedRelation(
            predicate="used for", object="target1", weight=5.0, text="text1"
        ),
        SelectedRelation(predicate="RelatedTo", object="target2", weight=3.5),
    ]
...
def test_error_response_models():
    ...
    error_no_details = LLMErrorResponse(error="Simple error", error_type="ConfigError")
...
if __name__ == "__main__":
    try:
        ...
        print(
            "  ✓ SelectedRelation: ConceptNet relation with predicate, object, weight, text"
        )
        print("  ✓ ComponentTerm: Term with definitions and relations collections")
        print(
            "  ✓ DefinitionSuggestionRequest: Complete request model with exact schema compliance"
        )
        print(
            "  ✓ DefinitionSuggestionResponse: Structured response with definition/reasoning/discrepancies"
        )
