...
def create_layer(shared_client, title="Test Layer"):
    resp = shared_client.post("/api/structure_nodes/", json={"node_type": "layer", "title": title})
...
def test_create_get_update_delete_domain(shared_client):
    ...
    resp = shared_client.put(f"/api/structure_nodes/{domain_id}", json={"title": "Updated Domain"})
    assert resp.status_code == 200
...
def test_domain_duplicate_title(shared_client):
    layer_id = create_layer(shared_client, str(uuid.uuid4()))
    resp1 = shared_client.post("/api/structure_nodes/", json={
        "node_type": "domain",
        "title": "Unique Domain",
        "definition": "A domain.",
        "parent_node_id": layer_id
    ...
    resp2 = shared_client.post("/api/structure_nodes/", json={
        "node_type": "domain",
        "title": "Unique Domain",
    ...
    assert resp2.status_code == 409  # Conflict is more appropriate than Bad Request for duplicates
...
def test_list_domains(shared_client):
    layer_id = create_layer(shared_client, str(uuid.uuid4()))
    for i in range(3):
        shared_client.post("/api/structure_nodes/", json={
            "node_type": "domain",
            "title": f"Domain {i}",
            "definition": f"Def {i}",
            "parent_node_id": layer_id
    ...
    resp = shared_client.get(f"/api/structure_nodes/?node_type=domain&parent_node_id={layer_id}")
    assert resp.status_code == 200
...
def test_move_domains_with_terms(shared_client):
    ...
    term_resp = shared_client.post("/api/structure_nodes/", json={
        "node_type": "term",
...
def test_move_domains_conflict_warning(shared_client):
    ...
    source_domain_resp = shared_client.post("/api/structure_nodes/", json={
        "node_type": "domain",
        "title": domain_title,
