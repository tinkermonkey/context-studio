...
def create_layer_and_domain(shared_client):
    ...
    layer_resp = shared_client.post("/api/structure_nodes/", json={
        "node_type": "layer",
    ...
    domain_resp = shared_client.post("/api/structure_nodes/", json={
        "node_type": "domain",
        "title": str(uuid.uuid4()), 
        "definition": "def", 
...
def create_term(shared_client, domain_id, layer_id, title=None, definition=None, parent_term_id=None):
    data = {
        "node_type": "term",
...
def test_create_get_update_delete_term(shared_client):
    ...
    # Update
    resp = shared_client.put(f"/api/structure_nodes/{term_id}", json={"title": "T1 Updated"})
    assert resp.status_code == 200
...
def test_term_duplicate_title_within_domain(shared_client):
    ...
    resp = shared_client.post("/api/structure_nodes/", json={
        "node_type": "term",
        "title": "T1", 
        "definition": "def", 
...
def test_term_parent_and_circular_reference(shared_client):
    ...
    t2 = create_term(shared_client, domain_id, layer_id, title="Child", parent_term_id=t1["id"])
    # Parent must exist
    bad_uuid = str(uuid.uuid4())
    resp = shared_client.post("/api/structure_nodes/", json={
        "node_type": "term",
...
def test_list_terms_pagination(shared_client):
    ...
    resp = shared_client.get(f"/api/structure_nodes/?node_type=term&parent_node_id={domain_id}&limit=2")
    assert resp.status_code == 200
    assert len(resp.json()["data"]) <= 2
    resp = shared_client.get(f"/api/structure_nodes/?node_type=term&parent_node_id={domain_id}&sort_by=title")
    assert resp.status_code == 200
...
def test_create_get_update_delete_term_relationship(shared_client):
    ...
    resp = shared_client.post("/api/structure_nodes/links", json={
    ...
    resp = shared_client.get(f"/api/structure_nodes/links?source_node_id={t1['id']}&target_node_id={t2['id']}")
    assert resp.status_code == 200
    links = resp.json()
    assert len(links) == 1
    assert links[0]["predicate"] == "rel"
    # Update
    resp = shared_client.put(f"/api/structure_nodes/links/{rel_id}", json={
        "source_node_id": t1["id"], 
        "target_node_id": t2["id"], 
        "predicate": "rel2"
    })
...
def test_term_relationship_invalid_cases(shared_client):
    ...
    resp1 = shared_client.post("/api/structure_nodes/links", json={
        "source_node_id": t1["id"], 
        "target_node_id": t2["id"], 
        "predicate": "rel"
    })
    assert resp1.status_code == 201
    resp2 = shared_client.post("/api/structure_nodes/links", json={
        "source_node_id": t1["id"], 
        "target_node_id": t2["id"], 
        "predicate": "rel"
    })
...
def test_move_terms_basic(shared_client):
    ...
    target_domain_resp = shared_client.post("/api/structure_nodes/", json={
        "node_type": "domain",
        "title": str(uuid.uuid4()),
    ...
    term = create_term(shared_client, source_domain_id, source_layer_id, 
                      title=f"Test Term {uuid.uuid4()}")
    
    # Move the term to the target domain using structure_nodes move API
    move_resp = shared_client.post("/api/structure_nodes/move", json={
        "node_ids": [term["id"]],
...
def test_move_terms_with_children(shared_client):
    ...
    # Create parent term
    parent_term = create_term(shared_client, source_domain_id, source_layer_id,
                             title=f"Parent Term {uuid.uuid4()}")
    
    # Create child term
    child_term = create_term(shared_client, source_domain_id, source_layer_id,
...
def test_move_terms_conflict_warning(shared_client):
    ...
    source_term = create_term(shared_client, source_domain_id, source_layer_id,
                             title=term_title)
    
    # Create term with same title in target domain
    target_term = create_term(shared_client, target_domain_id, source_layer_id,
                             title=term_title)
...
def test_move_terms_invalid_target(shared_client):
    ...
    assert "not exist" in move_resp.json()["detail"] or "not found" in move_resp.json()["detail"].lower()
...
def test_safe_deletion_workflow(shared_client):
    ...
    # Create child terms
    child1 = create_term(shared_client, source_domain_id, source_layer_id,
                        title=f"Child 1 {uuid.uuid4()}",
                        parent_term_id=parent_term["id"])
    child2 = create_term(shared_client, source_domain_id, source_layer_id,
    ...
    for child_id in children_ids:
        update_resp = shared_client.put(f"/api/structure_nodes/{child_id}", json={
            "parent_node_id": source_domain_id
        })
