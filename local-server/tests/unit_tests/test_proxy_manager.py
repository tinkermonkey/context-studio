...
class TestReferenceAPIProxyManager:
    ...
    def test_start_proxy_import_error(self, mock_wait, mock_get_settings):
        ...
        with patch('builtins.__import__', side_effect=ImportError("No module named 'reference_api_buddy'")):
            result = manager.start_proxy()
    ...
    def test_restart_proxy(self, mock_get_settings):
        ...
        with patch.object(manager, 'stop_proxy') as mock_stop, \
             patch.object(manager, 'start_proxy', return_value=True) as mock_start:
