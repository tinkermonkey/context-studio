...
def poll_until(
    ...
    a truthy value or the timeout is exceeded. Useful for waiting for asynchronous
    ...
    while True:
        ...
        if elapsed >= effective_timeout:
            ...
            raise TimeoutError(
                f"Timeout waiting for {description} after {effective_timeout:.1f}s. "
...
def poll_until_no_exception(
    ...
    Raises:
        TimeoutError: If the function continues to raise exceptions after timeout
    """
    start_time = time.time()
    last_exception = None
