...
def test_create_and_get_domain(client):
    ...
    domain_id = create_domain(client, layer_id, title=f"Physics_{uuid4()}", definition="The study of matter and energy.")
...
def test_list_domains(client):
    ...
    resp = client.get(f"/api/structure_nodes/?node_type=domain&parent_node_id={layer_id}")
...
def test_domains_pagination(client):
    ...
    resp = client.get(f"/api/structure_nodes/?node_type=domain&parent_node_id={layer_id}&limit=2&sort_by=title")
    assert resp.status_code == 200
    data = resp.json()
    ...
    # Test second page
    resp2 = client.get(f"/api/structure_nodes/?node_type=domain&parent_node_id={layer_id}&skip=2&limit=2&sort_by=title")
