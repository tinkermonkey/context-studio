...
from triage_scripts.triage_helper import create_test_app_with_migrations, create_test_client, cleanup_test_database
from sqlalchemy import text
...
def test_schema_org_endpoints_flow(tmp_path, monkeypatch):
    ...
    try:
        ...
        monkeypatch.setattr('embeddings.generate_embeddings.generate_embedding', _deterministic_embedding)
        monkeypatch.setattr('schema_org.manager.generate_embedding', _deterministic_embedding)
        ...
        resp = client.get('/api/schema-org/search', params={'query': 'Fruit', 'search_type': 'both', 'similarity_threshold': 0.0})
        assert resp.status_code == 200
        sdata = resp.json()
        ...
        resp = client.get('/api/schema-org/search', params={
            'query': 'Fruit', 
        ...
        assert sdata.get('total_count', 0) >= 1, "Vector search should find the Fruit entity"
        ...
        assert fruit_entity.get('title') == 'Fruit', f"Expected 'Fruit', got '{fruit_entity.get('title')}'"
        ...
        assert pdata.get('total_count', 0) >= 1, "Vector search should find the color property"
        ...
        color_prop = pitems[0]
        assert color_prop.get('title') == 'color', f"Expected 'color', got '{color_prop.get('title')}'"
    finally:
