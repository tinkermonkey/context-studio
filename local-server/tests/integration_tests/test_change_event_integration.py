...
def test_mixed_event_processing_with_filtering(db_session, change_event_handler):
    ...
    node_unprocessed = change_event_handler.get_unprocessed_events(
        record_type=RecordType.STRUCTURE_NODE
    )
    link_unprocessed = change_event_handler.get_unprocessed_events(
        record_type=RecordType.STRUCTURE_NODE_LINK
    )
    predicate_unprocessed = change_event_handler.get_unprocessed_events(
    ...
    processor = EventProcessor(
        database_url=database_url, poll_interval=0.05, max_events=20
    )
...
def test_normalized_change_event_integration(db_session):
    ...
    node_event = handler.fire_created_event(
        RecordType.STRUCTURE_NODE, node_id, {"title": "Normalized Test"}
    )
...
def test_event_data_integrity_integration(db_session, change_event_handler):
    ...
    event = change_event_handler.fire_created_event(
        RecordType.STRUCTURE_NODE, integrity_node_id, complex_data
    )
...
def test_high_volume_event_processing(db_session, change_event_handler):
    ...
    for i in range(50):  # Create 50 events of mixed types
        ...
        events.append(
            change_event_handler.fire_created_event(
                record_type, unique_id, {"id": unique_id, "title": f"Volume Test {i}"}
            )
        )
    ...
    processor = EventProcessor(
        database_url=database_url, poll_interval=0.05, max_events=100
    )
