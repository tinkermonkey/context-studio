...
def test_create_term_with_parent_and_circular_reference(client):
    ...
    resp = client.put(f"/api/structure_nodes/{parent['id']}", json={"parent_term_id": child["id"]})
...
def test_find_term_invalid_created_at(client):
    ...
    resp = client.post("/api/structure_nodes/find", json={
        "title": "test",
...
def test_terms_pagination(client):
    ...
    # Test first page
    resp = client.get(f"/api/structure_nodes/?parent_node_id={domain_id}&node_type=term&limit=2&sort_by=title")
    assert resp.status_code == 200
    data = resp.json()
    assert len(data["data"]) == 2
    assert data["total"] == 5
    ...
    # Test second page
    resp2 = client.get(f"/api/structure_nodes/?parent_node_id={domain_id}&node_type=term&skip=2&limit=2&sort_by=title")
    assert resp2.status_code == 200
    data2 = resp2.json()
    assert len(data2["data"]) == 2
    
    # Test last page
    resp3 = client.get(f"/api/structure_nodes/?parent_node_id={domain_id}&node_type=term&skip=4&limit=2&sort_by=title")
