...
class TestPredicateAPIIntegration:
    ...
    def test_list_predicates_pagination(self, client, db_session):
        ...
        response = client.get("/api/predicates/?skip=0&limit=100")  # Get more than we expect
        assert response.status_code == 200
        data = response.json()
    ...
    def test_list_predicates_sorting(self, client, db_session):
        ...
        expected_titles = [f"{test_prefix}_Alpha", f"{test_prefix}_Beta", f"{test_prefix}_Zebra"]
        ...
        expected_identifiers = [f"{test_prefix.lower()}_alpha", f"{test_prefix.lower()}_beta", f"{test_prefix.lower()}_zebra"]
    ...
    def test_update_predicate_partial(self, client, db_session):
        ...
        # Create predicate
        predicate_data = {"title": f"Original Title {test_id}", "definition": "Original definition"}
        create_response = client.post("/api/predicates/", json=predicate_data)
        assert create_response.status_code == 201
        predicate_id = create_response.json()["id"]
