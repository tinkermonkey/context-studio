...
def build_eval_ontology() -> tuple[SQLiteOntologyRepository, SqliteSchemaVectorIndex]:
    """
...
def build_registry(nlp, embedding) -> dict[str, Variant]:
    ...
    - 'default' / 'default+grounding': the LLM pipeline
      ...
      against the `build_eval_ontology()` `eval_repo` / `eval_index`, mirroring
    ...
    if _default_cassettes_present():
        eval_repo, eval_index = build_eval_ontology()
        register_variant(_make_default_variant(eval_repo, grounding=False))
        register_variant(
            _make_default_variant(eval_repo, grounding=True, eval_index=eval_index)
        )
    else:
        print(
