...
from services.task_manager import initialize_task_manager, shutdown_task_manager  # noqa: E501
...
from reference_db.manager import get_reference_manager, cleanup_reference_manager  # noqa: E501
...
def create_app(dataset_id=None, engine=None, session_local=None, service_factory=None):  # noqa: E501
    ...
    async def lifespan(app):
        try:
            ...
            if service_factory is not None:
                ...
                app.state.service_factory = ServiceFactory(cache_ttl_seconds=3600)  # noqa: E501
            set_service_factory(app.state.service_factory)  # Set module-level reference  # noqa: E501
            ...
            if dataset_manager is None:
                ...
                logger.info("Dataset manager not available - using direct database access from config")  # noqa: E501
                ...
                if engine:
                    ...
                    fresh_engine = create_engine(settings.database.default_url, poolclass=None)  # noqa: E501
                ...
                if dataset_id:
                    ...
                    if not success:
                        logger.error(f"Failed to switch to specified dataset: {dataset_id}")  # noqa: E501
                        raise RuntimeError(f"Cannot start with dataset {dataset_id}")  # noqa: E501
                    ...
                    if existing_datasets:
                        ...
                        most_recent = max(existing_datasets, key=lambda d: d.last_accessed)  # noqa: E501
                        logger.info(f"No active dataset set, using most recent: {most_recent.title}")  # noqa: E501
                        dataset_manager.switch_dataset(most_recent.id)
                    else:
                        # No datasets exist at all - create default
                        logger.info("No datasets found, creating default dataset...")  # noqa: E501
                        dataset_manager.create_dataset("Default Dataset", "default.db")  # noqa: E501
            ...
            try:
                cache_path = operations_db_manager.operations_db_path.replace(".db", "_llm_cache.db")  # noqa: E501
            ...
            if dataset_manager is None:
                ...
                if engine is None:
                    ...
                    if db_url.startswith("sqlite:///"):
                        ...
                        logger.info(f"Database migrations applied to: {db_path}")  # noqa: E501
            else:
                ...
                if active_dataset:
                    ...
                    logger.info(f"Database migrations applied for dataset: {active_dataset.title}")  # noqa: E501
                else:
                    logger.warning("No active dataset found after initialization.")  # noqa: E501
            ...
            if dataset_manager is not None and active_dataset:
                ...
                if current_engine is None:
                    logger.warning("No current engine available, skipping event processor initialization")  # noqa: E501
                    ...
                    try:
                        ...
                        session_maker = session_local or get_current_session_local()  # noqa: E501
                        if session_maker is None:
                            raise RuntimeError("No session local available for version manager")  # noqa: E501

                        db_session = session_maker()
                        version_manager = VersionManager(db_session)
                        working_tree_manager = WorkingTreeManager(db_session, version_manager)  # noqa: E501
                        logger.info("VersionManager and WorkingTreeManager created for EventProcessor")  # noqa: E501
            ...
            if dataset_manager is not None:
                ...
                try:
                    ...
                    if pipeline.get_nlp() is not None:
                        ...
                        logger.warning(f"NLP pipeline preload failed: {error_msg}")  # noqa: E501
                except Exception as e:
                ...
                try:
                    ...
                    if graph_service:
                        logger.info("GraphService successfully warmed up and cached")  # noqa: E501
                    else:
                    ...
                    logger.info("Continuing startup despite GraphService warmup failure")  # noqa: E501
            else:
                logger.info("Skipping NLP and GraphService warmup (dataset manager not available)")  # noqa: E501
            ...
            try:
                ...
                logger.info("Loading embedding model (this may take a moment)...")  # noqa: E501
                ...
                logger.info(f"Reference DB and Embedding Model warmed up successfully in {warmup_time:.0f}ms")  # noqa: E501
                logger.info("Subsequent embedding/search operations will be fast (~20-50ms)")  # noqa: E501

            except Exception as e:
                logger.error(f"Error warming up Reference DB/Embeddings: {e}")
                # Continue startup even if warmup fails
                logger.info("Continuing startup despite Reference DB warmup failure")  # noqa: E501
            ...
            logger.info("Initializing TaskManager for background task processing...")  # noqa: E501
            try:
                task_manager = initialize_task_manager(max_queue_size=100, max_dlq_size=1000)  # noqa: E501
            ...
            if hasattr(app.state, "event_processor") and app.state.event_processor:  # noqa: E501
    ...
    app.include_router(predicates.router, prefix="/api/predicates", tags=["predicates"])  # noqa: E501
    ...
    app.include_router(property_definitions.router, tags=["property_definitions"])  # noqa: E501
    app.include_router(pipeline_configurations.router, tags=["pipeline_configurations"])  # noqa: E501
    ...
    app.include_router(changeset_management.router, tags=["changeset-management"])  # noqa: E501
    app.include_router(proposal_management.router, tags=["proposal-management"])  # noqa: E501
    app.include_router(identity_management.router, tags=["identity-management"])  # noqa: E501

    # Phase 3: Enhanced database management monitoring endpoints
    app.include_router(database_monitoring.router, tags=["database-monitoring"])  # noqa: E501

    # Enhanced Event Processor monitoring endpoints
    app.include_router(event_processor_monitoring.router, tags=["event-processor-monitoring"])  # noqa: E501

    # Phase 4: Advanced collaborative features
    app.include_router(conflict_resolution.router, tags=["conflict-resolution"])  # noqa: E501
    ...
    app.include_router(rag_pipeline.router, prefix="/api/rag", tags=["rag-pipeline"])  # noqa: E501
    ...
    async def global_exception_handler(request: Request, exc: Exception):
        ...
        error_msg = re.sub(r"[A-Z]:\\[\w\-\\]+\\[\w\-.\\]+\.py", "[REDACTED]", error_msg)  # noqa: E501
        ...
        return JSONResponse(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, content={"detail": error_msg})  # noqa: E501
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run the Context Studio FastAPI server.")  # noqa: E501
    parser.add_argument(
        "--host",
        type=str,
