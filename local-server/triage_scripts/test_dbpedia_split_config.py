...
async def test_configuration():
    ...
    try:
        ...
        assert lookup_config.upstream_url == "https://lookup.dbpedia.org", \
            f"❌ Wrong lookup URL: {lookup_config.upstream_url}"
        print(f"✅ dbpedia_lookup URL: {lookup_config.upstream_url}")
        
        assert sparql_config.upstream_url == "https://dbpedia.org", \
        ...
        assert 'dbpedia_lookup' in enabled_sources, \
            "❌ dbpedia_lookup not in enabled sources"
        assert 'dbpedia_sparql' in enabled_sources, \
            ...
            if response.success:
                ...
                if response.results:
                    for i, result in enumerate(response.results[:3], 1):
                        ...
                        uri = result.uri if hasattr(result, 'uri') else result.get('uri', 'N/A')
                        print(f"   {i}. {label} - {uri}")
                return True
            else:
...
async def test_proxy_routing():
    ...
    try:
        ...
        if settings.proxy_server.enabled:
            ...
            print(f"✅ Proxy is enabled on {settings.proxy_server.host}:{settings.proxy_server.port}")
