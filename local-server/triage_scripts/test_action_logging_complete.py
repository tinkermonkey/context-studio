...
def main():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for i, entry in enumerate(action_log, 1):
            ...
            print(f"   {i:2}. {timestamp.strftime('%H:%M:%S')} - {entry['action'].upper()}")
            print(f"       Dataset: {entry.get('dataset_title', 'N/A')} ({entry['dataset_id'][:8]}...)")
        ...
        if os.path.exists(dm.action_log_path):
            ...
            if log_data:
                ...
                required_fields = ['timestamp', 'action', 'dataset_id', 'dataset_title', 'details']
