...
class PipelineFlavorService:
    ...
    async def list_flavors(self, pipeline: Optional[PipelineType] = None) -> List[PipelineFlavor]:
        ...
        if pipeline:
            ...
            user_flavors = await self._get_user_flavors(pipeline)
            ...
            all_user_flavors = await self._get_all_user_flavors()
    ...
    async def get_enabled_flavors(self, pipeline: PipelineType) -> List[PipelineFlavor]:
        ...
        user_flavors = await self._get_user_flavors(pipeline, enabled_only=True)
        flavors.extend(user_flavors)
    ...
    async def _get_user_flavors(self, pipeline: PipelineType, enabled_only: bool = False) -> List[PipelineFlavor]:
    ...
    async def _get_all_user_flavors(self) -> List[PipelineFlavor]:
