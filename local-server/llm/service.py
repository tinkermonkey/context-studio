...
class LLMService:
            ...
            if config and config.api_key_env_var:
                ...
                if api_key:
                    ...
                    if (
                        ...
                        missing_keys.append(
                            f"{config.api_key_env_var} (invalid format)"
                        )
                        continue
        ...
        if not has_valid_config:
            if missing_keys:
                raise LLMConfigurationError(
                    f"No valid API keys found. Missing or invalid: {', '.join(missing_keys)}"
                )
            else:
                raise LLMConfigurationError(
                    "No enabled models have API key configuration"
                )
    ...
    def execute_pipeline_flavor_sync(
    ...
    async def execute_pipeline_flavor(
        ...
        try:
            ...
            user_prompt = self._render_user_prompt_generic(
                flavor.user_prompt, request.context_data, strict=False
            )
            ...
            structured_output_class = PIPELINE_STRUCTURED_OUTPUT_MAPPING.get(
            ...
            self.logger.debug(
                f"Using timeout of {timeout} seconds for pipeline: {request.pipeline_type}"
            )
            try:
                ...
                raise LLMTimeoutError(
                    f"Request timed out after {timeout} seconds for pipeline: {request.pipeline_type}"
                )
            ...
            structured_output, response_content = (
                self._process_response_with_structured_output(
            ...
            self.logger.warning(
                f"OpenAI rate limit exceeded for pipeline {request.pipeline_type}: {e}"
            )
            self.execution_tracker.complete_execution(
                execution_id=execution_id,
                response_message="",
            ...
            self.logger.error(
                f"OpenAI authentication error for pipeline {request.pipeline_type}: {e}"
            )
            self.execution_tracker.complete_execution(
                execution_id=execution_id,
                response_message="",
            ...
            self.logger.error(
                f"Unexpected error executing pipeline {request.pipeline_type}: {e}"
            )
            self.execution_tracker.complete_execution(
                execution_id=execution_id,
                response_message="",
    ...
    async def execute_pipeline_flavor_streaming(
        ...
        try:
            ...
            yield StreamingLLMResponse(
                flavor_id=flavor.id, execution_id=execution_id, done=False
            )
            ...
            async for chunk in llm.astream(messages):
                if hasattr(chunk, "content") and chunk.content:
                    yield StreamingLLMResponse(
                        token=chunk.content, flavor_id=flavor.id, done=False
                    )
    ...
    async def _get_flavor(
        ...
        try:
            ...
            self.logger.warning(
                f"Flavor '{flavor_identifier}' not found, using default"
            )
            return self.flavor_service.get_default_flavor(pipeline)

    def _create_llm_from_flavor(
        self, flavor: PipelineFlavor, structured_output_class=None
    ):
            ...
            try:
                ...
                with warnings.catch_warnings():
                    warnings.filterwarnings(
                        "ignore", category=UserWarning, module="langchain_openai"
                    )
    ...
    def _create_base_llm(self, flavor: PipelineFlavor):
        ...
        self.logger.debug(
            f"Creating {flavor.llm_model} with config: {list(filtered_config.keys())}"
        )
        ...
        try:
            return provider_router.get_llm_for_model(
                flavor.llm_model, **filtered_config
            )
        except Exception as e:
    ...
    def _process_response_with_structured_output(
        ...
        if structured_output_class:
            ...
            structured_output = self._extract_structured_output_from_response(
                response, structured_output_class
            )

            if structured_output:
                # Convert structured output back to text for compatibility
                response_content = self._structured_output_to_text(
                    structured_output, structured_output_class
                )
                self.logger.debug(
            ...
            self.logger.debug(
                f"No structured output class configured for pipeline {pipeline_type}"
            )
    ...
    def _extract_structured_output_from_response(
        ...
        try:
            ...
            if not any(hasattr(response, field_name) for field_name in field_names):
                self.logger.debug(
                    f"Response does not have expected fields: {field_names}"
                )
                return None
            ...
            required_fields = [
                name
                for name, field in structured_output_class.model_fields.items()
                if field.is_required()
            ]

            if required_fields and not any(
                extracted_data.get(field) for field in required_fields
            ):
                self.logger.warning(
                    f"No required fields have content. Required: {required_fields}, Extracted: {extracted_data}"
                )
            ...
            self.logger.warning(
                f"Failed to extract structured output from response: {e}"
            )
            return None
    ...
    def _parse_structured_output_from_text(
        ...
        try:
            ...
            if required_fields and not any(
                parsed_output.get(field) for field in required_fields
            ):
                self.logger.warning(
                    f"No required fields found in text parsing for pipeline {pipeline_type}"
                )
                return None

            self.logger.info(
                f"Successfully parsed structured output from text for pipeline {pipeline_type}"
            )
            return parsed_output

        except Exception as e:
            self.logger.error(
                f"Failed to parse structured output from text for pipeline {pipeline_type}: {e}"
            )
            return None
                ...
                for key, value in context_data.items():
                    if value is None:
                        ...
                        if all(isinstance(item, str) for item in value):
                            safe_context[key] = (
                                ", ".join(value) if value else "Not specified"
                            )
                        else:
