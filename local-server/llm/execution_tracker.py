    ...
    def record_selection(self, selection_request: RecordSelectionRequest) -> str:
        ...
        try:
            ...
            try:
                ...
                session.commit()
                logger.info(f"Recorded selection: {selection_id} for execution: {selection_request.execution_id}")
                return selection_id
    ...
    def get_execution_analytics(
        ...
        try:
            ...
            try:
                ...
                where_clause = "WHERE started_at >= date('now', '-{} days')".format(days_back)
                if pipeline_type:
