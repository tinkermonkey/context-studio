...
for token in doc:
    ...
    print(
        f"{'Head':>10}: {token.head.text} (ID: {token.head.i}, IDX: {token.head.idx})"
    )
    print(
        f"{'Ancestors':>10}: {', '.join([ancestor.text + ' (ID: ' + str(ancestor.i) + ', IDX: ' + str(ancestor.idx) + ')' for ancestor in token.ancestors])}"
    )
    print(f"{'Conjuncts':>10}: {list(token.conjuncts)}")
    print(
        f"{'Children':>10}: {', '.join([child.text + ' (ID: ' + str(child.i) + ', IDX: ' + str(child.idx) + ')' for child in token.children])}"
    )
