...
class RAGExtractionRequest(BaseModel):
    ...
    text: str = Field(
        ..., description="Text to analyze and extract entities from.", min_length=1
    )
...
class ExtractedEntity(BaseModel):
    ...
    text: str = Field(
        ..., description="The entity text as it appears in the source.", min_length=1
    )
    type: str = Field(
        ...,
...
class LayerMetrics(BaseModel):
    ...
    execution_time_ms: float = Field(
        ..., ge=0.0, description="Execution time for this layer in milliseconds."
    )
    entities_found: int = Field(
        ..., ge=0, description="Number of entities found by this layer."
    )
    entities_deduplicated: int = Field(
        default=0,
        ge=0,
...
class ProcessingMetrics(BaseModel):
    ...
    kg_layer: LayerMetrics = Field(
        ..., description="Metrics for knowledge graph layer."
    )
    nlp_layer: LayerMetrics = Field(
        ..., description="Metrics for NLP processing layer."
    )
    llm_layer: LayerMetrics = Field(
        ..., description="Metrics for LLM enrichment layer."
    )
    web_layer: LayerMetrics = Field(..., description="Metrics for web search layer.")

    total_execution_time_ms: float = Field(
        ..., ge=0.0, description="Total pipeline execution time in milliseconds."
    )
    total_entities: int = Field(
        ..., ge=0, description="Total number of unique entities after deduplication."
    )
    total_sentences: int = Field(
        ..., ge=0, description="Total number of sentences processed."
    )
