...
class ResultNormalizer:
    ...
    def normalize_dbpedia_search_response(
        ...
        nodes = [
            SearchNode(
                ...
                relevance_score=(
                    min(result.score / max_score, 1.0) if max_score > 0 else 1.0
                ),
            )
            ...
            for binding in bindings:
                ...
                if "item" in binding:
                    ...
                    if item_uri and item_uri not in seen_entities:
                        ...
                        label = binding.get("itemLabel", {}).get(
                            "value", item_uri.split("/")[-1]
                        )
                        description = binding.get("itemDescription", {}).get(
                            "value", ""
                        )
    ...
    def normalize_conceptnet_query_response(
        ...
        for edge in response.edges:
            ...
            if edge.start and edge.start.get("@id"):
                ...
                if (
                    f"/c/{self.default_language}/" in start_id
                    and start_id not in seen_nodes
                ):
            ...
            if edge.end and edge.end.get("@id"):
                end_id = edge.end["@id"]
                if (
                    f"/c/{self.default_language}/" in end_id
                    and end_id not in seen_nodes
                ):
            ...
            if edge.start and edge.end and edge.rel:
                ...
                if edge.rel.get("@id") == "/r/ExternalURL":
                    ...
                    if end_url and any(
                        domain in end_url for domain in ["dbpedia.org", "wikidata.org"]
                    ):
                        ...
                        if "dbpedia.org" in end_url:
                            ...
                            if "/Q" in end_url:
                                ...
                                target_id = (
                                    f"wikidata:http://www.wikidata.org/entity/Q{q_id}"
                                )
                            else:
        ...
        if "results" in response.results and "bindings" in response.results["results"]:
            for binding in response.results["results"]["bindings"]:
                ...
                if all(key in binding for key in ["item", "property", "object"]):
                    ...
                    property_label = binding.get("propertyLabel", {}).get(
                        "value", property_id
                    )
                    ...
                    if object_uri not in seen_objects:
                        ...
                        if object_uri and any(
                            domain in object_uri
                            for domain in [
                            ...
                            if any(
                                ...
                                ext in file_path.lower()
                                for ext in [".mp3", ".wav", ".ogg", ".mp4", ".avi"]
                            ):
                                title = "media file"
    ...
    def normalize_wikidata_entity_response(
        ...
        entity_url = (
            response.entity_url or f"http://www.wikidata.org/entity/{entity_id}"
        )
        ...
        node = SearchNode(
            ...
            title=title,
            definition=definition,
            attributes={"entity_id": entity_id, "uri": entity_url, "raw_data": data},
